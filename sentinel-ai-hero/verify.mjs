...
// --- Document & font ---
check("title mentions SENTINEL AI", (await page.title()).includes("SENTINEL AI"));
...
check("navbar is fixed", (await nav.evaluate((el) => getComputedStyle(el).position)) === "fixed");
check("logo says SENTINEL", (await nav.locator("a", { hasText: "SENTINEL" }).first().textContent())?.trim() === "SENTINEL");
...
check(
  ...
  JSON.stringify(hrefs) === JSON.stringify(["#services", "#about-us", "#projects", "#team", "#contacts"]),
  hrefs.join(", "),
);
...
check(
  ...
  (await quote.evaluate((el) => getComputedStyle(el).backgroundColor)) === "rgb(46, 46, 46)",
);

// --- Hero heading & green accent ---
const h1 = page.locator("h1");
check("h1 reads SENTINEL AI", /SENTINEL\s+AI/.test((await h1.textContent()) ?? ""));
const spanColor = await h1.locator("span").evaluate((el) => getComputedStyle(el).color);
const [r, g, b] = spanColor.match(/\d+/g).map(Number);
check("AI span is vivid green (hsl 119 99% 46%)", g > 200 && r < 30 && b < 30, spanColor);
...
for (const text of [
  ...
  check(`copy present: "${text.slice(0, 40)}…"`, (await page.getByText(text, { exact: false }).count()) >= 1);
}
...
check("section fills viewport (>= 900px)", layout.height >= 900, `${layout.height}px`);
check("section aligns content to bottom (items-end)", layout.alignItems === "flex-end");
check("section bg is hero-bg (hsl 0 0% 8% ≈ rgb 20)", layout.bg === "rgb(20, 20, 20)", layout.bg);
...
check("h1 fade-up completed (opacity 1)", h1Opacity === "1", `opacity=${h1Opacity}`);
