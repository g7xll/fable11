...
try {
  ...
  check("navbar fixed + transparent + z-50", /fixed/.test(headerClass) && /bg-transparent/.test(headerClass) && /z-50/.test(headerClass), headerClass);
  check("sunburst svg icon in navbar", (await header.locator("svg circle").count()) === 1 && (await header.locator("svg line").count()) === 12);
  for (const label of ["Products", "Customer Stories", "Resources", "Pricing", "Book A Demo", "Get Started"]) {
  ...
  check("Get Started: white pill w/ black text", gsStyles.bg === "rgb(255, 255, 255)" && gsStyles.color === "rgb(0, 0, 0)", JSON.stringify(gsStyles));
  ...
  check("pre-headline uses Instrument Serif", preFont.includes("Instrument Serif"), preFont);
  ...
  check("headline uses Instrument Sans semibold", h1Styles.font.includes("Instrument Sans") && h1Styles.weight === "600", `${h1Styles.font} / ${h1Styles.weight}`);
  ...
  check("headline 136px at lg viewport", h1Styles.size === "136px", h1Styles.size);
  check("headline animation settled (opacity 1, no scale)", h1Styles.opacity === "1" && (h1Styles.transform === "none" || h1Styles.transform === "matrix(1, 0, 0, 1, 0, 0)"), `${h1Styles.opacity} / ${h1Styles.transform}`);
  ...
  check("subheadline settled at opacity 0.7", Math.abs(parseFloat(subOpacity) - 0.7) < 0.01, subOpacity);

  // ---- CTA buttons ----
  const primary = page.getByRole("button", { name: /Start Building Free/ });
  check("primary CTA present", (await primary.count()) === 1);
  const arrowCircle = await primary.locator("span").nth(1).evaluate((el) => {
    const s = getComputedStyle(el);
    return { bg: s.backgroundColor, w: s.width, h: s.height, radius: s.borderRadius };
  });
  check("primary CTA blue arrow circle 40px #3054ff", arrowCircle.bg === "rgb(48, 84, 255)" && arrowCircle.w === "40px" && arrowCircle.h === "40px", JSON.stringify(arrowCircle));
  check("secondary CTA present", (await page.getByRole("button", { name: /See Examples/ }).count()) === 1);
  check("lucide arrow icons rendered", (await page.locator("svg.lucide-arrow-right").count()) === 2);
  ...
  check("video muted/loop/playsInline", videoState.muted && videoState.loop && videoState.playsInline);
  check("video poster fallback set", videoState.poster.includes("images.unsplash.com/photo-1647356191320"));
  check("video object-cover @ 60% opacity", videoState.objectFit === "cover" && videoState.opacity === "0.6", `${videoState.objectFit} / ${videoState.opacity}`);
  check("HLS stream playing", !videoState.paused && videoState.readyState >= 2 && videoState.currentTime > 0, `readyState=${videoState.readyState} t=${videoState.currentTime.toFixed(2)}s`);

  check("black/60 overlay with backdrop blur", (await page.locator("[class*='bg-black/60'][class*='backdrop-blur']").count()) === 1);
  check("two mix-blend-screen gradient blobs", (await page.locator("[class*='mix-blend-screen']").count()) === 2);
  ...
  console.log("[INFO] screenshots saved to screenshots/desktop.png and screenshots/mobile.png");
} finally {
  await browser.close();
  preview.kill();
}
