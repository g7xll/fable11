(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    let open = false;
    const ICON_OPEN = '<svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 9h16.5m-16.5 6.75h16.5"/></svg>';
    const ICON_CLOSE = '<svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>';
    const setMenu = (v) => {
      open = v;
    ...
    document.querySelectorAll('.acc-trigger').forEach(t => {
      t.addEventListener('click', () => {
        ...
        if (isOpen) {
          item.querySelectorAll('.bar i').forEach(bar => bar.style.width = bar.dataset.w + '%');
        }
      });
    });
    ...
    if (tw) {
      ...
      const run = () => {
        let li = 0;
        const typeLine = () => {
          if (li >= lines.length) { tw.appendChild(cursor); return; }
          const { t, c } = lines[li];
      ...
      const twObs = new IntersectionObserver((entries) => {
        entries.forEach(e => { if (e.isIntersecting) { run(); twObs.disconnect(); } });
      }, { threshold: 0.3 });
      twObs.observe(tw);
    }
  });
