...
(function () {
  ...
  if (typed) {
    var roles = ["Design Engineer", "Systems Thinker", "Open-Source Maintainer", "Full-Stack Builder"];
    if (reduce) {
  ...
  if (ringEl) {
    ...
    if ("IntersectionObserver" in window && ringPanel) {
      var ro = new IntersectionObserver(function (es) {
        es.forEach(function (e) { if (e.isIntersecting) { fillRing(); ro.disconnect(); } });
      }, { threshold: 0.4 });
  ...
  if (stats) {
    ...
    if ("IntersectionObserver" in window) {
      var so = new IntersectionObserver(function (es) {
        es.forEach(function (e) { if (e.isIntersecting) { runCounts(); so.disconnect(); } });
      }, { threshold: 0.5 });
      so.observe(stats);
    } else { runCounts(); }
  }
  ...
  /* ---- Scroll-spy nav ---- */
  var navLinks = Array.prototype.slice.call(document.querySelectorAll(".nav-pill a"));
  var sections = ["work", "about", "stack", "contact"].map(function (id) { return document.getElementById(id); });
  if ("IntersectionObserver" in window) {
  ...
  if (open) open.addEventListener("click", function () { sheet.classList.add("open"); });
  if (close) close.addEventListener("click", function () { sheet.classList.remove("open"); });
  ...
  if (form) {
    function fail(id) { document.getElementById(id).classList.add("err"); }
    function clearErr(id) { document.getElementById(id).classList.remove("err"); }
    ["name", "email", "message"].forEach(function (id) {
      var input = document.getElementById(id);
      input.addEventListener("input", function () { clearErr("f-" + id); });
    });
    form.addEventListener("submit", function (e) {
      ...
      if (!name) { fail("f-name"); ok = false; } else clearErr("f-name");
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { fail("f-email"); ok = false; } else clearErr("f-email");
      if (msg.length < 4) { fail("f-message"); ok = false; } else clearErr("f-message");
      if (!ok) return;
