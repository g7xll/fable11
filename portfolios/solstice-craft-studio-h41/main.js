...
(function () {
  ...
  if (toggle && fsMenu) {
    const close = () => { document.body.classList.remove('nav-open'); fsMenu.classList.remove('open'); };
    const open = () => { document.body.classList.add('nav-open'); fsMenu.classList.add('open'); };
    toggle.addEventListener('click', () =>
      document.body.classList.contains('nav-open') ? close() : open());
    fsMenu.querySelectorAll('a').forEach(a => a.addEventListener('click', close));
  ...
  if (flow && fill && !reduced && 'IntersectionObserver' in window) {
    const io = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) { setTimeout(() => { fill.style.width = '100%'; }, 250); io.unobserve(e.target); }
      });
    }, { threshold: 0.3 });
    io.observe(flow);
  ...
  if (proj) {
    Array.from(proj.children).forEach(c => proj.appendChild(c.cloneNode(true)));
  }
