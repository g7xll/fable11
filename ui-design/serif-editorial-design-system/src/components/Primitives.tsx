...
export function SectionLabel({
	...
	const labelColor =
		tone === "onDark" ? "text-accent-secondary" : "text-accent";
	...
	if (align === "left") {
		return (
			<div className={cn("flex items-center gap-4", className)}>
				...
				<span
					className={cn("rule w-16 origin-left animate-rule-grow", ruleColor)}
					aria-hidden
				/>
			</div>
		);
	...
	return (
		<div className={cn("flex items-center gap-4", className)}>
			<span
				className={cn("rule flex-1 origin-right animate-rule-grow", ruleColor)}
				aria-hidden
			/>
			<span className={cn("small-caps whitespace-nowrap", labelColor)}>
				{children}
			</span>
			<span
				className={cn("rule flex-1 origin-left animate-rule-grow", ruleColor)}
				aria-hidden
			/>
		</div>
	);
