...
export default function AiBudgetTrackerHeroVelara({
	className,
}: {
	className?: string;
}) {
	...
	return (
		<>
			...
			<section
				className={
					"relative mb-12 overflow-hidden min-h-[1100px] pt-[10px] px-6 md:px-12 bg-black " +
					(className || "")
				}
			>
				{/* VIDEO BACKGROUND */}
						...
						<main className="flex-1 p-10 overflow-hidden bg-gradient-to-br from-transparent to-[#A855F7]/[0.02]">
							...
							<div className="flex items-center justify-between gap-8 mb-12">
								<div className="flex items-center gap-4 bg-white/[0.03] border border-white/5 rounded-2xl px-6 py-4 flex-1 max-w-[500px] shadow-inner">
									...
									/>
									<div className="text-[10px] font-bold text-white/20 border border-white/10 px-2 py-1 rounded bg-white/5">
										cmd K
									</div>
								</div>
							...
							<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
								...
								<div className="lg:col-span-2 p-10 rounded-[40px] bg-gradient-to-br from-white/[0.08] to-white/[0.01] border border-white/10 relative overflow-hidden flex flex-col justify-between min-h-[300px] shadow-2xl">
									...
									<div className="flex justify-between items-start relative z-10">
										<div>
											<h4 className="text-white/40 text-[11px] font-bold uppercase tracking-[0.25em] mb-4">
										...
										<div className="flex gap-2.5">
											{["1D", "1W", "1M", "3M", "1Y"].map((t: string) => (
												<button
													key={t}
													className={
														"w-11 h-11 rounded-xl text-[10px] font-bold border transition-all " +
														(t === "1M"
									...
									<div className="h-32 flex items-end gap-2.5 relative z-10 px-2 mt-8">
										{chartData.map((val: number, i: number) => (
											<motion.div
												...
												className={
													"flex-1 rounded-t-xl transition-all duration-700 relative " +
													(i === 6
