...
export default function Footer02Velara({ className }: { className?: string }) {
	...
	return (
		<>
			<footer
				className={
					"bg-black text-[#AAA] font-sans overflow-hidden select-none antialiased " +
					(className || "")
				}
			>
						...
						return (
							...
							>
								<div
									className={
										"bg-[#1A1A1A] border " +
										(isActive ? "border-[#A855F7]" : "border-[#333]") +
										" rounded-md px-3 py-1.5 whitespace-nowrap shadow-lg shadow-purple-500/20 transition-all duration-700"
									}
								>
									<span
										className={
											(isActive ? "text-white" : "text-[#888]") +
											" text-[13px] font-medium tracking-tight transition-colors duration-700"
										}
									>
										{badge.name}
									</span>
					...
					<div className="max-w-[1440px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-12">
						<div>
							...
							<ul className="space-y-3">
								{["Home", "About Us", "Projects", "Pricing"].map(
									(link: string) => (
										<li
											key={link}
											className="text-[#AAA] text-[14px] hover:text-white transition-colors cursor-pointer leading-loose"
										>
											{link}
										</li>
									),
						...
						<div>
							...
							<ul className="space-y-3">
								{["Blogs", "License", "Contact Us", "Case Study"].map(
									(link: string) => (
										<li
											key={link}
											className="text-[#AAA] text-[14px] hover:text-white transition-colors cursor-pointer leading-loose"
