  ...
  return (
    <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl bg-[#06100C] ring-1 ring-inset ring-black/40">
      ...
      >
        ...
        <motion.g animate={{ opacity: show("mask") }} transition={{ duration: 0.4 }}>
          <rect x="16" y="16" width="448" height="328" rx="14" fill="url(#bc-mask)" />
          <rect
            x="16"
            y="16"
            width="448"
            ...
            strokeWidth="1.5"
          />
          <rect x="16" y="16" width="448" height="328" rx="14" fill="url(#bc-glow)" />
          {/* mounting holes */}
          {[
            [42, 42],
            [438, 42],
            ...
            <circle key={i} cx={x} cy={y} r="7" fill="#06100C" stroke="#34D17F" strokeOpacity="0.4" strokeWidth="1.5" />
          ))}
        </motion.g>

        {/* COPPER — traces, pours and pads */}
        <motion.g animate={{ opacity: show("copper") }} transition={{ duration: 0.4 }}>
          {/* ground pour hatch on the right */}
          <g stroke="#E8A24B" strokeOpacity="0.16" strokeWidth="1">
            {Array.from({ length: 14 }).map((_, i) => (
              <line key={i} x1={300 + i * 12} y1="40" x2={300 + i * 12} y2="320" />
            ))}
          </g>
          {/* routed traces (45°) */}
          <g
          ...
          <g>
            <rect x="196" y="146" width="68" height="68" rx="4" fill="#0A1A12" stroke="#E8A24B" strokeWidth="2" strokeOpacity="0.7" />
          ...
          {[
            ...
            <rect key={i} x={x - 6} y={y - 5} width="12" height="10" rx="1.5" fill="#F5B968" />
          ))}
        </motion.g>

        {/* VIAS — plated through-holes, visible on copper + assembled */}
        <motion.g animate={{ opacity: layer === "silk" || layer === "mask" ? 0.06 : 1 }} transition={{ duration: 0.4 }}>
          {[
            [250, 126],
            [250, 180],
            [300, 166],
            ...
            <circle key={i} cx={x} cy={y} r="4.5" fill="#06100C" stroke="#1FB66E" strokeWidth="2" />
          ))}
        </motion.g>
        ...
        <motion.g
          ...
          fontWeight="600"
        >
          <text x="200" y="134">U1</text>
          <text x="58" y="80">R1</text>
          <text x="58" y="120">R2</text>
          <text x="58" y="170">C1</text>
          <text x="58" y="220">C2</text>
          <text x="402" y="138">D1</text>
          <text x="434" y="222">J1</text>
          {/* board name + rev */}
          <text x="34" y="332" fillOpacity="0.6" fontSize="10">FOUNDRY · rev_C</text>
          {/* part outline boxes */}
