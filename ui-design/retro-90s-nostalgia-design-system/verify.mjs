...
try {
	...
	// Title + landmarks
	check(
		"document title is WEBMASTER 95",
		(await page.title()).includes("WEBMASTER 95"),
	);
	check(
		"has <header>(taskbar role)/<nav>/<main>/<footer>",
		(await page.locator("nav, main, footer").count()) >= 3,
	);
	...
	check(
		"--background token is #c0c0c0",
		tokens.background.toLowerCase() === "#c0c0c0",
		tokens.background,
	);
	check(
		"--accent token is #0000ff",
		tokens.accent.toLowerCase() === "#0000ff",
		tokens.accent,
	);
	check(
		"--title-bar token is #000080",
		tokens.titleBar.toLowerCase() === "#000080",
		tokens.titleBar,
	);
	...
	const marqueeCount = await page
		.locator(".marquee .marquee__track span")
		.count();
	check(
		"[1] marquee element with multiple colored spans",
		marqueeCount >= 6,
		`${marqueeCount} spans`,
	);
	const marqueeAnimated = await page
		.locator(".marquee__track")
		.first()
		.evaluate((el) => {
	...
	const rainbow = await page
		.locator(".text-rainbow")
		.first()
		.evaluate((el) => {
	...
	check(
		"[2] rainbow text uses rainbow keyframes",
	...
	check(
		"[3] button top/left edge is white",
		btnEdges.top === "rgb(255, 255, 255)",
		btnEdges.top,
	);
	check(
		"[3] button right/bottom edge is gray",
		btnEdges.right === "rgb(128, 128, 128)",
		btnEdges.right,
	);
	check("[3] button has inset box-shadow bevel", /inset/.test(btnEdges.shadow));
	check(
		"[3] button border-radius is 0",
		btnEdges.radius === "0px",
		btnEdges.radius,
	);

	// 4. card with Windows 95 title bar gradient
	const barGradient = await page
		.locator(".window__bar")
		.first()
		.evaluate((el) => getComputedStyle(el).backgroundImage);
	check(
		"[4] title bar has navy→blue linear-gradient",
		/linear-gradient/.test(barGradient) && /0, 0, 128/.test(barGradient),
		barGradient.slice(0, 60),
	);
	...
	check(
		"[5] tiled crosshatch background on body",
		(bodyBg.match(/linear-gradient/g) || []).length >= 4,
	);
	...
	check(
		"[6] link is blue",
		linkStyle.color === "rgb(0, 0, 255)",
		linkStyle.color,
	);
	check(
		"[6] link is underlined",
		linkStyle.deco.includes("underline"),
		linkStyle.deco,
	);
	await link.hover();
	const hoverColor = await link.evaluate((el) => getComputedStyle(el).color);
	check(
		"[6] link turns red on hover",
	...
	const rowColors = await page.evaluate(() => {
		const rows = Array.from(document.querySelectorAll(".specimen__row")).slice(
			0,
			2,
		);
	...
	check(
		"[7] alternating rows differ (odd vs even)",
	...
	check(
		"[8] multiple groove HR dividers present",
		hrCount >= 4,
		`${hrCount} dividers`,
	);
	check(
		"[8] groove HR is 4px with gradient",
		hrStyle.h === "4px" && /linear-gradient/.test(hrStyle.bg),
		hrStyle.h,
	);
	...
	const counterDigit = await page
		.locator(".counter__digit")
		.first()
		.evaluate((el) => {
			const cs = getComputedStyle(el);
	...
	check(
		"[9] hit counter digits are green",
		counterDigit.color === "rgb(0, 255, 0)",
		counterDigit.color,
	);
	check(
		"[9] hit counter digits are monospace (VT323)",
		/VT323/.test(counterDigit.family),
		counterDigit.family,
	);
	...
	check(
		"[9] hit counter counted up to 1048576",
		counterText === "1048576",
		counterText,
	);
	const stat100 = await page
		.locator('.statline__num[data-count="100"]')
		.textContent();
	...
	check(
		"[10] NEW!/HOT! badge has pulse-glow animation",
		/pulse-glow/.test(pulse),
		pulse,
	);
	check(
		"[10] has a NEW! and a HOT! badge",
		(await page.getByText("NEW!", { exact: true }).count()) >= 1 &&
			(await page.getByText("HOT!", { exact: true }).count()) >= 1,
	);

	// 11. construction stripe background
	const stripe = await page
		.locator(".bg-construction")
		.first()
		.evaluate((el) => getComputedStyle(el).backgroundImage);
	check(
		"[11] construction stripes (repeating 45deg yellow/black)",
		/repeating-linear-gradient/.test(stripe) && /255, 255, 0/.test(stripe),
	);
	...
	for (let i = 0; i < 60 && !onButton; i++) {
		...
		onButton = await page.evaluate(() => {
			...
			return (
				!!e &&
				e.tagName === "BUTTON" &&
				/\bbtn\b/.test(e.className) &&
				e.textContent.trim() === "Default"
			);
		});
	}
	...
	check(
		"[12] keyboard-focused button shows dotted 2px outline",
		onButton && focusStyle.style === "dotted" && focusStyle.width === "2px",
		`${focusStyle.style} ${focusStyle.width} reached=${onButton}`,
	);
	...
	check(
		"[13] pressed button translates (matrix not 'none')",
		pressed.transform !== "none" && pressed.transform.includes("1, 1"),
		pressed.transform,
	);
	check(
		"[13] pressed button edge flips to gray (inset)",
		pressed.top === "rgb(128, 128, 128)",
		pressed.top,
	);

	// 14. icons have 2px stroke width
	const strokeW = await page
		.locator("svg symbol")
		.first()
		.evaluate((el) => el.getAttribute("stroke-width"));
	check(
		"[14] icon symbols declare stroke-width=2",
		strokeW === "2",
		String(strokeW),
	);
	const renderedStroke = await page
		.locator(".feature__icon svg use")
		.first()
		.evaluate((el) => {
	...
	check(
		"[14] icon stroke renders thick (2px)",
		renderedStroke === "2px" || renderedStroke === "2",
		renderedStroke,
	);
	...
	check(
		"[15] ZERO border-radius on every element",
		radii.length === 0,
		radii.slice(0, 4).join(" | "),
	);
	...
	check(
		"clicking a swatch updates the preview to its hex",
		preview.trim() === "#FF0000",
		preview,
	);
	check(
		"clicked swatch gets aria-pressed=true",
		(await swatches.nth(0).getAttribute("aria-pressed")) === "true",
	);
	...
	await tab2.click();
	check(
		"clicking a tab selects it (aria-selected)",
		(await tab2.getAttribute("aria-selected")) === "true",
	);
	check(
		"selected tab reveals its panel",
		!(await page.locator("#panel-2").isHidden()),
	);
	...
	check(
		"guestbook rejects empty submit",
		(await page.locator("#gbNote").getAttribute("data-state")) === "err",
	);
	...
	check(
		"guestbook adds a new entry on valid submit",
		afterCount === beforeCount + 1,
		`${beforeCount}->${afterCount}`,
	);
	check(
		"new guestbook entry lands on top",
		(
			await page.locator("#guestbookList .gb-entry").first().textContent()
		).includes("TestSurfer_77"),
	);
	...
	check(
		"CTA rejects invalid email",
		(await page.locator("#ctaNote").getAttribute("data-state")) === "err",
	);
	await page.locator("#ctaEmail").fill("surfer@geocities.com");
	await page.locator("#ctaForm button[type=submit]").click();
	await page.waitForTimeout(120);
	check(
		"CTA accepts valid email",
		(await page.locator("#ctaNote").getAttribute("data-state")) === "ok",
	);
	...
	check(
		...
		!(await page.locator("#startMenu").isHidden()),
	);
	check(
		"start button aria-expanded=true",
		(await page.locator("#startBtn").getAttribute("aria-expanded")) === "true",
	);
	...
	const linksDisplay = await mpage
		.locator("#navlinks")
		.evaluate((el) => getComputedStyle(el).display);
	check(
		"nav links hidden on mobile (collapsed)",
		linksDisplay === "none",
		linksDisplay,
	);
	...
	check(
		"bevel edges kept on mobile (white top)",
		mBtn.top === "rgb(255, 255, 255)",
		mBtn.top,
	);

	// marquee still scrolls on mobile
	const mMarquee = await mpage
		.locator(".marquee__track")
		.first()
		.evaluate((el) => getComputedStyle(el).animationName);
	check(
		"marquee continues to scroll on mobile",
		mMarquee === "marquee",
		mMarquee,
	);
	...
	check(
		"body gets .nav-open after burger tap",
		await mpage.evaluate(() => document.body.classList.contains("nav-open")),
	);
	check(
		"nav links visible after tap",
		await mpage.locator("#navlinks").isVisible(),
	);
	...
	check(
		"hero headline still ≥ 40px on mobile",
		mh1.size >= 40,
		`${Math.round(mh1.size)}px`,
	);
