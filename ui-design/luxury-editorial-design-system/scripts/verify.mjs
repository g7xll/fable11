...
grain > 0
	? ok("Paper-grain overlay present")
	: bad("Paper-grain overlay missing");

const gridLines = await page
	.locator(".w-px.bg-foreground\\/\\[0\\.12\\]")
	.count();
...
const gridFallback = await page.evaluate(() => {
	const lines = [...document.querySelectorAll("span")].filter((s) => {
		...
		return (
			...
			parseFloat(cs.width) <= 1.5 &&
			parseFloat(cs.height) > 400
		);
	});
	return lines.length;
});
...
afterOpen === "true" && panelVisible
	...
	: bad(
			`After click: aria-expanded=${afterOpen}, panelVisible=${panelVisible}`,
		);
...
const primaryBtn = page
	.locator("button:has-text('View the collection')")
	.first();
...
const restTransform = await overlay
	.first()
	.evaluate((el) => getComputedStyle(el).transform);
await primaryBtn.hover();
await page.waitForTimeout(700);
const hoverTransform = await overlay
	.first()
	.evaluate((el) => getComputedStyle(el).transform);
restTransform !== hoverTransform
	? ok(
			`Overlay transform changes on hover (rest="${restTransform}" → hover="${hoverTransform}")`,
		)
	: bad(`Overlay transform did not change on hover (${restTransform})`);

const overlayBg = await overlay
	.first()
