...
// ── Title & shell ──────────────────────────────────────────────────────────
check("page title", (await page.title()).includes("Flatline"), await page.title());
check(
...
check("--color-brand = #3b82f6", tokens.brand.toLowerCase() === "#3b82f6", tokens.brand);
check("--color-grass = #10b981", tokens.grass.toLowerCase() === "#10b981", tokens.grass);
check("--color-sun = #f59e0b", tokens.sun.toLowerCase() === "#f59e0b", tokens.sun);
check("--color-ink = #111827", tokens.ink.toLowerCase() === "#111827", tokens.ink);
...
for (const t of [
	...
	check(`hero text "${t}"`, (await page.getByText(t, { exact: false }).count()) >= 1);
}
...
check("how-it-works is dark (Gray 900)", sectionColors.how === "rgb(17, 24, 39)", sectionColors.how);
check("footer is dark (Gray 900)", sectionColors.footer === "rgb(17, 24, 39)", sectionColors.footer);
...
check("hero CTA radius is rounded-md (6px)", btnStyle.radius === "6px", btnStyle.radius);
...
const outlineBorder = await page.evaluate(() => {
	const el = Array.from(document.querySelectorAll("a,button")).find(
		(e) => e.className.includes("btn-outline"),
	);
	return el ? getComputedStyle(el).borderTopWidth : "";
});
check("outline button uses thick border-4", outlineBorder === "4px", outlineBorder);
...
const cardStyle = await featureCard.evaluate((el) => {
	const s = getComputedStyle(el);
	return { radius: s.borderTopLeftRadius, shadow: s.boxShadow, bg: s.backgroundColor };
});
check("feature card radius rounded-lg (8px)", cardStyle.radius === "8px", cardStyle.radius);
check("feature card has no shadow", cardStyle.shadow === "none", cardStyle.shadow);
check(
...
const restBorder = await field.evaluate((el) => getComputedStyle(el).borderColor);
await field.focus();
await page.waitForTimeout(250);
const focusState = await field.evaluate((el) => {
	const s = getComputedStyle(el);
	return { border: s.borderColor, width: s.borderTopWidth, bg: s.backgroundColor };
});
check("input border transparent at rest", restBorder.includes("rgba(0, 0, 0, 0)") || restBorder === "rgba(0, 0, 0, 0)", restBorder);
check("input focus = solid blue 2px border", focusState.border === "rgb(59, 130, 246)" && focusState.width === "2px", `${focusState.border} ${focusState.width}`);
check("input focus bg turns white", focusState.bg === "rgb(255, 255, 255)", focusState.bg);
...
check("focus-visible adds a solid ring", linkRing !== "none", linkRing.slice(0, 40));
...
const restsLarger =
	!!scaleInfo &&
	((scaleInfo.scale && scaleInfo.scale !== "none" && parseFloat(scaleInfo.scale) > 1) ||
...
check("FAQ items use thick border-2 (2px)", faqItemBorder === "2px", faqItemBorder);

const faqBtn = page.getByRole("button", { name: /What stack does it target/ });
check(
	"FAQ answer hidden before click",
	(await page.getByText("Flatline is built on React", { exact: false }).count()) === 0 ||
		!(await page.getByText("Flatline is built on React", { exact: false }).first().isVisible()),
);
await faqBtn.click();
...
check(
	...
	(await page.getByText("You're in. Watch your inbox.", { exact: false }).count()) === 1,
);

// ── Stat count-ups reached their targets ───────────────────────────────────
check("stat 100% rendered", (await page.getByText("100%", { exact: false }).count()) >= 1);
check("stat 4.9/5 rendered", (await page.getByText("4.9/5", { exact: false }).count()) >= 1);
