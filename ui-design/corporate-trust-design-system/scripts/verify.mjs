...
// ── Title & brand ────────────────────────────────────────────────────────
check(
	"page title mentions Northwind",
	(await page.title()).includes("Northwind"),
	await page.title(),
);
check(
	"brand wordmark present",
	(await page.getByText("Northwind").count()) >= 1,
);
...
check(
	"--color-slate-200 (border) = #e2e8f0",
	tokens.slate200 === "#e2e8f0",
	tokens.slate200,
);
check(
	"--color-emerald-500 = #10b981",
	tokens.emerald500 === "#10b981",
	tokens.emerald500,
);
check(
	"--color-canvas (bg) = #f8fafc",
	tokens.canvas === "#f8fafc",
	tokens.canvas,
);
...
check(
	"card shadow is color-tinted (indigo), not gray",
	tinted,
	cardShadow.slice(0, 60),
);
...
check(
	"primary button has a colored shadow",
	/rgba?\(\s*79,\s*70,\s*229/.test(btnShadow),
	btnShadow.slice(0, 60),
);
...
check(
	"primary button is pill (rounded-full)",
	parseFloat(btnRadius) >= 100,
	btnRadius,
);
...
check(
	"feature card radius is rounded-xl (12px)",
	cardBox && parseFloat(cardBox.radius) === 12,
	cardBox?.radius,
);
check(
	"feature card has a 1px hairline border",
	cardBox && parseFloat(cardBox.border) === 1,
	cardBox?.border,
);
...
check(
	"hero card has a perspective scene",
	iso.perspective !== "none" && parseFloat(iso.perspective) > 500,
	iso.perspective,
);
check(
	"hero card carries a 3D isometric transform",
	iso.is3d,
	"matrix3d expected",
);
...
const stepGlow = await page.evaluate(() => {
	const badge = Array.from(
		document.querySelectorAll("#how .rounded-full"),
...
check(
	...
	!!stepGlow &&
		stepGlow.gradient &&
		/rgba?\(\s*79,\s*70,\s*229/.test(stepGlow.shadow),
...
check(
	"feature card lifts on hover (transform changes)",
	before.t !== after.t,
	`${before.t} -> ${after.t}`,
);
...
check(
	...
	(await page.getByText("Most popular").count()) === 1,
);
const popularScale = await page.evaluate(() => {
	...
	void card;
	return {
		scale: getComputedStyle(badge.parentElement).scale,
		transform: "none",
	};
});
const restsLarger =
...
const restBorder = await footerInput.evaluate(
	(el) => getComputedStyle(el).borderTopWidth,
);
await footerInput.focus();
await page.waitForTimeout(250);
const focusBorder = await footerInput.evaluate(
	(el) => getComputedStyle(el).borderColor,
);
check(
	"input has a hairline border at rest",
	parseFloat(restBorder) === 1,
	restBorder,
);
check(
	"input border turns indigo-400 on focus",
	focusBorder === "rgb(129, 140, 248)",
	focusBorder,
);
...
for (const target of ["99.99%", "4.9/5", "180+"]) {
	check(
		`stat "${target}" reached`,
		(await page.getByText(target, { exact: false }).count()) >= 1,
	);
}
...
check(
	"no images missing alt/aria-hidden",
	blankAlt === 0,
	`${blankAlt} offending`,
);
