				...
				<div className="flex flex-wrap gap-4">
					{tags.map((tag: string, i: number) => (
						<div
							key={i}
							className={
								"px-[10px] py-[3px] rounded-[6px] text-center font-sans text-[14px] font-medium leading-[20px] " +
								getTagStyles(tag)
							}
						>
							{tag}
...
export default function Blog01Finsyc({ className }: { className?: string }) {
	return (
		<>
			...
			<section
				className={
					"w-full bg-[#FFFFFF] py-16 md:py-[100px] flex justify-center " +
					(className || "")
				}
			>
					...
					<div className="w-full max-w-[1248px] mx-auto">
						...
						<div className="flex flex-col items-center text-center mb-12 md:mb-[80px]">
							...
							>
								...
								<span className="italic text-[rgba(4,39,24,0.40)]">manage</span>{" "}
								money smarter
							</motion.h2>
							...
							>
								Learn how to save more, spend wisely, and make better financial
								decisions with expert tips.
							</motion.p>
						</div>
