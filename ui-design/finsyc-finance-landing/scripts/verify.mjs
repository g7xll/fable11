...
console.log(`image check: ${imgStats.total - imgStats.broken.length}/${imgStats.total} images decoded`);
if (imgStats.broken.length) fail("broken images:\n  " + imgStats.broken.join("\n  "));
...
if (docHeight < 4000) fail("page suspiciously short (" + docHeight + "px) — sections may not have rendered");

// 6) No JS page errors / failed local assets.
if (pageErrors.length) fail("page errors:\n  " + pageErrors.join("\n  "));
else console.log("page errors: none");
if (failedAssets.length) fail("failed local assets:\n  " + failedAssets.join("\n  "));
