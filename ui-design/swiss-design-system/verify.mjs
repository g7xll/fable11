...
try {
	...
	check(
		"--accent token is #ff3000",
		tokens.accent.toLowerCase() === "#ff3000",
		tokens.accent,
	);
	check(
		"--foreground token is #000000",
		tokens.fg.toLowerCase() === "#000000",
		tokens.fg,
	);
	...
	check(
		"hero headline ≥ 80px at desktop",
		h1size >= 80,
		`${Math.round(h1size)}px`,
	);
	const h1transform = await h1.evaluate(
	...
	const interLoaded = await page.evaluate(async () => {
		...
		return (
			document.fonts.check('900 48px "Inter"') &&
	...
	// Patterns present
	check(
		"grid pattern element present",
		(await page.locator(".swiss-grid-pattern").count()) >= 1,
	);
	check(
		"dot pattern element present",
		(await page.locator(".swiss-dots").count()) >= 1,
	);
	check(
		"diagonal pattern element present",
		(await page.locator(".swiss-diagonal").count()) >= 1,
	);
	...
	check(
		"≥ 6 numbered eyebrows",
		(await page.locator(".eyebrow__num").count()) >= 6,
	);
	...
	const statText = await page
		.locator(".stat__num[data-count='21']")
		.textContent();
	...
	check(
		"FAQ closed initially (aria-expanded=false)",
		(await faqBtn.getAttribute("aria-expanded")) === "false",
	);
	await faqBtn.click();
	await page.waitForTimeout(300);
	check(
		"FAQ opens (aria-expanded=true)",
	...
	const answerH = await page
		.locator(".faq__a")
		.first()
		.evaluate((el) => el.getBoundingClientRect().height);
	check(
		"FAQ answer panel expands (height > 0)",
		answerH > 0,
		`${Math.round(answerH)}px`,
	);
	...
	check(
		"toggle group sets aria-pressed on click",
		(await choices.nth(1).getAttribute("aria-pressed")) === "true",
	);
	check(
		"toggle group clears previous selection",
		(await choices.nth(0).getAttribute("aria-pressed")) === "false",
	);
	...
	check(
		...
		(await page.locator("#cta-note").textContent()).includes("valid"),
	);
	...
	check(
		"CTA accepts valid email (success state)",
		okState === "ok",
		String(okState),
	);
	...
	check(
		"has skip link to content",
		(await page.locator("a.sr-only[href='#system']").count()) === 1,
	);
	const decorativeHidden = await page
		.locator(".hero__panel[aria-hidden='true']")
		.count();
	...
	mpage.on(
		"console",
		(m) =>
			m.type() === "error" && !isNoiseM(m.text()) && mErrors.push(m.text()),
	);
	...
	check(
		"body gets .menu-open after burger tap",
		await mpage.evaluate(() => document.body.classList.contains("menu-open")),
	);
	const drawerVisible = await mpage
		.locator("#drawer")
		.evaluate((el) => getComputedStyle(el).visibility);
	check(
		"mobile drawer becomes visible",
		drawerVisible === "visible",
		drawerVisible,
	);
	check(
		"burger aria-expanded=true when open",
	...
	check(
		"hero headline still ≥ 48px on mobile",
		mh1 >= 48,
		`${Math.round(mh1)}px`,
	);
