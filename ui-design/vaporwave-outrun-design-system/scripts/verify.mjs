...
// ── Title & shell ──────────────────────────────────────────────────────────
check(
	"page title mentions NEONWAVE",
	(await page.title()).includes("NEONWAVE"),
	await page.title(),
);
check(
...
check(
	"Share Tech Mono on body",
	bodyFont.includes("Share Tech Mono"),
	bodyFont,
);
...
check(
	...
	tokens.magenta === "rgb(255, 0, 255)",
	tokens.magenta,
);
check(
	"--color-cyan = #00ffff",
	tokens.cyan === "rgb(0, 255, 255)",
	tokens.cyan,
);
check(
	"--color-orange = #ff9900",
	tokens.orange === "rgb(255, 153, 0)",
	tokens.orange,
);
check(
	"--color-chrome = #e0e0e0",
	tokens.chrome === "rgb(224, 224, 224)",
	tokens.chrome,
);
...
const layers = await page.evaluate(() => {
	...
	return {
		...
		sun: {
			z: z(sun),
			pos: pos(sun),
			blur: sun ? getComputedStyle(sun).filter : "",
		},
		scan: { z: z(scan), pos: pos(scan) },
	};
});
check(
	"grid floor present, fixed, z-0",
	layers.grid.pos === "fixed" && layers.grid.z === "0",
	JSON.stringify(layers.grid),
);
check(
	"blurred sun present + fixed",
	layers.sun.pos === "fixed",
	JSON.stringify(layers.sun),
);
check(
	"sun has heavy blur",
	layers.sun.blur.includes("blur(100px)") || layers.sun.blur.includes("blur"),
	layers.sun.blur,
);
check(
	"scanlines present, fixed, z-50",
	layers.scan.pos === "fixed" && layers.scan.z === "50",
	JSON.stringify(layers.scan),
);
...
const sharpReport = await page.evaluate(() => {
	...
	for (const el of Array.from(
		document.querySelectorAll("button.btn, .card, .window, .field, .eyebrow"),
	)) {
		const r = getComputedStyle(el).borderTopLeftRadius;
		if (!allow.has(r))
			offenders.push(el.className.toString().slice(0, 30) + ":" + r);
	}
	return offenders;
});
check(
	"buttons/cards/windows are rounded-none",
	sharpReport.length === 0,
	sharpReport.slice(0, 4).join(" | "),
);
...
check(
	"primary button is skewed at rest",
	restState.transform !== "none",
	restState.transform,
);
check(
	"primary button has cyan border",
	restState.border === "rgb(0, 255, 255)",
	restState.border,
);
...
check(
	"primary button un-skews on hover",
	hoverState.transform === "none" ||
		hoverState.transform === "matrix(1, 0, 0, 1, 0, 0)",
	hoverState.transform,
);
check(
	"primary button fills cyan on hover",
	hoverState.bg === "rgb(0, 255, 255)",
	hoverState.bg,
);
check(
	"primary button glows on hover",
	hoverState.shadow !== "none" && hoverState.shadow.includes("255"),
	hoverState.shadow.slice(0, 40),
);
...
check(
	"card title is cyan",
	!!cardTitle && cardTitle.color === "rgb(0, 255, 255)",
	cardTitle?.color,
);
check(
	"card title has glow filter",
	!!cardTitle && cardTitle.filter.includes("drop-shadow"),
	cardTitle?.filter?.slice(0, 40),
);
check(
	"card title uses Orbitron",
	!!cardTitle && cardTitle.font.includes("Orbitron"),
	cardTitle?.font,
);
...
check(
	"diamond icon container is rotated",
...
check(
	"6 feature modules",
	(await page.locator("#features .card").count()) === 6,
);
...
check(
	"feature card has cyan top laser border",
	!!featBorder &&
		featBorder.top === "rgb(0, 255, 255)" &&
		featBorder.topW === "2px",
	JSON.stringify(featBorder),
);
...
check(
	"hero gradient text uses sunset gradient",
	!!sunset && sunset.img.includes("gradient") && sunset.img.includes("255"),
	sunset?.img?.slice(0, 60),
);

// ── Benefits file-explorer: clicking a file switches the preview ────────────
check(
	"benefits shows default file title",
	(await page.getByText("One source of truth", { exact: false }).count()) >= 1,
);
...
check(
	...
	await page
		.getByText("Composable by design", { exact: false })
		.first()
		.isVisible(),
);
...
check(
	"popular tier rests larger (scale > 1)",
	restsLarger,
...
check(
	...
	await page
		.getByText("React + TypeScript primitives", { exact: false })
		.first()
		.isVisible(),
);
...
const fieldRest = await field.evaluate(
...
check(
	"input underline magenta at rest",
	fieldRest === "rgb(255, 0, 255)",
	fieldRest,
);
check(
	"input underline turns cyan on focus",
	fieldFocus.border === "rgb(0, 255, 255)",
	fieldFocus.border,
);
check(
	"input glows on focus",
	fieldFocus.shadow !== "none" && fieldFocus.shadow.includes("255"),
	fieldFocus.shadow.slice(0, 40),
);
...
check(
	...
	(await page.getByText("transmission received", { exact: false }).count()) >=
		1,
);
...
await page.evaluate(() => {
	...
	if (cell)
		window.scrollTo(0, cell.getBoundingClientRect().top + window.scrollY - 300);
});
...
check(
	"stat counts up to 100%",
	statTexts.includes("100%"),
	statTexts.join(" "),
);
check(
	"stat counts up to 2,088",
	statTexts.includes("2,088"),
	statTexts.join(" "),
);
check(
	"stat counts up to 60fps",
	statTexts.includes("60fps"),
	statTexts.join(" "),
);
check(
	"stat counts up to 4.9/5",
	statTexts.includes("4.9/5"),
	statTexts.join(" "),
);
