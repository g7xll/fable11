...
/* ---------------- Desktop (1440x900) ---------------- */
{
  ...
  check('--font-heading var', vars.heading.includes('Helvetica Now Display Bold'), vars.heading);
  ...
  /* Navbar */
  check('logo svg in header', (await page.locator('header svg[viewBox="0 0 256 256"]').count()) >= 1);
  for (const item of ['Vault', 'Plans', 'Install', 'News', 'Help']) {
    check(`desktop nav link "${item}" visible`, await page.locator(`nav >> text=${item}`).first().isVisible());
  }
  ...
  check('"Start For Free" white text', sb.color === 'rgb(255, 255, 255)', sb.color);
  ...
  check('heading font Helvetica Now Display Bold', h.font.includes('Helvetica Now Display Bold'), h.font);
  check('heading size 48px at 1440w (clamp cap 3rem)', h.size === '48px', h.size);
  check('heading line-height 1.05 (50.4px)', h.lineHeight === '50.4px', h.lineHeight);
  check('heading letter-spacing -0.01em (-0.48px)', h.letterSpacing === '-0.48px', h.letterSpacing);
  check('heading color #192837', h.color === 'rgb(25, 40, 55)', h.color);
  check('heading margin-bottom 24px', h.marginBottom === '24px', h.marginBottom);
  ...
  check('subtext line-height 1.65 (29.04px)', s.lineHeight === '29.04px', s.lineHeight);
  ...
  check('CTA glow shadow', c.shadow.includes('rgba(115, 66, 226, 0.28)'), c.shadow);
  ...
  check('CTA has ArrowRightCircle icon @20px', c.icons === 1 && c.iconWidth === '20');
  ...
  check('CTA hover scales up', /matrix\(1\.0[3-4]/.test(hovered.transform), hovered.transform);
  check('CTA hover brightens', /brightness\(1\.0[5-9]|brightness\(1\.1/.test(hovered.filter), hovered.filter);
...
/* ---------------- Mobile (390x844) ---------------- */
{
  ...
  check('desktop nav hidden on mobile', !(await page.locator('nav >> text=Plans').first().isVisible()));
  const toggle = page.getByRole('button', { name: 'Open menu' });
  check('hamburger visible on mobile', await toggle.isVisible());

  const mobileH = await page.locator('h1').evaluate((el) => getComputedStyle(el).fontSize);
  check('heading clamps to 1.65rem (26.4px) on mobile', mobileH === '26.4px', mobileH);
  ...
  check('sheet width min(88vw,360px) = 343.2px', Math.abs(sh.width - 343.2) < 1, String(sh.width));
  check('sheet full height (100dvh)', Math.abs(sh.height - 844) < 1, String(sh.height));
  check('sheet flush right', Math.abs(sh.right - 390) < 1, String(sh.right));
  check('sheet shadow', sh.shadow.includes('rgba(25, 40, 55, 0.18)'), sh.shadow);
  ...
  const backdrop = await page.evaluate(() => {
    ...
    const el = els.find((d) => getComputedStyle(d).backgroundColor === 'rgba(25, 40, 55, 0.35)');
    if (!el) return null;
    const cs = getComputedStyle(el);
    return { blur: cs.backdropFilter || cs.webkitBackdropFilter, position: cs.position };
  });
  check('backdrop rgba(25,40,55,0.35) present', backdrop !== null);
  check('backdrop blur(4px)', backdrop?.blur === 'blur(4px)', backdrop?.blur ?? 'none');
  check('backdrop fixed', backdrop?.position === 'fixed', backdrop?.position ?? 'none');

  /* Sheet content */
  const sheetLinks = await sheet.locator('nav a').count();
  check('sheet lists 5 nav links', sheetLinks === 5, String(sheetLinks));
  check('sheet has Start For Free CTA', await sheet.getByRole('button', { name: 'Start For Free' }).isVisible());
  check('sheet has Sign In CTA', await sheet.getByRole('button', { name: 'Sign In' }).isVisible());
  check('sheet links faded in (stagger done)', await sheet.locator('nav a').last().evaluate(
  ...
  /* Close via Escape */
  await toggle.click();
  await page.getByRole('dialog', { name: 'Mobile navigation' }).waitFor({ state: 'visible' });
  await page.keyboard.press('Escape');
  await page.getByRole('dialog', { name: 'Mobile navigation' }).waitFor({ state: 'detached' });
  check('sheet closes via Escape', (await page.getByRole('dialog').count()) === 0);
