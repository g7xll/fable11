document.addEventListener('DOMContentLoaded', () => {
  ...
  if (belt && !reduce) {
    ...
    setInterval(() => {
      ...
      if (i >= total) {
        setTimeout(() => { belt.style.transition = 'none'; i = 0; belt.style.transform = 'translateX(0)'; }, 920);
      }
    }, 3000);
  }
  ...
  if (aiNode && aiBar) {
    aiNode.addEventListener('click', () => {
      aiBar.style.width = '0%';
      setTimeout(() => { aiBar.style.width = (60 + Math.floor(Math.random() * 40)) + '%'; }, 120);
    });
  }
  ...
  if (term) {
    ...
    term.addEventListener('mouseenter', () => {
      if (reduce) return;
      lines.forEach((l, k) => {
        l.style.opacity = '0'; l.style.transform = 'translateX(-10px)';
        setTimeout(() => { l.style.opacity = '1'; l.style.transform = 'translateX(0)'; }, k * 90);
      });
    });
  }
  ...
  if (counter && !reduce) {
    ...
    setInterval(() => {
      last.style.transform = 'translateY(-10px)'; last.style.opacity = '0';
      setTimeout(() => { last.textContent = Math.floor(Math.random() * 10); last.style.transform = 'translateY(0)'; last.style.opacity = '1'; }, 150);
    }, 2600);
  }
  ...
  if (wm) {
    wm.innerHTML = wm.textContent.split('').map(c => `<span>${c}</span>`).join('');
    ...
    wm.addEventListener('mousemove', e => {
      chars.forEach(ch => {
        ...
        } else { ch.style.color = ''; ch.style.opacity = '0.1'; ch.style.transform = 'scale(1)'; }
      });
    });
    wm.addEventListener('mouseleave', () => chars.forEach(ch => { ch.style.color = ''; ch.style.opacity = '0.1'; ch.style.transform = 'scale(1)'; }));
  }
});
