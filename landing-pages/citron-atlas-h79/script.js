(function () {
  ...
  function wireSlider(trackId, prevId, nextId, step) {
    ...
    if (!track) return;
    var amount = step || function () { return track.clientWidth; };
    if (prev) prev.addEventListener('click', function () {
      track.scrollBy({ left: -(typeof amount === 'function' ? amount() : amount), behavior: 'smooth' });
    });
    if (next) next.addEventListener('click', function () {
      track.scrollBy({ left: (typeof amount === 'function' ? amount() : amount), behavior: 'smooth' });
    });
  }
  wireSlider('indTrack', 'indPrev', 'indNext', function () { return 420; });
  ...
  tabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      ...
      tabs.forEach(function (t) { t.classList.toggle('is-open', t === tab); });
