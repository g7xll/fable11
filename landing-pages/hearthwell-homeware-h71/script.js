(function () {
  ...
  function makeSlider(slides, dotsHost, opts) {
    ...
    function start() {
      ...
      timer = setInterval(function () { if (!paused) next(); }, opts.interval);
    }
    function restart() { clearInterval(timer); start(); }
    if (opts.pauseEl) {
      opts.pauseEl.addEventListener("mouseenter", function () { paused = true; });
      opts.pauseEl.addEventListener("mouseleave", function () { paused = false; });
    }
    go(0);
    start();
  }
  ...
  email.addEventListener("input", function () { email.style.borderColor = ""; });
})();
