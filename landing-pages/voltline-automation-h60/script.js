(() => {
  ...
  if (btn && grid) {
    ...
    btn.addEventListener('mouseenter', () => {
      ...
      shuffle().forEach((c, i) => timers.push(setTimeout(() => { c.style.opacity = '1'; }, i * 22)));
    });
    btn.addEventListener('mouseleave', () => {
      clear();
      btn.classList.remove('is-filled');
      shuffle().forEach((c, i) => timers.push(setTimeout(() => { c.style.opacity = '0'; }, i * 14)));
    });
  }
  ...
  document.querySelectorAll('.spacebtn').forEach((b) => {
    ...
    if (!field) return;
    seedStars(field, 14);
    b.addEventListener('mouseenter', () => { field.style.opacity = '1'; });
    b.addEventListener('mouseleave', () => { field.style.opacity = '0'; });
  });
  ...
  if (reduced || !('IntersectionObserver' in window)) {
    statNums.forEach((el) => {
      el.innerHTML = el.dataset.count + '<span class="u">' + (el.dataset.suffix || '') + '</span>';
    });
  } else {
    const sio = new IntersectionObserver((entries) => {
      entries.forEach((e) => {
        if (e.isIntersecting) { animateStat(e.target); sio.unobserve(e.target); }
      });
    }, { threshold: 0.6 });
