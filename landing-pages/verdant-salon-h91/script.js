...
(function () {
  ...
  document.querySelectorAll('.img-card, .case-media').forEach(function (card) {
  ...
  var io = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      ...
      if (e.target.matches('.img-card, .case-media')) e.target.classList.add('revealed');
      io.unobserve(e.target);
    });
  }, { threshold: 0.12 });

  document.querySelectorAll('.reveal, .img-card, .case-media').forEach(function (el) {
    io.observe(el);
  });
  ...
  document.querySelectorAll('[data-target]').forEach(function (el) { statIO.observe(el); });
