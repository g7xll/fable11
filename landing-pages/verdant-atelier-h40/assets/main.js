...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (burger) burger.addEventListener('click', () => toggle(!mobile.classList.contains('open')));
  document.querySelectorAll('.mobile a').forEach((a) => a.addEventListener('click', () => toggle(false)));

  // City pills toggle
  document.querySelectorAll('.pills span').forEach((p) => {
    p.addEventListener('click', () => {
      p.parentElement.querySelectorAll('span').forEach((s) => s.classList.remove('active'));
