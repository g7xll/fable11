...
(function () {
  ...
  /* ---------- Solutions dropdown ---------- */
  const menuBtn = $("#menu-btn"), dropdown = $("#dropdown");
  function closeMenu() { dropdown.classList.remove("open"); menuBtn.classList.remove("open"); menuBtn.setAttribute("aria-expanded", "false"); }
  ...
  document.addEventListener("click", (e) => { if (!dropdown.contains(e.target) && !menuBtn.contains(e.target)) closeMenu(); });
  ...
  /* ---------- Pricing toggles ---------- */
  const pricing = {
    retainer: { off: false, on: { amt: "$14k", per: "/ month" }, alt: { amt: "$36k", per: "/ quarter" } },
    sprint: { off: false, on: { amt: "$52k", per: "/ sprint", cap: "Fixed Engagement" }, alt: { amt: "$9k", per: "/ month", cap: "Standby Retainer" } },
    mandate: { off: false, on: { amt: "1.2%", cap: "Buy-side Advisory" }, alt: { amt: "1.6%", cap: "Sell-side Mandate" } },
  };
  $$(".knob[data-toggle]").forEach((knob) => {
    ...
    knob.addEventListener("click", () => {
      ...
      const amtEl = $("#" + key + "-amt"); if (amtEl) amtEl.textContent = data.amt;
      const perEl = $("#" + key + "-per"); if (perEl && data.per) perEl.textContent = data.per;
      const capEl = $("#" + key + "-cap"); if (capEl && data.cap) capEl.textContent = data.cap;
      // emphasize active label
      const onLbl = $('[data-on="' + key + '"]'), offLbl = $('[data-off="' + key + '"]');
      if (onLbl && offLbl) { onLbl.classList.toggle("t-off", cfg.off); onLbl.classList.toggle("t-on", !cfg.off);
        offLbl.classList.toggle("t-on", cfg.off); offLbl.classList.toggle("t-off", !cfg.off); }
    });
  });

  /* ---------- Marquee build ---------- */
  const ticks = [
    ["NIFTY 50", "+1.24%", "up"], ["S&P 500", "+0.62%", "up"], ["EVIDENCE", "▲", "neu"],
    ["GILT 10Y", "−0.18%", "dn"], ["SIGNAL / NOISE", "4.8 : 1", "neu"], ["VIX", "−2.1%", "dn"],
    ["SHARPE", "2.34", "neu"], ["HANG SENG", "+0.91%", "up"], ["CONVICTION", "▲", "neu"],
    ["BRENT", "+0.44%", "up"], ["ALPHA · TTM", "+31.4%", "up"], ["DXY", "−0.07%", "dn"],
  ];
  const marquee = $("#marquee");
  if (marquee) {
    const build = () => ticks.map(([a, b, c]) =>
      `<span class="tick"><span>${a}</span><span class="${c}">${b}</span></span>`).join("");
  ...
  const tcVal = $("#tc-val"), tcLab = $("#tc-lab");
  let mi = 0;
  if (!reduceMotion && tcVal && tcLab) {
    setInterval(() => {
      mi = (mi + 1) % metrics.length;
      tcVal.style.opacity = "0"; tcLab.style.opacity = "0";
      setTimeout(() => {
        tcVal.innerHTML = metrics[mi][0]; tcLab.textContent = metrics[mi][1];
        drawSpark();
        tcVal.style.opacity = "1"; tcLab.style.opacity = "1";
      }, 320);
    }, 3800);
    [tcVal, tcLab].forEach((e) => { e.style.transition = "opacity 0.3s"; });
  }
  ...
  if (canvas && !reduceMotion) {
    ...
    function resize() {
      ...
      for (let i = 0; i < count; i++) {
        pts.push({ x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - 0.5) * 0.25, vy: (Math.random() - 0.5) * 0.25, c: Math.random() > 0.8 });
      }
    }
    function frame() {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < 120) {
            ctx.strokeStyle = `rgba(243,241,234,${(1 - d / 120) * 0.10})`;
      ...
      for (const p of pts) {
        ...
        ctx.beginPath(); ctx.arc(p.x, p.y, p.c ? 1.8 : 1.2, 0, Math.PI * 2); ctx.fill();
      }
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      ...
      if (!name) { msg.textContent = "Please enter your name."; msg.className = "news-msg err"; return; }
      if (!valid) { msg.textContent = "Please enter a valid email."; msg.className = "news-msg err"; return; }
