(function () {
  ...
  if (console_) {
    var conIo = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          console_.classList.add('in');
          if (chartLine && !reduce) {
            chartLine.style.transition = 'stroke-dashoffset 1.8s cubic-bezier(.2,.7,.2,1) .3s';
  ...
  function onScroll() {
    header.style.borderBottomColor = window.scrollY > 8 ? 'rgba(0,0,0,.07)' : 'transparent';
  }
