(function () {
  ...
  function initSolutions() {
    ...
    var active = 0, timer = null, DURATION = 5200;

    function set(i) {
      active = i;
      steps.forEach(function (s, k) { s.classList.toggle('active', k === i); });
      mocks.forEach(function (m, k) { m.classList.toggle('show', k === i); });
    }
    function restartProgress() {
      ...
      if (bar) { bar.style.transition = 'none'; bar.style.transform = 'scaleX(0)';
        void bar.offsetWidth; bar.style.transition = ''; bar.style.transform = ''; }
    }
    function schedule() {
      if (reduce) return;
      clearTimeout(timer);
      timer = setTimeout(function () { go((active + 1) % steps.length); }, DURATION);
    }
    function go(i) { set(i); restartProgress(); schedule(); }
    ...
    items.forEach(function (item) {
      ...
      btn.addEventListener('click', function () {
        ...
        items.forEach(function (o) {
          ...
          b.style.maxHeight = '0px'; b.style.opacity = '0';
        });
        if (!open) {
  ...
  function initNeural() {
    ...
    var W = 0, H = 0, nodes = [], edges = [], particles = [], running = false, raf = null;
    ...
    function build() {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        dlist.sort(function (a, b) { return a.d - b.d; });
      ...
      for (var p = 0; p < Math.min(edges.length, 40); p++) {
        particles.push({ e: (Math.random() * edges.length) | 0, t: Math.random(), sp: 0.0025 + Math.random() * 0.004 });
      }
    }
    ...
    function draw(now) {
      ...
      // warm wash
      var g = ctx.createRadialGradient(W * 0.5, H * 0.45, 10, W * 0.5, H * 0.45, Math.max(W, H) * 0.7);
      ...
      for (var q = 0; q < particles.length; q++) {
        var pt = particles[q];
        pt.t += pt.sp;
        if (pt.t > 1) { pt.t = 0; pt.e = (Math.random() * edges.length) | 0; }
        var e = edges[pt.e]; if (!e) continue;
      ...
      for (var n = 0; n < nodes.length; n++) {
        ...
        ctx.beginPath(); ctx.arc(nd.x, nd.y, nd.r + pulse * 1.4, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = 'rgba(43,24,10,' + (0.3 + pulse * 0.4) + ')';
        ctx.beginPath(); ctx.arc(nd.x, nd.y, nd.r * 0.45, 0, Math.PI * 2); ctx.fill();
      }
