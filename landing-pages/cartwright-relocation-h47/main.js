(function () {
  ...
  const setHTML = partners.map(([d, name]) => `
  ...
  if (marquee) marquee.innerHTML = `<div class="mset">${setHTML}</div><div class="mset" aria-hidden="true">${setHTML}</div>`;
  ...
  if (proc) {
    ...
    const po = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting) { run(); po.disconnect(); }
    }, { threshold: 0.3 });
    po.observe(proc);
  }
  ...
  if (deck) {
    ...
    const advance = () => {
      ...
      animating = true;
      const top = cards[0];
      top.style.transition = 'transform .55s cubic-bezier(.2,.8,.2,1), opacity .55s';
      ...
      setTimeout(() => {
        ...
        requestAnimationFrame(() => {
          moved.style.transition = 'transform .55s cubic-bezier(.2,.8,.2,1), opacity .55s';
          animating = false;
        });
      }, 560);
    };

    const start = () => { if (!reduce) timer = setInterval(advance, 4200); };
    deck.addEventListener('click', () => { clearInterval(timer); advance(); start(); });
    start();
  }
