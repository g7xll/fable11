(function () {
  ...
  var people = [
    { img: './assets/img/person-1.jpg', q: '"Helix Strata didn’t just solve our problems; they fundamentally evolved how we think about scale. Their architecture let us 10x our volume without increasing overhead by a single percent."', name: 'Sarah Lee', role: 'CEO, TECH INFINITY' },
    { img: './assets/img/person-2.jpg', q: '"They re-platformed a fifteen-year-old monolith with zero downtime. What we thought would take two years and a fortune took two quarters. Genuinely the best partner we’ve hired."', name: 'Marcus Okafor', role: 'COO, NORTHWIND LOGISTICS' },
    { img: './assets/img/person-3.jpg', q: '"The clarity Helix brings to chaos is unmatched. They turned our tangled operations into a clean, observable system we now actually trust to scale us into new markets."', name: 'Priya Nandakumar', role: 'CTO, VERITAS HEALTH' }
  ];
  var idx = 0;
  ...
  if (prev && next) {
    prev.addEventListener('click', function () { idx = (idx - 1 + people.length) % people.length; render(idx); });
    next.addEventListener('click', function () { idx = (idx + 1) % people.length; render(idx); });
  }
})();
