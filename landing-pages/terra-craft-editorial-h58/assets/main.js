(function () {
  ...
  window.addEventListener('load', function () {
    document.querySelectorAll('.hero .reveal, .hero-panel.reveal').forEach(function (el) {
      el.classList.add('in');
    });
  });
  ...
  if (panel && !reduce) {
    ...
    function apply() {
      cards.forEach(function (c) {
        ...
        var f = d / 40;
        c.style.transform = 'translate3d(' + (mx * f).toFixed(2) + 'px,' + (my * f + sy * f * 0.4).toFixed(2) + 'px,0)';
      });
      raf = null;
    }
    ...
    panel.addEventListener('mouseleave', function () { mx = 0; my = 0; schedule(); });
    ...
    cards.forEach(function (c) {
      c.addEventListener('mouseenter', function () { c.style.zIndex = 5; });
    });
  }
  ...
  function animateCount(el) {
    ...
    if (reduce) { el.innerHTML = prefix + target.toLocaleString(); return; }
  ...
  if (form) {
    form.addEventListener('submit', function (ev) {
      ...
      form.innerHTML = '<span class="msg">Thank you — a note from the atelier is on its way.</span>';
    });
  }
})();
