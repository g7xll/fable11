(function () {
  ...
  document.querySelectorAll('[data-split]').forEach(function (el) {
    ...
    nodes.forEach(function (node) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (node.nodeName === 'BR') { el.appendChild(node.cloneNode()); return; }
        var span = document.createElement('span');
        span.className = 'char';
        if (node.className) span.className += ' ' + node.className;
        span.innerHTML = node.innerHTML !== undefined ? node.innerHTML : node.textContent;
        el.appendChild(span);
      }
    });
  });
  ...
  if (reduced) {
    ...
    document.querySelectorAll('[data-split]').forEach(function (el) {
      el.querySelectorAll('.char').forEach(function (c) { c.classList.add('show'); });
    });
    return;
  }
  ...
  var observer = new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      if (entry.isIntersecting) {
        ...
        entry.target.querySelectorAll('[data-split] .char').forEach(function (c) { c.classList.add('show'); });
  ...
  var headObserver = new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      if (entry.isIntersecting) {
        entry.target.querySelectorAll('.char').forEach(function (c) { c.classList.add('show'); });
