(() => {
  ...
  if (layer && !reduce) {
    const cell = 10, count = 50;
    const cols = Math.ceil(window.innerWidth / cell), rows = Math.ceil(32 / cell);
  ...
  function setLine(p, text) { p.innerHTML = text ? text.replace(/ /g, '&nbsp;') : '&nbsp;'; }
  function buildTerminal() {
    term.innerHTML = '';
    lines.forEach(ln => { const p = document.createElement('p'); setLine(p, ln); term.appendChild(p); });
  ...
  function typeTerminal() {
    ...
    function nextLine() {
      ...
      const text = lines[li];
      if (text === '') { setLine(p, ''); li++; setTimeout(nextLine, 80); return; }
      let ci = 0;
      (function typeChar() {
  ...
  if (term) { reduce ? buildTerminal() : typeTerminal(); }
  ...
  function animateCount(el) {
    ...
    if (reduce) { el.textContent = el.dataset.count + suffix; return; }
  ...
  if (envelope) {
    if (reduce) {
      ...
      const eio = new IntersectionObserver((entries, obs) => {
        entries.forEach(e => { if (e.isIntersecting) { envelope.classList.add('open'); obs.unobserve(e.target); } });
      }, { threshold: 0.4 });
