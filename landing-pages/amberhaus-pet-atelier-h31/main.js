document.addEventListener('DOMContentLoaded', () => {
  /* ---------- testimonial data ---------- */
  const reviews = [
    {t:'card', q:'The level of care here is unprecedented. Bella always returns from her spa sessions looking like royalty.', img:'assets/img/av1.jpg', nm:'Sarah Jenkins', rl:"Bella's Guardian"},
    {t:'card', q:'Curated treats my picky companion actually enjoys. The boutique selection is truly top-tier.', img:'assets/img/av2.jpg', nm:'Marcus Thorne', rl:"Oliver's Human"},
    {t:'cream', q:'Amberhaus is the only place I trust with Shadow’s grooming. They are literal magicians.', img:'assets/img/av3.jpg', nm:'Thomas Ward', rl:"Shadow's Best Friend"},
    {t:'image', img:'assets/img/tcard1.jpg', cap:'Pet Portraits MMXXIV'},
    {t:'card', q:'The artisan accessories collection is unmatched. Every piece feels custom-made for our home.', img:'assets/img/av4.jpg', nm:'Julian Pierce', rl:"Baron's Owner"},
    {t:'amber', em:'✨', label:'Next-Generation Pet Boutique'},
    {t:'card', q:'Holistic care that actually respects the pet. Cooper looks amazing and had zero stress.', img:'assets/img/av5.jpg', nm:'Chloe Smith', rl:"Cooper's Mom"},
    {t:'image2', img:'assets/img/tcard2.jpg', label:'luna.', t1:'Lumina Collection', t2:'Luxury Cat Furnishings'},
  ];
  ...
  document.querySelectorAll('[data-slider]').forEach((slider, sIdx) => {
    ...
    }, 5000), sIdx * 1500); // stagger left/right
  });
  ...
  const mt1 = document.getElementById('mtxt1'), mt2 = document.getElementById('mtxt2');
  let menuOpen = false;
  const setMenu = (open) => {
    menuOpen = open;
  ...
  const openOverlay = () => { overlay.classList.add('open'); document.body.style.overflow = 'hidden'; if (menuOpen) setMenu(false); };
  const closeOverlay = () => { overlay.classList.remove('open'); document.body.style.overflow = ''; };

  document.addEventListener('click', (e) => {
    if (e.target.closest('[data-open-overlay]')) { e.preventDefault(); openOverlay(); return; }
    if (e.target.closest('[data-close-overlay]')) { closeOverlay(); return; }
    if (e.target.closest('#menuBtn')) { setMenu(!menuOpen); return; }
    if (menuOpen && !navBox.contains(e.target)) setMenu(false);
  });

  addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { if (overlay.classList.contains('open')) closeOverlay(); if (menuOpen) setMenu(false); }
  });
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', function (e) {
      ...
      if (target) {
        ...
        scrollTo({top: target.getBoundingClientRect().top + scrollY - 80, behavior:'smooth'});
      }
    });
  });
});
