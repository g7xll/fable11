...
function hydrate() {
  ...
  set("link-email", el => el.href = `mailto:${PROFILE.email}?subject=${encodeURIComponent("Hello Nova")}`);
  set("link-call", el => el.href = `tel:${PROFILE.phone}`);
  set("link-wa", el => el.href = `https://wa.me/${PROFILE.whatsapp}?text=${encodeURIComponent("Hi Nova! I found your Lumen card.")}`);
...
function startClock() {
  ...
  const tick = () => { try { out.textContent = fmt.format(new Date()); } catch { out.textContent = new Date().toLocaleTimeString(); } };
  tick();
