		...
		>
			...
			>
				...
				<span className="text-sm font-medium">
					{data.stats.batteryLevel}% Charge
				</span>
			</motion.div>
		</motion.div>
	);
			...
			>
				{options.map((opt) => (
					<motion.button
						...
						className="relative w-24 h-12 rounded-full flex items-center justify-center text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
					>
						{activeId === opt.id && (
							<motion.div
			...
			<header className="fixed top-0 inset-x-0 z-40 pointer-events-none">
				<div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between text-xs uppercase tracking-[0.25em]">
					<span className="flex items-center gap-2 font-display font-semibold text-zinc-200">
						...
						Sonar&nbsp;<span className="text-zinc-500">Spatial</span>
					</span>
					<span className="hidden sm:inline font-mono text-[0.65rem] text-zinc-600">
						AUDIO&nbsp;ENGINE&nbsp;v5.0
					</span>
				</div>
			</header>
