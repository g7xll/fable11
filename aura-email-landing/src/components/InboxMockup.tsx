...
const NAV_ITEMS: { icon: LucideIcon; label: string; count?: number; active?: boolean }[] = [
            ...
            <nav className="mt-4 space-y-0.5">
              {NAV_ITEMS.map(({ icon: Icon, label, count, active }) => (
                ...
                >
                  ...
                  {count !== undefined && <span className="text-[10px] text-white/40">{count}</span>}
                </a>
              ))}
            </nav>
              ...
              {MESSAGES.map((m) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className={`text-xs truncate ${
                        m.unread ? 'font-semibold text-white' : 'font-medium text-white/70'
                      }`}
                    >
                      {m.name}
                    </span>
                  ...
                  <div className="mt-0.5 flex items-center gap-1.5">
                    {m.unread && <span className="w-1.5 h-1.5 rounded-full bg-[#00d2ff] shrink-0" />}
                    <span
                  ...
                  <p className="mt-0.5 text-xs text-white/40 truncate">{m.preview}</p>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              <div className="liquid-glass rounded-lg p-3 mt-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-semibold text-white">Summary by Aura</span>
                </div>
              ...
              <div className="mt-4 space-y-3">
                {BODY_PARAGRAPHS.map((paragraph) => (
                  <p key={paragraph} className="text-xs text-white/70 leading-[1.7]">
                    {paragraph}
                  </p>
                ))}
                <p className="text-xs text-white/50 leading-[1.7]">— The Linear team</p>
              </div>
