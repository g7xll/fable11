...
/* ---------------- Desktop (1440x900) ---------------- */
{
  ...
  check('hls.js attached MediaSource (blob: src)', v.src.startsWith('blob:'), v.src.slice(0, 30));
  check('video autoplay/muted/loop/playsInline', v.autoplay && v.muted && v.loop && v.playsInline);
  ...
  check(
    ...
    await lines.first().evaluate((el) => getComputedStyle(el.parentElement).display !== 'none'),
  );
  ...
  check(
    ...
    (await glow.locator('feGaussianBlur').getAttribute('stdDeviation')) === '25',
  );
  ...
  check('card is 200x200px', c.width === 200 && c.height === 200, `${c.width}x${c.height}`);
  check('card shifted -50px (translate-y-[-50px])', c.transform === 'matrix(1, 0, 0, 1, 0, -50)', c.transform);
  check('card bg rgba(255,255,255,0.01)', c.background === 'rgba(255, 255, 255, 0.01)', c.background);
  check('card background-blend-mode luminosity', c.blend === 'luminosity', c.blend);
  check('card backdrop-filter blur(4px)', c.backdrop === 'blur(4px)', c.backdrop);
  check('card inset highlight shadow', c.shadow.includes('inset'), c.shadow);
  check('::before border padding 1.4px', c.beforePadding === '1.4px', c.beforePadding);
  check(
    '::before mask-composite exclude/xor',
    /exclude|xor/.test(c.beforeMaskComposite),
    c.beforeMaskComposite,
  );
  check('::before 180deg gradient frame', c.beforeBg.includes('linear-gradient'), c.beforeBg.slice(0, 40));

  check('card tag "[ 2025 ]" visible', await page.getByText('[ 2025 ]').isVisible());
  ...
  check('"Industry" in Instrument Serif italic', serifCss.family.includes('Instrument Serif') && serifCss.style === 'italic', `${serifCss.family} / ${serifCss.style}`);
  ...
  check('eyebrow Plus Jakarta Sans', e.family.includes('Plus Jakarta Sans'), e.family);
  check('eyebrow 11px bold', e.size === '11px' && e.weight === '700', `${e.size}/${e.weight}`);
  ...
  check('headline Inter Extra Bold', h.family.startsWith('Inter') && h.weight === '800', `${h.family}/${h.weight}`);
  check('headline 72px on desktop', h.size === '72px', h.size);
  check('headline uppercase + tracking-tight', h.transform === 'uppercase' && h.tracking < 0, `${h.transform}/${h.tracking}px`);
  ...
  const d = await desc.evaluate((el) => {
    const cs = getComputedStyle(el);
    return { size: cs.fontSize, color: cs.color, maxWidth: cs.maxWidth, family: cs.fontFamily };
  });
  check('description Inter 14px', d.family.startsWith('Inter') && d.size === '14px', `${d.family}/${d.size}`);
  check('description 70% white', d.color === 'rgba(255, 255, 255, 0.7)', d.color);
  ...
  check('CTA pill / uppercase / bold', parseFloat(b.radius) > 100 && b.transform === 'uppercase' && b.weight === '700', `${b.radius}/${b.transform}/${b.weight}`);
  check('CTA has ArrowRight icon', b.hasIcon);

  /* Navigation */
  check('logo "CODENEST" visible', await page.getByText('CODENEST').first().isVisible());
  const nav = page.locator('nav[aria-label="Primary"]');
  for (const item of ['PROJECTS', 'BLOG', 'ABOUT', 'RESUME']) {
    check(`desktop nav "${item}" visible`, await nav.getByText(item).isVisible());
  }
  const navLink = nav.getByText('PROJECTS');
  check('nav links 16px Inter', (await navLink.evaluate((el) => getComputedStyle(el).fontSize)) === '16px');
  await navLink.hover();
...
/* ---------------- Mobile (390x844) ---------------- */
{
  ...
  check(
    ...
    !(await page.locator('nav[aria-label="Primary"]').getByText('PROJECTS').isVisible()),
  );
  check(
  ...
  check(
    ...
    (await page.locator('h1').evaluate((el) => getComputedStyle(el).fontSize)) === '40px',
  ...
  const o = await overlay.evaluate((el) => {
    ...
    const cs = getComputedStyle(el);
    return { w: r.width, h: r.height, x: r.x, y: r.y, position: cs.position, bg: cs.backgroundColor };
  });
  check('overlay is full-screen fixed', o.position === 'fixed' && o.x === 0 && o.y === 0 && o.w === 390 && o.h === 844, `${o.w}x${o.h}@${o.x},${o.y}`);
  ...
  check('overlay closes via X', (await page.locator('#mobile-menu').count()) === 0);
