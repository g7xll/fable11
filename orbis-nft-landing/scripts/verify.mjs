...
check('Page loads (HTTP 200)', res?.status() === 200, `status=${res?.status()}`)

// --- Fonts ---
const fontHref = await page.$eval('link[href*="fonts.googleapis.com/css2"]', (el) => el.getAttribute('href'))
check(
  'Google Fonts link loads Anton + Condiment',
  fontHref === 'https://fonts.googleapis.com/css2?family=Anton&family=Condiment&display=swap',
  String(fontHref),
)
...
check('6 videos on the page (hero, about, 3 cards, cta)', videos.length === 6, `found=${videos.length}`)
check(
...
check(
  ...
  videos.slice(0, 5).every((v) => v.objectFit === 'cover') && videos[5].objectFit !== 'cover',
  videos.map((v) => v.objectFit).join(','),
)
const EXPECTED_IDS = ['045634', '151551', '053923', '054411', '055427', '055729']
check(
...
// --- Navigation ---
const navLinks = await page.$$eval('nav a', (els) => els.map((a) => a.textContent?.trim()))
check(
  'Nav has the 5 links',
  JSON.stringify(navLinks) === JSON.stringify(['Homepage', 'Gallery', 'Buy NFT', 'FAQ', 'Contact']),
  navLinks.join(', '),
)
...
check('Hero heading text', /Beyond earth\s*and \( its \) familiar boundaries/.test(h1 ?? ''), JSON.stringify(h1))
...
check(
  ...
  h1Style.transform === 'uppercase' && h1Style.font.includes('Anton') && h1Style.size === '90px',
...
for (const { text, blend } of [
  ...
  const info = await page.evaluate((t) => {
    const span = [...document.querySelectorAll('span')].find(
      (el) => el.textContent?.trim() === t && getComputedStyle(el).fontFamily.includes('Condiment'),
    )
    if (!span) return null
    const s = getComputedStyle(span)
    return { color: s.color, blend: s.mixBlendMode, transform: s.textTransform }
  }, text)
  check(
...
const glass = await page.$$eval('.liquid-glass', (els) =>
  els.map((el) => {
    const s = getComputedStyle(el)
    return { blur: s.backdropFilter || s.webkitBackdropFilter, overflow: s.overflow, position: s.position }
  }),
)
check('Liquid glass applied to 13+ elements', glass.length >= 13, `count=${glass.length}`)
...
check(
  ...
  glass.every((g) => g.blur.includes('blur(4px)') && g.position !== 'static' && g.overflow === 'hidden'),
)

// --- Collection ---
const scores = await page.$$eval('article', (cards) => cards.map((c) => c.textContent ?? ''))
check('3 NFT cards', scores.length === 3, `count=${scores.length}`)
check(
  'Rarity scores 8.7/10, 9/10, 8.2/10',
  ['8.7/10', '9/10', '8.2/10'].every((s, i) => scores[i]?.includes(s) && scores[i]?.toLowerCase().includes('rarity score:')),
)
const gridCols = await page.$eval('article', (card) =>
...
const arrowBtn = await page.$eval('article button', (b) => {
  ...
  return { w: s.width, h: s.height, radius: s.borderRadius, bg: s.backgroundImage, svg: !!b.querySelector('svg') }
})
check(
  'Card arrow button is 48px circular purple gradient with SVG chevron',
  arrowBtn.w === '48px' && arrowBtn.h === '48px' && arrowBtn.bg.includes('linear-gradient') && arrowBtn.svg,
  JSON.stringify(arrowBtn),
)

// --- See all creators ---
const body = await page.textContent('body')
check('SEE ALL CREATORS button text present', /See/.test(body) && /All/.test(body) && /Creators/.test(body))
const neonBar = await page.evaluate(() => {
  const bars = [...document.querySelectorAll('span')].filter(
    (el) => getComputedStyle(el).backgroundColor === 'rgb(111, 255, 0)' && el.textContent === '',
  )
  return bars.length > 0 ? getComputedStyle(bars[0]).height : null
})
check('Neon underline bar under SEE ALL CREATORS (10px at lg)', neonBar === '10px', String(neonBar))

// --- CTA ---
check(
  'CTA heading lines present',
  ['Join us.', 'Reveal what’s hidden.', 'Define what’s next.', 'Follow the signal.'].every((l) =>
    body.includes(l),
  ),
)
...
check('texture.png served (HTTP 200, PNG)', texRes.status() === 200 && (await texRes.body()).subarray(1, 4).toString() === 'PNG')
...
const mobileCols = await page.$eval('article', (card) =>
  getComputedStyle(card.parentElement).gridTemplateColumns.split(' ').length,
)
...
const heroSocialVisible = await page.$$eval('header ~ div a[aria-label]', (els) =>
  els.filter((el) => el.checkVisibility()).length,
)
check('Mobile hero social icons visible (3, centered row)', heroSocialVisible === 3, `visible=${heroSocialVisible}`)
await page.screenshot({ path: '/tmp/orbis-mobile.png', fullPage: true })

// --- Tablet grid ---
await page.setViewportSize({ width: 800, height: 1000 })
await page.waitForTimeout(300)
const tabletCols = await page.$eval('article', (card) =>
  getComputedStyle(card.parentElement).gridTemplateColumns.split(' ').length,
)
