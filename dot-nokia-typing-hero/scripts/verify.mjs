...
try {
  ...
  check(
    ...
    Boolean(
      video && video.autoplay && video.loop && video.muted && video.playsInline &&
  ...
  const sawTyping = sampleArr.some(
    (t) => t !== "Are you here?" && "Are you here?".startsWith(t) && t.length > 0,
  );
  ...
  check(
    "Nokia webfont CSS loads",
    nokiaCss.ok,
    `status=${nokiaCss.status}`,
  );
