...
check('video absolute + object-cover', vidStyle.position === 'absolute' && vidStyle.objectFit === 'cover', JSON.stringify(vidStyle))
...
const rootBg = await page.evaluate(() => getComputedStyle(document.querySelector('#root > div')).backgroundColor)
...
check('foreground z-10 flex-col', fgZ.z === '10' && fgZ.display === 'flex' && fgZ.dir === 'column', JSON.stringify(fgZ))

// --- Logo SVG ---
const svg = page.locator('nav svg')
check('logo svg 18x18 viewBox 256', (await svg.getAttribute('width')) === '18' && (await svg.getAttribute('height')) === '18' && (await svg.getAttribute('viewBox')) === '0 0 256 256')
...
check('logo path d starts/ends correctly', pathD.startsWith('M 160 88') && pathD.endsWith('Z'))

// --- Navbar ---
const navLinks = await page.locator('nav a').allTextContents()
check('nav links Story/Products/Help/Support', JSON.stringify(navLinks) === JSON.stringify(['Story', 'Products', 'Help', 'Support']), JSON.stringify(navLinks))
const pillBgs = await page.evaluate(() =>
  [...document.querySelectorAll('nav > div')].map((d) => getComputedStyle(d).backgroundColor),
)
check('both pills #EDEDED', pillBgs.length === 2 && pillBgs.every((c) => c === 'rgb(237, 237, 237)'), JSON.stringify(pillBgs))
...
check('logo pill circular 44px (sm:w-11)', logoPill.w === 44 && logoPill.h === 44, JSON.stringify(logoPill))
const navLinkSize = await page.evaluate(() => getComputedStyle(document.querySelector('nav a')).fontSize)
...
check('badge text + arrow', (await badge.count()) === 1 && (await badge.textContent()).includes('→'))
const badgeSize = await badge.evaluate((el) => getComputedStyle(el).fontSize)
check('badge 11.5px', badgeSize === '11.5px', badgeSize)

const h1 = page.locator('h1')
check('headline text', (await h1.textContent()).trim() === 'Simple, smart prosthetics made for people who keep fighting.')
const h1Style = await h1.evaluate((el) => {
  const s = getComputedStyle(el)
  return { size: s.fontSize, weight: s.fontWeight, lh: s.lineHeight }
})
check('headline 1.75rem(28px)/500 at desktop', h1Style.size === '28px' && h1Style.weight === '500', JSON.stringify(h1Style))
...
check('subtext 13px gray-400', subStyle.size === '13px' && subStyle.color === 'rgb(156, 163, 175)', JSON.stringify(subStyle))

const cta = page.locator('a', { hasText: 'Try a free fitting' })
check('CTA text + arrow', (await cta.count()) === 1 && (await cta.textContent()).includes('→'))
...
check('CTA resting: blue-500 text, blue-400 border', ctaBefore.color === 'rgb(59, 130, 246)' && ctaBefore.border === 'rgb(96, 165, 250)', JSON.stringify(ctaBefore))
...
check('CTA hover fills blue-500 / white text', ctaAfter.bg === 'rgb(59, 130, 246)' && ctaAfter.color === 'rgb(255, 255, 255)', JSON.stringify(ctaAfter))
const arrowTransform = await cta.locator('span').evaluate((el) => getComputedStyle(el).transform)
check('CTA arrow translates on hover', arrowTransform.includes('matrix') && arrowTransform !== 'none', arrowTransform)

const firstNav = page.locator('nav a').first()
await firstNav.hover()
await page.waitForTimeout(350)
const navHoverColor = await firstNav.evaluate((el) => getComputedStyle(el).color)
check('nav link hover gray-900', navHoverColor === 'rgb(17, 24, 39)', navHoverColor)

// --- No extra sections ---
const anchors = await page.locator('a').count()
check('exactly 6 anchors (4 nav + badge + CTA)', anchors === 6, String(anchors))
check('no extra h2/section/footer', (await page.locator('h2, section, footer, form').count()) === 0)

// --- Mobile viewport sanity ---
await page.setViewportSize({ width: 375, height: 720 })
await page.waitForTimeout(200)
const mobNavSize = await page.evaluate(() => getComputedStyle(document.querySelector('nav a')).fontSize)
...
const mobLogoPill = await page.evaluate(() => document.querySelector('nav > div').offsetWidth)
check('logo pill 40px (w-10) on mobile', mobLogoPill === 40, String(mobLogoPill))
