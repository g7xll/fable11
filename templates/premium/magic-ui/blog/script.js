...
(function () {
  ...
  if (stored === "dark") { root.classList.remove("light"); root.classList.add("dark"); }
...
function initFilter() {
  ...
  if (mob && list) mob.addEventListener("click", () => {
...
function initTOC() {
  ...
  const headings = [...tocButtons].map((b) => document.getElementById(b.dataset.target)).filter(Boolean);
  tocButtons.forEach((b) => b.addEventListener("click", () => {
    const el = document.getElementById(b.dataset.target);
    if (el) window.scrollTo({ top: el.getBoundingClientRect().top + scrollY - 80, behavior: "smooth" });
  }));
  function spy() {
    let active = headings[0];
    for (const h of headings) { if (h.getBoundingClientRect().top - 100 <= 0) active = h; }
    tocButtons.forEach((b) => b.classList.toggle("active", b.dataset.target === (active && active.id)));
  }
  document.addEventListener("scroll", spy, { passive: true });
  spy();
  // heading click-to-copy
  document.querySelectorAll(".prose h2[id]").forEach((h) => h.addEventListener("click", () => {
