...
(function () {
  ...
  const CLOCK = '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>';
  const LOGO = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c.5 2.2.5 3.6 0 5.8C13.6 6.2 14.7 5.3 16.6 4.1 15.4 6 14.5 6.9 12.9 8.5c2.2-.5 3.6-.5 5.8 0-2.2.5-3.6.5-5.8 0 1.6 1.6 2.5 2.5 3.7 4.4-1.9-1.2-2.8-2.1-4.4-3.7.5 2.2.5 3.6 0 5.8-.5-2.2-.5-3.6 0-5.8-1.6 1.6-2.5 2.5-4.4 3.7 1.2-1.9 2.1-2.8 3.7-4.4-2.2.5-3.6.5-5.8 0 2.2-.5 3.6-.5 5.8 0C9.9 6.9 9 6 7.8 4.1 9.7 5.3 10.6 6.2 12 7.8c-.5-2.2-.5-3.6 0-5.8Z"/></svg>';
  ...
  function buildMenu() {
    ...
    ov.querySelectorAll('.menu-nav a').forEach(a => { if (a.getAttribute('href') === cur) a.classList.add('active'); });
    document.addEventListener('click', e => {
      if (e.target.closest('[data-menu-open]')) ov.classList.add('open');
      if (e.target.closest('[data-menu-close]') || e.target === ov) ov.classList.remove('open');
    });
  ...
  function initTheme() {
    ...
    function render() { if (btn) btn.innerHTML = document.documentElement.classList.contains('dark') ? MOON : SUN; }
    render();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    buildHeader(); buildMenu(); buildFooter();
    initTheme(); startClock(); initReveal();
  });
})();
