...
function applyTheme(t) {
  ...
  try { localStorage.setItem("mainline-theme", t); } catch (e) {}
}
...
const ICONS = {
  chevron: '<svg class="chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>',
  ...
  arrow: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 17 17 7M7 7h10v10"/></svg>',
};

const FEATURES = [
  { title: "Modern product teams", desc: "Mainline is built on the habits that make the best product teams successful", href: "/#feature-modern-teams",
  ...
  { title: "Resource Allocation", desc: "Mainline your resource allocation and execution", href: "/#resource-allocation",
...
function navLink(href, label, page) {
  const active = page === label.toLowerCase().replace(/\s/g, "") ? " active" : "";
...
function wireInteractions() {
  ...
  document.querySelectorAll(".nav-dropdown").forEach(dd => {
    ...
    const open = (v) => { dd.classList.toggle("open", v); trig.setAttribute("aria-expanded", v); };
    dd.addEventListener("mouseenter", () => open(true));
    dd.addEventListener("mouseleave", () => open(false));
    trig.addEventListener("click", (e) => { e.preventDefault(); open(!dd.classList.contains("open")); });
  });

  // mobile menu
  const mm = document.querySelector(".mobile-menu");
  document.querySelector(".nav-burger")?.addEventListener("click", () => mm.classList.add("open"));
  document.querySelector(".mobile-close")?.addEventListener("click", () => mm.classList.remove("open"));
  ...
  document.querySelectorAll("[data-accordion]").forEach(acc => {
    acc.querySelectorAll(".acc-item").forEach(item => {
      ...
      trig.addEventListener("click", () => {
        ...
        if (acc.dataset.accordion === "single") acc.querySelectorAll(".acc-item").forEach(i => setAcc(i, false));
        setAcc(item, !isOpen);
      });
    });
  });
