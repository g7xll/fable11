...
(function () {
  ...
  document.querySelectorAll(".custom-select").forEach((sel) => {
    ...
    selected.addEventListener("click", (e) => {
      ...
      document.querySelectorAll(".select-items").forEach((o) => { if (o !== items) o.setAttribute("hidden", ""); });
      document.querySelectorAll(".select-selected").forEach((o) => { if (o !== selected) o.classList.remove("select-arrow-active"); });
      if (hidden) { items.removeAttribute("hidden"); selected.classList.add("select-arrow-active"); }
      else { items.setAttribute("hidden", ""); selected.classList.remove("select-arrow-active"); }
    });
    items.querySelectorAll(".select-item").forEach((it) => {
      it.addEventListener("click", () => {
        const lbl = selected.querySelector("span");
        if (lbl && !it.querySelector("a")) lbl.textContent = it.textContent.trim();
      });
    });
  });
  document.addEventListener("click", () => {
    document.querySelectorAll(".select-items").forEach((o) => o.setAttribute("hidden", ""));
    document.querySelectorAll(".select-selected").forEach((o) => o.classList.remove("select-arrow-active"));
  });
  ...
  if (window.matchMedia("(max-width: 1279px)").matches) {
    document.querySelectorAll("header nav li.group > a[href='#']").forEach((a) => {
  ...
  if (window.Swiper) {
    ...
    function navFor(el) {
      let next = el.querySelector(":scope > .swiper-button-next, :scope > * .swiper-button-next");
      let prev = el.querySelector(":scope > .swiper-button-prev, :scope > * .swiper-button-prev");
      ...
      return next instanceof Element && prev instanceof Element ? { nextEl: next, prevEl: prev } : false;
    }
    function init(el, opts) {
      if (!(el instanceof Element) || el.swiper) return;
      try { new Swiper(el, opts); } catch (e) { /* ignore malformed carousel */ }
    }

    document.querySelectorAll(".hero-carousel").forEach((el) => {
      let pag = el.querySelector(".swiper-pagination");
      if (!pag) { pag = document.createElement("div"); pag.className = "swiper-pagination"; el.appendChild(pag); }
      init(el, { spaceBetween: 30, loop: true, autoplay: { delay: 4000, disableOnInteraction: false }, pagination: { el: pag, clickable: true } });
    });

    document.querySelectorAll(".categories-carousel").forEach((el) => {
      init(el, {
        slidesPerView: 2, spaceBetween: 20,
        breakpoints: { 480: { slidesPerView: 3 }, 640: { slidesPerView: 4 }, 768: { slidesPerView: 5 }, 1000: { slidesPerView: 6 } },
    ...
    document.querySelectorAll(".swiper").forEach((el) => {
      if (el.swiper) return;
      if (el.classList.contains("hero-carousel") || el.classList.contains("categories-carousel") || el.classList.contains("testimonial-carousel")) return;
      // skip nested wrappers / non-carousel swiper utility divs with no slides
      if (!el.querySelector(".swiper-wrapper") || !el.querySelector(".swiper-slide")) return;
      init(el, {
        slidesPerView: 1, spaceBetween: 20,
        breakpoints: { 480: { slidesPerView: 2 }, 768: { slidesPerView: 3 }, 1000: { slidesPerView: 4 } },
        navigation: navFor(el),
      });
    });
  }
  ...
  (function countdown() {
    ...
    if (!nums.length) {
      const candidates = Array.from(document.querySelectorAll("span,p,div,h3,h4")).filter((e) => {
    ...
    const target = Date.now() + (3 * 24 * 3600 + 12 * 3600 + 47 * 60 + 30) * 1000;
    function tick() {
      ...
      const vals = [d, h, m, s];
      for (let i = 0; i < 4; i++) nums[i].textContent = String(vals[i]).padStart(2, "0");
    }
    tick();
  ...
  /* ---------- Quantity steppers ---------- */
  document.querySelectorAll('[aria-label="button for remove product"], [aria-label="button for add product"]').forEach((btn) => {
  ...
  (function tabs() {
    const tabBtns = Array.from(document.querySelectorAll("button")).filter((b) => {
      ...
      return t === "Description" || t === "Additional Information" || t === "Reviews";
    });
    if (tabBtns.length < 2) return;
    const tabBar = tabBtns[0].parentElement;
    const panels = {};
    const descPanel = tabBar.parentElement && tabBar.parentElement.nextElementSibling;
    ...
    tabBtns.forEach((b) => b.addEventListener("click", () => setActive(b.textContent.trim())));
  })();

  /* ---------- Shop grid / list view toggle ---------- */
  (function viewToggle() {
    const gridBtn = document.querySelector('[aria-label="button for product grid tab"]');
    const listBtn = document.querySelector('[aria-label="button for product list tab"]');
    if (!gridBtn || !listBtn) return;
    const cards = document.querySelectorAll(".group");
    const productGrid = cards.length ? cards[0].parentElement : null;
    function activate(btn) {
      [gridBtn, listBtn].forEach((b) => b.classList.toggle("text-blue", b === btn));
    }
    gridBtn.addEventListener("click", () => {
      activate(gridBtn);
      if (productGrid) { productGrid.style.display = ""; productGrid.classList.remove("nm-list-view"); }
    });
    listBtn.addEventListener("click", () => {
      activate(listBtn);
      if (productGrid) { productGrid.classList.add("nm-list-view"); }
    });
  })();

  /* ---------- Mini-cart drawer ---------- */
  (function cartDrawer() {
    const cartBtn = Array.from(document.querySelectorAll("header button")).find((b) =>
      b.querySelector("svg path[d^='M12 4V5']") || (b.querySelector("span.bg-red") && b.querySelector("svg"))
    );
    if (!cartBtn) return;
    const overlay = document.createElement("div"); overlay.className = "nm-overlay";
    const drawer = document.createElement("div"); drawer.className = "nm-cart-drawer";
    drawer.innerHTML =
    ...
    function open() { overlay.classList.add("show"); drawer.classList.add("show"); document.body.style.overflow = "hidden"; }
    function close() { overlay.classList.remove("show"); drawer.classList.remove("show"); document.body.style.overflow = ""; }
  ...
  /* ---------- Quick-view modal ---------- */
  (function quickView() {
    const qvBtns = document.querySelectorAll('[aria-label="button for quick view"]');
    if (!qvBtns.length) return;
    const overlay = document.createElement("div"); overlay.className = "nm-overlay nm-qv-overlay";
    const modal = document.createElement("div"); modal.className = "nm-qv-modal";
    modal.innerHTML =
    ...
    function close() { overlay.classList.remove("show"); modal.classList.remove("show"); document.body.style.overflow = ""; }
    overlay.addEventListener("click", close);
    ...
    qvBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (card) {
          ...
          const title = card.querySelector("h3, a[href='product-details.html']");
          if (img) modal.querySelector(".nm-qv-img img").src = img.getAttribute("src");
          if (title) modal.querySelector(".nm-qv-title").textContent = title.textContent.trim();
          const priceText = (card.textContent.match(/\$\s?\d[\d,.]*/g) || []).slice(-1)[0] || "";
          modal.querySelector(".nm-qv-price").textContent = priceText;
        }
        overlay.classList.add("show"); modal.classList.add("show"); document.body.style.overflow = "hidden";
      });
    });
  })();
