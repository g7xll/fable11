...
(function () {
  ...
  document.addEventListener("keydown", function (e) {
    if ((e.metaKey || e.ctrlKey) && (e.key === "k" || e.key === "K")) {
      e.preventDefault();
      if (searchDialog && searchDialog.classList.contains("hidden")) openSearch();
      else closeSearch();
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest('[aria-label="Toggle navigation"]')) { e.preventDefault(); openMobileNav(); }
    else if (e.target.closest('[data-mobilenav-backdrop]')) closeMobileNav();
    else if (mnav && !mnav.classList.contains("hidden") && e.target.closest("#mobile-nav a")) closeMobileNav();
  });

  /* ---------- Code panel tabs ---------- */
  document.querySelectorAll("[data-tab-group]").forEach(function (group) {
    // panels live in a sibling container; scope to the enclosing .not-prose / rounded-2xl wrapper
    var scope = group.closest(".not-prose") || group.parentElement.parentElement;
    ...
    function select(idx) {
      tabs.forEach(function (t, i) {
        ...
        if (on) { t.dataset.selected = ""; } else { delete t.dataset.selected; }
      });
  ...
  document.addEventListener("click", function (e) {
    ...
    if (label) {
      ...
      btn.dataset.copied = "";
      setTimeout(function () { label.textContent = orig; delete btn.dataset.copied; }, 1600);
    }
  });
  ...
  document.querySelectorAll("[data-helpful]").forEach(function (form) {
    ...
    form.querySelectorAll("button[data-response]").forEach(function (b) {
      b.addEventListener("click", function () {
        if (question) { question.style.opacity = "0"; question.style.pointerEvents = "none"; }
  ...
  function setupSidebar(nav) {
    ...
    links.forEach(function (a) {
      ...
      if (href.indexOf("#") === -1) {
        if (hp === path) {
          ...
          a.classList.remove("text-zinc-600", "hover:text-zinc-900", "dark:text-zinc-400", "dark:hover:text-white");
          activeTop = a;
        }
      }
    });
    ...
    var subList = activeTop ? activeTop.parentElement.querySelector("ul") : null;
    if (subList) subList.style.opacity = "1";

    // scroll spy over headings referenced by sub-links / TOC anchors
    var hashLinks = links.filter(function (a) { return a.getAttribute("href").indexOf("#") !== -1; });
    var spy = [];
    ...
    function moveHighlight(target) {
      ...
      if (bar) { bar.style.top = top + "px"; bar.style.height = r.height + "px"; bar.style.opacity = "1"; }
      if (visGroup) {
    ...
    function update() {
      ...
      if (spy.length) {
        ...
        spy.forEach(function (s) {
          ...
          if (t - probe <= 0 && t > bestTop) { bestTop = t; best = s; }
        });
        spy.forEach(function (s) {
