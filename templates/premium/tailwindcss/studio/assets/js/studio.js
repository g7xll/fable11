...
(function () {
  ...
  function initNav() {
    var toggle = document.querySelector('button[aria-controls][aria-label="Toggle navigation"]');
    if (!toggle) return;
    ...
    var closeHTML = '<path d="M5.636 4.223 4.222 5.637 10.586 12l-6.364 6.363 1.414 1.415L12 13.414l6.364 6.364 1.414-1.415L13.414 12l6.364-6.363-1.414-1.414L12 10.586 5.636 4.223Z"></path>';
    ...
    function setClosed() {
      ...
      window.setTimeout(function () {
        if (!open) { panel.setAttribute("inert", ""); panel.setAttribute("aria-hidden", "true"); }
      }, 520);
    }

    // The X-close button(s) inside the panel.
    panel.querySelectorAll('button[aria-label="Toggle navigation"]').forEach(function (b) {
      if (b === toggle) return;
    ...
    toggle.addEventListener("click", function () { open ? setClosed() : setOpen(); });
    window.addEventListener("resize", function () { if (open) panel.style.height = "auto"; });
    document.addEventListener("keydown", function (e) { if (e.key === "Escape" && open) setClosed(); });
  }

  function initFadeIn() {
    var nodes = Array.prototype.slice.call(
      document.querySelectorAll('[style*="opacity:0"][style*="translateY"], [style*="opacity: 0"][style*="translateY"]')
    );
    if (!nodes.length) return;
    var reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    nodes.forEach(function (el) {
      el.style.transition = reduce ? "none" : "opacity .7s cubic-bezier(.16,1,.3,1), transform .7s cubic-bezier(.16,1,.3,1)";
      el.style.willChange = "opacity, transform";
    });
    function reveal(el) { el.style.opacity = "1"; el.style.transform = "none"; }
    if (reduce || !("IntersectionObserver" in window)) { nodes.forEach(reveal); return; }
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (!entry.isIntersecting) return;
        var el = entry.target;
        var sibs = Array.prototype.slice.call(el.parentNode ? el.parentNode.children : [el]).filter(function (c) { return nodes.indexOf(c) !== -1; });
        var idx = Math.max(0, sibs.indexOf(el));
        window.setTimeout(function () { reveal(el); }, Math.min(idx, 8) * 90);
        io.unobserve(el);
      });
    ...
    window.setTimeout(function () {
      nodes.forEach(function (el) { if (el.style.opacity === "0") reveal(el); });
    }, 1600);
  }

  function init() { initNav(); initFadeIn(); }
