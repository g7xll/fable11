...
(function () {
  ...
  const playSVG = '<svg viewBox="0 0 36 36" aria-hidden="true"><path d="M33.75 16.701C34.75 17.2783 34.75 18.7217 33.75 19.299L11.25 32.2894C10.25 32.8668 9 32.1451 9 30.9904L9 5.00962C9 3.85491 10.25 3.13323 11.25 3.71058L33.75 16.701Z"></path></svg>';
  const pauseSVG = '<svg viewBox="0 0 36 36" aria-hidden="true"><path d="M8.5 4C7.67157 4 7 4.67157 7 5.5V30.5C7 31.3284 7.67157 32 8.5 32H11.5C12.3284 32 13 31.3284 13 30.5V5.5C13 4.67157 12.3284 4 11.5 4H8.5ZM24.5 4C23.6716 4 23 4.67157 23 5.5V30.5C23 31.3284 23.6716 32 24.5 32H27.5C28.3284 32 29 31.3284 29 30.5V5.5C29 4.67157 28.3284 4 27.5 4H24.5Z"></path></svg>';

  // Build a silent WAV data URI so the player has a real duration without remote assets.
  function silentWav(seconds) {
    const sr = 8000, n = sr * seconds, bytes = 44 + n * 2;
  ...
  function setPlayIcons(playing) {
    ...
    [ppMain, ppMobile].forEach((b) => { if (b) { b.innerHTML = svg; b.setAttribute('aria-label', lbl); } });
  }
  function render() {
  ...
  document.querySelectorAll('[data-play]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (currentId === id && !audio.paused) { audio.pause(); }
      else { startEpisode(id, title); }
    });
  });
  ...
  rewind && rewind.addEventListener('click', () => { audio.currentTime = Math.max(0, audio.currentTime - 10); render(); });
  forward && forward.addEventListener('click', () => { audio.currentTime = Math.min((audio.duration || 64), audio.currentTime + 10); render(); });
  ...
  let dragging = false;
  slider.addEventListener('mousedown', (e) => { dragging = true; seekFromEvent(e); });
