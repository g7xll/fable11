...
(() => {
  ...
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(drawNet);
  ...
  stage && stage.addEventListener('mousemove', (e) => {
    ...
    raf = requestAnimationFrame(() => {
      ...
      if (cen) cen.style.transform = `translate(calc(-50% + ${(nx * 10).toFixed(1)}px), calc(-50% + ${(ny * 10).toFixed(1)}px))`;
      drawNet();
      raf = null;
    });
  });
  stage && stage.addEventListener('mouseleave', () => {
    stage.querySelectorAll('.satellite').forEach((s) => { s.style.transform = ''; });
    const cen = stage.querySelector('.central');
  ...
  sw && sw.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(); } });
