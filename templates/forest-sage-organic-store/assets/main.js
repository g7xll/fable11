...
(function () {
  ...
  // ---- marquee ----
  var phrase = '';
  var words = ['Fresh Produce', 'Seasonal', 'Farm Direct', 'Zero Waste', 'Regenerative'];
  words.forEach(function (w) { phrase += w + ' <span class="star">✦</span> '; });
  ...
  products.forEach(function (p, i) {
    ...
    el.innerHTML =
      '<div class="media">' + p.art +
      '<div class="ov"><button type="button">Quick Add</button></div></div>' +
      '<div class="info"><h3>' + p.name + '</h3><span class="price">' + p.price + '</span></div>';
    grid.appendChild(el);
  });
