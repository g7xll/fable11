...
window.PRODUCE = (function () {
  ...
  const citrus = card(['#f0b429', '#d98324'], { id: 'cit', body: `
    <g transform="translate(200 250)">
      ...
      <g fill="#ffe9b0" stroke="#f0b429" stroke-width="3">
        ${Array.from({length:12},(_,i)=>{const a=i*30*Math.PI/180;const x=Math.cos(a),y=Math.sin(a);
          return `<path d="M0 0 L${x*118-y*30} ${y*118+x*30} A118 118 0 0 0 ${x*118+y*30} ${y*118-x*30} Z" fill="#ffdf8a"/>`;}).join('')}
      </g>
      <circle r="30" fill="#fff3cf"/>
    </g>` });

  // ---- leafy green / herbs ----
  const herbs = card(['#7fa356', '#3f6b2e'], { id: 'hrb', body: `
    <g transform="translate(200 250)" stroke="#2f5722" stroke-width="6" fill="#83a95f">
      ${Array.from({length:9},(_,i)=>{const a=(-120+i*30)*Math.PI/180;
        ...
        <line x1="0" y1="0" x2="0" y2="-200"/></g>`;}).join('')}
      <circle r="20" fill="#3f6b2e" stroke="none"/>
    </g>` });

  // ---- berries ----
  const berries = card(['#8e4b6e', '#5a2945'], { id: 'ber', body: `
    <g transform="translate(200 250)">
      ${[[ -70,-40,70,'#7a2d52'],[60,-60,60,'#923a63'],[ -10,40,80,'#6b2547'],[90,40,52,'#a14573'],[-90,60,48,'#823255']]
        .map(([x,y,r,c])=>`<circle cx="${x}" cy="${y}" r="${r}" fill="${c}"/>
          <circle cx="${x-r*0.3}" cy="${y-r*0.35}" r="${r*0.25}" fill="rgba(255,255,255,0.25)"/>
          <circle cx="${x}" cy="${y-r*0.85}" r="${r*0.14}" fill="#3aa35a"/>`).join('')}
    </g>` });

  // ---- tomato ----
  const tomato = card(['#e3573c', '#a32d1c'], { id: 'tom', body: `
  ...
  // ---- avocado ----
  const avocado = card(['#a7c267', '#5f7d33'], { id: 'avo', body: `
    <g transform="translate(200 250)">
  ...
  // ---- bread / grain loaf ----
  const grain = card(['#e7c48a', '#bd8b4e'], { id: 'grn', body: `
