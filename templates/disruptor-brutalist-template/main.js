...
(() => {
	...
	const PROOF = [
		{
			name: "Maya Cole",
			role: "CTO // VELOCITY",
			q: "We cut our deploy time by 80%. Disruptor is unfair.",
			rot: -2,
			av: "#ccff00",
		},
		{
			name: "Jonas Reed",
			role: "ENG LEAD // FORGE",
			q: "Heavy borders, zero bloat. Exactly how we ship.",
			rot: 2,
			av: "#ffffff",
		},
		{
			name: "Lena Park",
			role: "FOUNDER // NORTHBYTE",
			q: "Brutalist by design, fast by default. Shipped in days.",
			rot: -2,
			av: "#ccff00",
		},
		{
			name: "Theo Mensah",
			role: "VP ENG // PULSE",
			q: "The most opinionated stack we've ever loved.",
			rot: 2,
			av: "#ffffff",
		},
	];
	...
	const STEPS = [
		{
			n: "01",
			title: "Design",
			body: "Lay the blueprint. Strict tokens, heavy borders, zero ambiguity. Every pixel has a job.",
		},
		{
			n: "02",
			title: "Build",
			body: "Assemble the machine. Lean core, no dead weight. Components that snap together hard.",
		},
		{
			n: "03",
			title: "Ship",
			body: "Push it live, loud. Solid shadows, high contrast, undeniable presence. No second-guessing.",
		},
	];
	...
	target.setHours(
		target.getHours() + 8,
		target.getMinutes() + 42,
		target.getSeconds() + 15,
		0,
	);
