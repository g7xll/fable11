(() => {
  ...
  /* ---------- Data ---------- */
  const projects = [
    { n: '01', title: 'Monolith',  cat: 'Brand Identity',     year: '2024', img: 'assets/img/project-1.jpg' },
    { n: '02', title: 'Aether',    cat: 'Digital Product',    year: '2024', img: 'assets/img/project-2.jpg' },
    { n: '03', title: 'Prisma',    cat: 'Art Direction',      year: '2023', img: 'assets/img/project-3.jpg' },
    { n: '04', title: 'Nocturne',  cat: 'Editorial',          year: '2023', img: 'assets/img/project-4.jpg' },
    { n: '05', title: 'Lumen',     cat: 'Creative Direction', year: '2022', img: 'assets/img/project-5.jpg' },
    { n: '06', title: 'Vanta',     cat: 'Web Design',         year: '2022', img: 'assets/img/project-6.jpg' },
  ];
  ...
  const groupHTML = marquee.concat(marquee).map(m => `
    <figure class="mq-card mq-card--${m.shape}">
      <img src="${m.img}" alt="" loading="lazy" />
      <figcaption class="mq-card__tag">${m.tag}</figcaption>
    </figure>`).join('');
  document.querySelectorAll('.marquee__group').forEach(g => { g.innerHTML = groupHTML; });
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    document.querySelectorAll('.hero__title .char').forEach(c => { c.style.transform = 'translateY(0)'; });
  ...
  document.querySelectorAll('.card, .intro .reveal').forEach(el => io.observe(el));
  ...
  if (fine && !window.matchMedia('(prefers-reduced-motion:reduce)').matches) {
    let mx = window.innerWidth / 2, my = window.innerHeight / 2, cx = mx, cy = my, seen = false;
    window.addEventListener('mousemove', (e) => {
      mx = e.clientX; my = e.clientY;
      if (!seen) { seen = true; cx = mx; cy = my; cursor.classList.add('is-visible'); }
    });
