...
const checks = {
  navGrid: await page.$eval('nav', (el) => getComputedStyle(el).gridTemplateColumns),
  ...
  heroCardRadius: await page.$eval('.hero-card', (el) => getComputedStyle(el).borderRadius),
  heroCardBg: await page.$eval('.hero-card', (el) => getComputedStyle(el).backgroundColor),
  heroGridMask: await page.$eval(
    '.hero-grid',
    (el) => getComputedStyle(el).maskImage || getComputedStyle(el).webkitMaskImage,
  ),
  navLinks: await page.$$eval('.nav-links a', (els) => els.map((a) => a.textContent)),
  ...
  headingWeight: await page.$eval('.hero-heading', (el) => getComputedStyle(el).fontWeight),
...
for (let i = 0; i < 5; i++) {
  gradientSamples.push(await page.$eval('#beam-gradient', (el) => el.getAttribute('x1')));
    ...
    new Promise((resolve) => {
      ...
      const iv = setInterval(() => {
        ...
        if (core instanceof SVGPathElement) {
          ...
          if (seen.beamHidden && core.style.opacity === '1') seen.beamRestored = true;
        }
      }, 30);
      setTimeout(() => {
        clearInterval(iv);
...
mobile.nodeSize = await page.$eval('.icon-node', (el) => getComputedStyle(el).width);
mobile.centerSize = await page.$eval('.icon-node-center', (el) => getComputedStyle(el).width);
mobile.beamDRecomputed = await page.$eval('.beam-svg path', (el) => el.getAttribute('d'));
await page.screenshot({ path: `${SHOT_DIR}/mobile.png`, fullPage: true });

await page.click('.menu-toggle');
await page.waitForTimeout(550);
mobile.menuActive = await page.$eval('.nav-menu', (el) => el.classList.contains('active'));
mobile.menuRight = await page.$eval('.nav-menu', (el) => getComputedStyle(el).right);
mobile.toggleActive = await page.$eval('.menu-toggle', (el) => el.classList.contains('active'));
mobile.bodyOverflow = await page.evaluate(() => document.body.style.overflow);
await page.screenshot({ path: `${SHOT_DIR}/mobile-menu.png` });

await page.click('.menu-toggle');
await page.waitForTimeout(550);
mobile.menuClosed = await page.$eval('.nav-menu', (el) => !el.classList.contains('active'));
mobile.bodyOverflowRestored = await page.evaluate(() => document.body.style.overflow === '');

await browser.close();

const report = { title, checks, gradientSamples, observed, mobile, consoleErrors: errors };
console.log(JSON.stringify(report, null, 2));

const failures = [];
if (!/^M -?[\d.]+,-?[\d.]+ L -?[\d.]+,-?[\d.]+ L -?[\d.]+,-?[\d.]+$/.test(checks.beamD ?? '')) {
  failures.push(`beam path d malformed: ${checks.beamD}`);
}
if (new Set(gradientSamples).size < 2) failures.push('gradient x1 never changed');
for (const [k, v] of Object.entries(observed)) if (!v) failures.push(`state machine: ${k} never observed`);
if (checks.brandCount !== 5) failures.push(`expected 5 brands, got ${checks.brandCount}`);
if (checks.navGrid === 'none') failures.push('nav is not a grid on desktop');
if (!checks.bodyFont.includes('Inter')) failures.push(`body font is ${checks.bodyFont}`);
if (!mobile.toggleVisible) failures.push('hamburger not visible on mobile');
if (!mobile.menuActive || mobile.menuRight !== '0px') failures.push('mobile menu did not slide in');
if (mobile.bodyOverflow !== 'hidden') failures.push('body overflow not locked while menu open');
if (!mobile.menuClosed || !mobile.bodyOverflowRestored) failures.push('mobile menu did not close cleanly');
if (mobile.beamDRecomputed === checks.beamD) failures.push('beam path not recomputed on resize');
