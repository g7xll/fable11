...
try {
  ...
  check("Video fixed at z-index 0", video?.position === "fixed" && video?.zIndex === "0");
  check("Video object-cover", video?.objectFit === "cover");
  check("Video covers full viewport", video?.coversViewport === true);
  check("Video src is the CloudFront mp4", /d8j0ntlcm91z4\.cloudfront\.net.+\.mp4/.test(video?.src ?? ""));
  ...
  check(
    ...
    /linear-gradient\(to top,\s*(black|rgb\(0, 0, 0\)) 0%,\s*transparent 45%\)/.test(veil?.maskImage ?? "") ||
      /linear-gradient\(rgba\(0, 0, 0, 0\) 55%,\s*rgb\(0, 0, 0\)\)/.test(veil?.maskImage ?? ""),
    veil?.maskImage,
  );
  check("Overlay pointer-events none", veil?.pointerEvents === "none");
  check("Overlay fixed at z-index 1", veil?.position === "fixed" && veil?.zIndex === "1");
  check(
  ...
  const glass = await page.evaluate(() => {
    const el = [...document.querySelectorAll("header .liquid-glass")].find((b) =>
  ...
  check("Glass background rgba(255,255,255,0.01)", glass?.backgroundColor === "rgba(255, 255, 255, 0.01)");
  check("Glass blend-mode luminosity", glass?.blendMode === "luminosity");
  check("Glass backdrop blur(4px)", glass?.backdropFilter?.includes("blur(4px)"));
  check("Glass inset highlight shadow", glass?.boxShadow?.includes("inset"));
  check(
    "Glass relative + overflow hidden + no border",
    glass?.position === "relative" && glass?.overflow === "hidden" && glass?.borderStyle === "none",
  );
  check("Glass pill is rounded-full", parseFloat(glass?.borderRadius ?? "0") > 100);
  check(
  ...
  check("Liquid glass reused across buttons", glassCount >= 5, `${glassCount} elements`);
  ...
  const anim = await page.evaluate(() => {
    ...
    const user = document.querySelector('header button[aria-label="Your profile"]');
  ...
  check(
    ...
    JSON.stringify(anim.delays.nav) === JSON.stringify(["0.1s", "0.15s", "0.2s", "0.25s", "0.3s"]),
    anim.delays.nav.join(", "),
  );
  check("Search 350ms / User 400ms", anim.delays.search === "0.35s" && anim.delays.user === "0.4s");
  check("Metadata 300ms", anim.delays.meta === "0.3s");
  check("Title 400ms / Desc 500ms", anim.delays.title === "0.4s" && anim.delays.desc === "0.5s");
  check(
  ...
  check("Stagger applied broadly", anim.animatedCount >= 13, `${anim.animatedCount} animated elements`);
  ...
  const hero = await page.evaluate(() => {
    ...
    return {
      ...
      desc: document.querySelector("main p").textContent.trim().replace(/\s+/g, " "),
  ...
  check(
    ...
    hero.titleSizePx === 72 && Math.abs(parseFloat(hero.titleTracking) - 72 * -0.04) < 0.1,
    `${hero.titleSizePx}px / ${hero.titleTracking}`,
  );
  check("Title font-normal", hero.titleWeight === "400");
  check(
    "Description text + gray-400",
    hero.desc === "A voyage through forgotten realms, where past and future intertwine." &&
  ...
  check(
    ...
    hero.watchNow.bg === "rgb(255, 255, 255)" && hero.watchNow.color === "rgb(0, 0, 0)",
  );
  check("Navbar z-50 above hero z-10", hero.headerZ === "50" && hero.mainZ === "10");
  ...
  const desktop = await page.evaluate(() => {
    ...
    const hamburger = document.querySelector('header button[aria-controls="mobile-menu"]');
    return {
      navLinks,
  ...
  check(
    ...
    JSON.stringify(desktop.navLinks) ===
      JSON.stringify(["Movies", "TV Series", "Editor's Pick", "Interviews", "User Reviews"]),
    desktop.navLinks.join(", "),
  );
  check("Nav visible on lg, hamburger hidden", desktop.navVisible && desktop.hamburgerHidden);
  ...
  const mobileOpen = await page.evaluate(() => {
    ...
    const settled = (s) =>
      ...
      (s.translate === "none" || s.translate === "" || /^0px(?: 0px)?$/.test(s.translate));
    return {
      opacity: cs.opacity,
      settled: settled(cs),
      ...
      bottomSearchVisible: visible(
        [...menu.querySelectorAll("button")].find((b) => b.textContent.includes("Search")),
      ),
      hamburgerExpanded:
        document
  ...
  check("Menu at top-[72px], z-40", mobileOpen.top === 72 && mobileOpen.zIndex === "40");
  check(
    "Menu styling (gray-900/95 + backdrop blur)",
    mobileOpen.bg === "rgba(16, 24, 40, 0.95)" || mobileOpen.bg.startsWith("oklab") || mobileOpen.bg.startsWith("color("),
    mobileOpen.bg,
  );
  check("Menu has 5 staggered links, all settled in", mobileOpen.linkCount === 5 && mobileOpen.linksSettled);
  check("Below sm: Search shown inside menu footer", mobileOpen.bottomSearchVisible);
  ...
  check("No console/page errors", realErrors.length === 0, realErrors.join(" | ").slice(0, 200));
