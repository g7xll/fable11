...
const before = await page.evaluate(() => document.querySelector('.fixed.inset-x-0.bottom-0')?.innerHTML?.length || 0);
const listen = await page.$('button[aria-label^="Play episode"]');
if (listen) {
  await listen.click();
  await page.waitForTimeout(1500);
  await page.screenshot({ path: path.join(OUT, "audio-player-active.png"), fullPage: false });
  ...
  interactions.push({ name: "play-episode", trigger: 'button[aria-label^="Play episode"]', delta: `audio player div populated (innerHTML ${before} -> populated), fixed bottom bar appears`, screenshot: "audio-player-active.png" });
}
