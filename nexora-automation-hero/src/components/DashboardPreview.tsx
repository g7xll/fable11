...
const QUICK_ACTIONS = ['Request', 'Transfer', 'Deposit', 'Pay Bill', 'Create Invoice']
...
const TRANSACTIONS: Transaction[] = [
  ...
  { date: 'Jun 09', description: 'Client Payment', amount: '+$125,000', positive: true, status: 'Completed' },
  { date: 'Jun 08', description: 'Payroll', amount: '-$85,450', status: 'Completed' },
  { date: 'Jun 06', description: 'Office Supplies', amount: '-$1,200', status: 'Completed' },
]
    ...
    <div className="min-w-0 flex-1 basis-0 overflow-hidden rounded-lg border border-border bg-background p-3">
      ...
      <div className="mt-1">
        {ACCOUNTS.map(([name, amount]) => (
          <div key={name} className="flex items-center justify-between gap-2 py-3 text-xs">
            <span className="text-muted-foreground">{name}</span>
            <span className="whitespace-nowrap font-medium text-foreground">{amount}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        <tbody>
          {TRANSACTIONS.map((tx) => (
            <tr key={tx.description}>
              ...
              <td className="py-1.5 font-medium text-foreground">{tx.description}</td>
              <td
