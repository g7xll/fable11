...
const subtitle = await page.getByText("Create apps and websites by chatting with AI").count();
check("Subtitle present", subtitle > 0);

const pill = await page.getByText("Better SEO – Apps built to be found").count();
...
for (const label of ["Solution", "Resources", "Community", "Enterprise", "Pricing", "Security"]) {
...
check("window.CoreRenderer is an object", webglInfo.coreRenderer === "object" || webglInfo.coreRenderer === "function");
check("window._heroProjectData loaded", webglInfo.heroData === "object");
check("Canvas has a live WebGL context", webglInfo.hasGL === true);
check("Canvas has non-zero dimensions", webglInfo.hasCanvas && webglInfo.w > 0 && webglInfo.h > 0);

// 4. Hero shader faded in (opacity 1)
const heroOpacity = await heroCanvas.evaluate((el) => getComputedStyle(el).opacity);
check("Hero shader faded in (opacity ~1)", parseFloat(heroOpacity) > 0.9);

// 5. Trusted-by section
const trusted = await page.getByText("Teams from top companies build with Lovable").count();
...
const promptBox = await page.evaluate(() => {
  ...
  const box = els.find((d) => {
    ...
    return Math.round(parseFloat(s.height)) === 160 && Math.round(parseFloat(s.borderTopLeftRadius)) === 30;
  });
  if (!box) return null;
...
const assetStatuses = await page.evaluate(async () => {
  const files = [
    "core-renderer.js", "hero-project.js", "back-gl-3.png", "back-gl.png", "back-gl-2.png",
    "logo.svg", "arrow-right.svg", "arrow-up.svg", "ai-select.svg", "dots.svg", "announcement.jpg",
    "logo-1.svg", "logo-2.svg", "logo-3.svg", "logo-4.svg", "logo-5.svg", "logo-6.svg",
  ];
  const out = {};
