...
const Explorations = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const parallax = (target: HTMLDivElement | null, from: number, to: number) =>
        gsap.fromTo(
          target,
          { y: from },
          {
    ...
    <section ref={sectionRef} className="relative min-h-[300vh] bg-bg">
      ...
      <div className="pointer-events-none absolute inset-0 z-20 overflow-hidden">
        <div className="mx-auto grid h-full max-w-[1400px] grid-cols-2 gap-12 px-6 md:gap-40 md:px-12">
          <div ref={colOneRef} className="flex flex-col items-end gap-20 pt-[35vh] md:gap-32">
            {COLUMN_ONE.map((item) => (
              <GalleryCard key={item.title} item={item} onOpen={setLightbox} />
            ))}
          </div>
          <div ref={colTwoRef} className="flex flex-col items-start gap-20 pt-[70vh] md:gap-32">
            {COLUMN_TWO.map((item) => (
              <GalleryCard key={item.title} item={item} onOpen={setLightbox} />
            ))}
          </div>
