...
try {
  ...
  check(/^\d{3}$/.test((await counter.textContent())?.trim() ?? ""), "counter shows zero-padded 3 digits");
  ...
  const heroVideoState = await page
    .locator("#home video")
    .evaluate((v) => ({ src: v.currentSrc || v.src, readyState: v.readyState }));
  check(
  ...
  check(
    await page.getByText("COLLECTION '26").isVisible(),
    "eyebrow visible",
  );
  check(
  ...
  check(
    await page.locator("#work h2").textContent().then((t) => t?.includes("Featured")),
  ...
  await page.waitForTimeout(2200);
  const statTexts = await page.locator("#resume .tabular-nums").allTextContents();
  check(
  ...
  check(
    (await page
      .locator("a[href='mailto:hello@michaelsmith.com']")
      .count()) >= 1,
    "mailto CTA present",
  );
  check(
  ...
  const hasActiveClass = (el) => el.className.split(/\s+/).includes("bg-stroke/50");
  ...
  check(realErrors.length === 0, `no console/page errors (${realErrors.length} found)`);
