...
try {
  ...
  check("vignette overlay present", (await page.locator('[data-testid="vignette"]').count()) === 1);
  check("film grain overlay present", (await page.locator('[data-testid="grain"]').count()) === 1);
  check(
  ...
  check('title reads "Darkness, Composed."', h1Text === "Darkness, Composed.", h1Text);
  ...
  check("accent line uses Cormorant italic", emFont.includes("Cormorant"), emFont);
  ...
  check("nav wordmark present", (await page.getByLabel("Noctis — home").count()) === 1);
  check(
    "both CTAs present",
  ...
  check(
    ...
    h1LineTransform === "none" || h1LineTransform === "matrix(1, 0, 0, 1, 0, 0)",
    h1LineTransform,
  );
  ...
  check(
    ...
    mobileVideoBox && mobileVideoBox.width >= 390 && mobileVideoBox.height >= 844,
    mobileVideoBox ? `${mobileVideoBox.width}x${mobileVideoBox.height}` : "no box",
  );
  ...
  const appErrors = consoleErrors.filter(
    (e) => !e.includes("res.cloudinary.com") && !e.toLowerCase().includes("failed to load resource"),
  );
  check("no application console errors", appErrors.length === 0, appErrors.join(" | "));
