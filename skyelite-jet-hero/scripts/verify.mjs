...
/* ---------------- Desktop ---------------- */
{
  ...
  check('brand "SkyElite" visible', await page.getByText('SkyElite').isVisible());
  for (const item of ['Start', 'Story', 'Rates', 'Benefits', 'FAQ']) {
    check(`desktop nav link "${item}" visible`, await page.locator(`nav >> text=${item}`).isVisible());
  }
  check(
  ...
  check('"Accessible." color is #202A36', a.color === 'rgb(32, 42, 54)', a.color);
  check('"Accessible." overlap margin-top -12px', a.marginTop === '-12px', a.marginTop);
  ...
  const dBg = await discover.evaluate((el) => getComputedStyle(el).backgroundColor);
  check('"Discover" bg gray-300', dBg === 'rgb(209, 213, 219)', dBg);

  await bookNow.hover();
  await page.waitForTimeout(350);
  const hoverBg = await bookNow.evaluate((el) => getComputedStyle(el).backgroundColor);
  ...
  check('hero content pulled up by -mt-80 (-320px)', contentMt === '-320px', contentMt);
...
/* ---------------- Mobile ---------------- */
{
  ...
  check('base text-6xl applied on mobile', mobileHeadingSize === '60px', mobileHeadingSize);

  await page.screenshot({ path: '/tmp/skyelite-mobile-menu.png' });

  await page.getByRole('button', { name: 'Close menu' }).click();
  await dropdown.waitFor({ state: 'detached' });
  check('dropdown closes via X', (await page.locator('#mobile-menu').count()) === 0);
