...
try {
	...
	check(
		"GodRays canvas + WebGL context present",
		heroCanvas.ok,
		heroCanvas.ok ? heroCanvas.ver : heroCanvas.reason,
	);
	check(
		"GodRays canvas has backing pixels",
		heroCanvas.ok && heroCanvas.w > 0 && heroCanvas.h > 0,
		`${heroCanvas.w}x${heroCanvas.h}`,
	);
	...
	check(
		"GodRays paints light (bloom visible)",
		paint.lit > 20 && paint.maxL > 40,
	...
	check(
		"demo form renders ('Get a Demo')",
		(await page.locator("text=Get a Demo").count()) > 0,
	);
	check(
	...
	check(
		"MeshGradient canvas mounts in modal",
		canvasesAfter > canvasesBefore,
		`${canvasesBefore} -> ${canvasesAfter}`,
	);
