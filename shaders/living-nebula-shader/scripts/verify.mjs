...
check(
	"page title",
	(await page.title()).startsWith("Living Nebula"),
	await page.title(),
);
check(
...
check(
	...
	(await page.locator(".overlay-content").count()) === 1,
);
...
check(
	"shader paints lit pixels (not black)",
	lit > samples * 0.2,
	`${lit}/${samples} lit`,
);
check(
	"shader renders nebula gas color",
	colored > 4,
	`${colored}/${samples} colored`,
);
...
check(
	"feed is animated / cursor-reactive (frames differ)",
	diff > 500,
	`${diff} differing px`,
);
...
check(
	"feed clock formatted T+HH:MM:SS",
	/^T\+\d{2}:\d{2}:\d{2}$/.test(clockA),
	clockA,
);
...
const fps = await page
	.locator(".telemetry__cell")
	.nth(1)
	.locator("dd")
	.innerText();
check(
	"render rate reported (non-zero)",
	/\d/.test(fps) && !/^0\s*fps/.test(fps),
	fps,
);
...
check(
	"warp center is live coordinates",
...
check(
	"cursor reticle present",
	(await page.locator(".cursor-aura").count()) === 1,
);
check(
	"reticle has 4 ticks",
	(await page.locator(".cursor-aura__tick").count()) === 4,
);

// ── Console framing ────────────────────────────────────────────────────────
check(
	"console brackets present (4 corners)",
	(await page.locator(".console__bracket").count()) === 4,
);
