...
function Cell({
	...
	return (
		<div className="flex flex-col gap-0.5">
			<dt className="font-mono text-[10px] uppercase tracking-[0.2em] text-ridge-dim">
				{label}
			</dt>
			<dd className="tnum font-mono text-[15px] leading-none text-haze">
				{value}
				{sub ? (
					<span className="ml-1 text-[10px] text-ridge-dim">{sub}</span>
				) : null}
			</dd>
		</div>
	);
...
export default function App() {
	...
	const [tab, setTab] =
		useState<(typeof SOURCE_TABS)[number]["id"]>("component");
			...
			<main className="relative z-10 mx-auto max-w-[1240px] px-5 pb-24 sm:px-8">
				<div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
					<div className="animate-fade-up">
						...
						<h1 className="font-display text-[40px] font-medium leading-[0.98] tracking-tightest text-haze sm:text-[56px]">
							...
							<br />
							as an{" "}
							<span className="italic text-ridge-bright ridge-split">
								instrument
							</span>
							.
						</h1>
					...
					<p className="max-w-sm text-sm leading-relaxed text-haze/60 animate-fade-up">
						A drop-in{" "}
						<code className="rounded bg-fog-850 px-1.5 py-0.5 font-mono text-[12px] text-signal">
							@/components/ui
						</code>{" "}
						shader: a single{" "}
						<code className="rounded bg-fog-850 px-1.5 py-0.5 font-mono text-[12px] text-haze/80">
							RawShaderMaterial
						</code>{" "}
						plane folded to face you and pushed forever forward through 3D
						Perlin noise — the far ridges dissolving into haze.
					</p>
				</div>
					...
					>
						...
						<div className="absolute inset-x-0 bottom-0 z-20 border-t border-fog-line/70 bg-fog-975/80 backdrop-blur-sm">
							<dl className="grid grid-cols-2 gap-x-4 gap-y-2 px-4 py-2.5 sm:grid-cols-5">
								...
								<Cell
									label="Cam depth"
									value={hud.cameraZ.toFixed(0)}
									sub="z"
								/>
								<Cell
									label="Visibility"
									value={(visibility * 10).toFixed(1)}
									sub="km"
								/>
								<Cell
									label="Ridge crest"
									value={`${Math.round(crest * 100)}`}
									sub="%"
								/>
							</dl>
						</div>
					...
					<div className="mt-4 grid gap-4 lg:grid-cols-[300px_1fr]">
						...
						<div className="relative overflow-hidden rounded-2xl border border-fog-line bg-fog-900/70 p-5">
							...
							<p className="mt-3 border-t border-fog-line/60 pt-3 text-[11px] leading-relaxed text-haze/45">
								...
								vertex by distance, so the lit fraction <em>is</em> the
								visibility.
							</p>
						</div>
						...
						<div className="relative overflow-hidden rounded-2xl border border-fog-line bg-fog-900/70 p-5">
							<div className="mb-4 flex items-center justify-between">
								...
								>
									{paused ? (
										<Play className="h-3 w-3 text-signal" />
									) : (
								...
								<div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
									{PRESETS.map((p) => (
										...
										>
											...
											<span className="mt-0.5 block text-[10px] leading-tight text-ridge-dim">
												{p.blurb}
											</span>
										</button>
									))}
				...
				<section className="mt-20">
					<SectionHeading
						...
						kicker="How it renders"
						title="Anatomy of the fog-bank"
					/>
					<div className="mt-6 grid gap-3 sm:grid-cols-3">
						{[
							...
							<div
								key={c.step}
								className="relative overflow-hidden rounded-xl border border-fog-line bg-fog-900/60 p-5"
							>
								<span className="font-mono text-[11px] tracking-[0.2em] text-signal/70">
									{c.step}
								</span>
								<h3 className="mt-2 font-display text-lg font-medium text-haze">
									{c.head}
								</h3>
								<p className="mt-1.5 text-[13px] leading-relaxed text-haze/55">
									{c.body}
								</p>
							</div>
						))}
				...
				<section className="mt-20 grid gap-8 lg:grid-cols-2">
					<div>
						<SectionHeading
							...
							kicker="Setup"
							title="Drop it into a shadcn app"
						/>
						<p className="mt-4 max-w-prose text-sm leading-relaxed text-haze/60">
							...
							for shadcn — Tailwind configured, the{" "}
							<code className="rounded bg-fog-850 px-1 font-mono text-[12px] text-signal">
								@/*
							</code>{" "}
							alias in place, and a{" "}
							<code className="rounded bg-fog-850 px-1 font-mono text-[12px] text-haze/80">
								components.json
							</code>
					...
					<div>
						...
						<div className="mt-4 space-y-4 text-sm leading-relaxed text-haze/60">
							<p>
								...
								. The default path here resolves to exactly that folder — so the
								brief's instruction is satisfied as-is.
							</p>
							<p>
								Keeping primitives under{" "}
								<code className="rounded bg-fog-850 px-1 font-mono text-[12px] text-haze/80">
									components/ui
								</code>{" "}
								is what lets the CLI, your editor's path-intellisense, and every
								other generated component resolve{" "}
								<code className="rounded bg-fog-850 px-1 font-mono text-[12px] text-signal">
									@/components/ui/glsl-hills
								</code>{" "}
								without bespoke wiring. It also draws a clean line between
								reusable building blocks and app-specific screens.
							</p>
							<div className="rounded-xl border border-fog-line bg-fog-900/60 p-4">
								<p className="mb-2 font-mono text-[10.5px] uppercase tracking-[0.2em] text-ridge-dim">
									Resolved paths
								</p>
								<ul className="space-y-1.5 font-mono text-[12px] text-haze/70">
									<li>
										<span className="text-signal/70">ui</span> →
										src/components/ui
									</li>
									<li>
										<span className="text-signal/70">utils</span> →
										src/lib/utils
									</li>
									<li>
										<span className="text-signal/70">alias</span> → @/* =
										./src/*
									</li>
								</ul>
							</div>
				...
				<section className="mt-20">
					...
					<p className="mt-4 max-w-prose text-sm leading-relaxed text-haze/60">
						...
						<code className="rounded bg-fog-850 px-1 font-mono text-[12px] text-signal">
							src/components/ui/glsl-hills.tsx
						</code>{" "}
						and use it anywhere. The GLSL is the brief's verbatim — the wrapper
						just adds TypeScript types, container-aware sizing, and the optional
						telemetry hooks.
					</p>
					<div className="mt-5">
						...
						<div className="mt-3">
							{SOURCE_TABS.map((s) =>
								s.id === tab ? (
									<CodeBlock key={s.id} code={s.code} lang={s.lang} />
								) : null,
							)}
						</div>
						<div className="mt-3">
				...
				<section className="mt-20">
							...
							<tbody className="divide-y divide-fog-line/60 font-mono text-haze/75">
								{[
									[
										...
										"string | number",
										"'100vw'",
										"CSS width of the host element",
									],
									[
										"height",
										"string | number",
										"'100vh'",
										"CSS height of the host element",
									],
									[
										"cameraZ",
										"number",
										"125",
										"Camera Z distance from the range",
									],
									[
										"planeSize",
										"number",
										"256",
										"Plane size + segment count (coupled)",
									],
									[
										"speed",
										"number",
										"0.5",
										"Drift multiplier on the per-frame delta",
									],
									...
									[
										...
										"number",
										"256",
										"Opacity-falloff divisor — fog depth",
									],
									...
									[
										...
										"(handles) => void",
										"—",
										"Fires once with uniforms/renderer/camera",
									],
									[
										"onFrame",
										"(state) => void",
										"—",
										"Per-frame { time, fps, cameraZ }",
									],
									...
									<tr key={prop} className="bg-fog-950/40">
										...
										<td className="hidden px-4 py-2.5 text-haze/50 sm:table-cell">
											{notes}
										</td>
									</tr>
								))}
					...
					<p className="mt-3 text-[12px] leading-relaxed text-ridge-dim">
						Note · <span className="text-haze/60">fog</span>,{" "}
						<span className="text-haze/60">haze</span>,{" "}
						<span className="text-haze/60">paused</span> and the two callbacks
						are additive extensions over the brief's five-prop API; the original
						defaults reproduce the source exactly.
					</p>
				</section>
