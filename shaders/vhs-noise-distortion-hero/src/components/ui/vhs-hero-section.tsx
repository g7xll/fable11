...
function DistortionBackground({ mousePosition }: DistortionBackgroundProps) {
	...
	const uniforms = useMemo(
		() => ({
			...
			uResolution: {
				value: [window.innerWidth, window.innerHeight] as [number, number],
			},
  ...
  `;
    ...
    vec3 distortedNoise(vec2 uv) {
      ...
      // Horizontal tracking-error band that sweeps the frame
      float band = smoothstep(0.0, 0.02, abs(uv.y - fract(uTime * 0.13)) ) ;
      f += (1.0 - band) * 0.18;
      ...
      color += mouseEffect * vec3(0.35, 0.08, 0.18);

      float scanline = sin(uv.y * 800.0) * 0.04;
      color += scanline;

      // gentle ambient distortion read so uDistortionStrength stays meaningful
      color += uDistortionStrength * 0.02;
    ...
    void main() {
      ...
      float aberration = 0.004 + length(uv - 0.5) * 0.006;
      vec3 color;
      ...
      // tint deep blacks toward the signature blood-red
      color = mix(color, color * vec3(1.15, 0.55, 0.65), 0.18);
		...
		if (pointsRef.current) {
			...
			for (let i = 0; i < count; i++) {
				...
				const mouseInfluence =
					1 /
					(1 +
						Math.abs(positions[i3] - mousePosition.x * 10) +
						Math.abs(positions[i3 + 1] - mousePosition.y * 10));
...
export default function DistortHero() {
	...
	// Running VHS timecode counter (HH:MM:SS:FF)
	useEffect(() => {
	...
	useEffect(() => {
		...
		const ctx = gsap.context(() => {
			...
			glitchTl
				...
				.to(heroRef.current, { filter: "none", duration: 0.1 })
				.to(heroRef.current, { x: 5, duration: 0.05 })
				.to(heroRef.current, { x: -5, duration: 0.05 })
				.to(heroRef.current, { x: 0, duration: 0.05 });
	...
	return (
		<div className="relative min-h-screen w-full overflow-hidden bg-black">
			...
			<div
				ref={heroRef}
				className="relative z-20 flex min-h-screen items-center justify-center px-6 animate-track-jump"
			>
				<div className="mx-auto max-w-5xl text-center">
					<div className="mb-6 flex items-center justify-center gap-3 text-[0.65rem] uppercase tracking-[0.45em] text-cyan-400/80">
					...
					<div className="mt-4">
						<GlitchText
							...
							glitchIntensity={0.6}
							glitchFrequency={150}
							className="tracking-[0.4em] opacity-80"
						/>
					</div>

					<p
						ref={subtitleRef}
						className="mx-auto mb-12 mt-10 max-w-3xl font-mono text-lg uppercase leading-relaxed tracking-wider text-red-400 md:text-2xl lg:text-3xl"
						style={{
							textShadow:
								"0 0 10px rgba(255, 0, 0, 0.5), 2px 0 #00ffff, -2px 0 #ff0000",
						}}
					>
						{">"} REALITY.CORRUPTED {"<"}
						<br />
					...
					<div ref={buttonRef}>
						<Button
							size="lg"
							className="group relative overflow-hidden border-2 border-red-500 bg-red-600/20 px-10 py-4 font-mono text-lg uppercase tracking-wider text-red-400 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:bg-red-600/40 hover:text-white"
						>
			...
			{/* Glitch UI Elements */}
			<div className="absolute left-8 top-8 z-30 animate-flicker">
			...
			<div className="absolute right-8 top-8 z-30 text-right">
				...
				<div className="mt-1 font-mono text-xs tabular-nums tracking-wider text-white/70">
					{timecode}
				</div>
			</div>

			<div className="absolute bottom-8 left-1/2 z-30 -translate-x-1/2 transform">
				<div className="h-16 w-px animate-pulse bg-gradient-to-b from-red-500/60 to-transparent" />
			</div>

			<div className="absolute bottom-7 left-8 z-30 font-mono text-[0.6rem] uppercase tracking-[0.3em] text-white/40">
				PLAY ▶ · TRACKING AUTO
			</div>
			<div className="absolute bottom-7 right-8 z-30 font-mono text-[0.6rem] uppercase tracking-[0.3em] text-white/40">
				STEREO · Hi-Fi
			</div>
