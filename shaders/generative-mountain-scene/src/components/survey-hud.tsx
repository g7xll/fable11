	...
	return (
		...
		>
			...
			<dl className="grid grid-cols-2 gap-px bg-slate-line/40 sm:grid-cols-3 lg:grid-cols-6">
				...
				<Cell
					label="Light · X"
					icon={<Sun className="h-3 w-3 text-altitude" />}
				>
...
function Cell({
	...
	return (
		<div
			className={`bg-ink-900/85 px-3.5 py-3 ${wide ? "col-span-2 sm:col-span-1" : ""}`}
		>
			<dt className="flex items-center gap-1.5 font-mono text-[9.5px] uppercase tracking-[0.16em] text-haze/40">
				{icon}
				{label}
			</dt>
			<dd className="mt-1 font-mono text-[15px] font-medium leading-none text-haze">
				{children}
			</dd>
		</div>
	);
