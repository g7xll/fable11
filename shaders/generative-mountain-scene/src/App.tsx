			...
			<section className="relative flex min-h-screen flex-col">
				...
				<div className="relative flex flex-1 items-stretch px-4 pb-5 pt-5 sm:px-8 sm:pb-8">
					...
					>
						...
						<div
							className="bg-survey-grid-fine pointer-events-none absolute inset-0 z-10 opacity-60"
							aria-hidden="true"
						/>
						<div
							className="bg-scanlines pointer-events-none absolute inset-0 z-10 opacity-30"
							aria-hidden="true"
						/>
						<div
							className="stage-vignette pointer-events-none absolute inset-0 z-10"
							aria-hidden="true"
						/>
						<div
							...
							<div className="w-full px-6 pb-16 sm:px-12 sm:pb-20">
								<div className="max-w-2xl">
									...
									<p className="mt-4 max-w-xl text-balance text-[15px] leading-relaxed text-haze/70 sm:text-base">
										A solid massif raised in real time — a 128² plane pushed
										along its normal by two octaves of 3D simplex noise, lit by
										a single point light that follows your cursor across the
										ridgeline.
									</p>
								</div>
							</div>
			...
			>
				<div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
					...
					<h2 className="mt-4 max-w-3xl font-display text-3xl font-semibold leading-tight tracking-tightest text-haze sm:text-[2.6rem]">
						One Three.js component. Solid terrain, no gaps, fully
						self-contained.
					</h2>
					<p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-haze/65">
						The brief ships a single React component. It mounts its own
						renderer, owns its animation loop, and tears everything down on
						unmount — so it slots straight into a shadcn/Tailwind/TypeScript
						project under{" "}
						<code className="rounded bg-ink-800 px-1.5 py-0.5 font-mono text-[13px] text-glacier">
							@/components/ui
						</code>
						. The only runtime dependency is{" "}
						<span className="text-haze">three</span>.
					</p>
					...
					<div className="mt-12 grid gap-5 lg:grid-cols-2">
						...
						<Step n={2} title="Why it lives in components/ui" icon={Layers}>
							<p className="text-[14px] leading-relaxed text-haze/65">
								shadcn resolves the{" "}
								<code className="font-mono text-glacier">ui</code> alias to{" "}
								<code className="font-mono text-haze">@/components/ui</code>.
								Dropping{" "}
								<code className="font-mono text-haze">mountain-scene.tsx</code>{" "}
								there means it is importable as{" "}
								<code className="font-mono text-haze">
									@/components/ui/mountain-scene
								</code>{" "}
								everywhere, sits beside the rest of your primitives, and stays
								upgrade-safe. If your project doesn't have that folder yet,
								create it — the alias in{" "}
								<code className="font-mono text-haze">components.json</code>{" "}
								expects it.
							</p>
						</Step>
					</div>
					...
					<div className="mt-20">
							...
							<ApiTable
								...
								rows={[
									[
										"baseColor?",
										"string",
										`Ridge tint → the color uniform. Default "#7dd3fc".`,
									],
									[
										"paused?",
										"boolean",
										"Freezes the time uniform's drift. Default false.",
									],
									[
										"className?",
										"string",
										"Classes for the full-bleed mount div.",
									],
									[
										"onFrame?",
										"(f) => void",
										"Per-frame telemetry callback (see below).",
									],
								]}
							/>
							<ApiTable
								...
								rows={[
									...
									[
										"time",
										"number",
										"Raw value of the time uniform (slow drift).",
									],
						...
						<p className="mt-6 max-w-2xl text-[13.5px] leading-relaxed text-haze/55">
							The live HUD above is wired to{" "}
							<code className="font-mono text-glacier">onFrame</code> — the
							survey clock is the{" "}
							<code className="font-mono text-haze">time</code> uniform, and the
							three Light · X/Y/Z readouts are the exact{" "}
							<code className="font-mono text-haze">pointLightPosition</code>{" "}
							vector the fragment shader uses to light the terrain.
						</p>
					</div>
				</div>
...
function AnatomyCard({
	...
	return (
		<div className="group relative overflow-hidden rounded-xl border border-slate-line bg-ink-900/40 p-5 transition-colors hover:border-glacier/40">
			<div
				className="bg-contour pointer-events-none absolute inset-0 opacity-0 transition-opacity group-hover:opacity-100"
				aria-hidden="true"
			/>
			<div className="relative">
				...
				<p className="mt-2 text-[13.5px] leading-relaxed text-haze/60">
					{body}
				</p>
			</div>
		</div>
...
function SourceTabs() {
	const [active, setActive] =
		useState<(typeof TABS)[number]["id"]>("component");
			...
			<table className="w-full text-left">
				<tbody>
					{rows.map(([name, type, desc]) => (
						<tr
							key={name}
							className="border-b border-slate-line/50 last:border-0"
						>
							...
							<td className="px-4 py-3 text-[13px] leading-relaxed text-haze/60">
								{desc}
							</td>
						</tr>
					))}
