...
/* ---------------------------------------------------------------------------
 * Aether Flow — drop-in WebGL shader component (shadcn `components/ui` piece).
 *
 * This is the component from the integration brief, ported to TypeScript for a
 * strict shadcn/Tailwind/TS project. The Three.js scene, uniforms, vertex /
 * fragment shaders, animation loop, resize + mousemove listeners, and the
 * two-effect update model are all preserved exactly as given. Two purely
 * additive, default-off props are exposed so a host page can observe the live
 * field without changing the original behaviour:
 *   - ShaderCanvas: `className` passthrough + optional `onWarp` callback that
 *     reports the normalized mouse/warp coordinate each pointer move.
 *   - ControlSlider: `className` passthrough on the native range input.
 * ------------------------------------------------------------------------- */
 ...
 * @param mountRef - Ref to the container element.
 * @param shaderProps - An object containing the shader's uniform values.
 * @param onWarp - Optional callback fired on pointer move with the normalized
