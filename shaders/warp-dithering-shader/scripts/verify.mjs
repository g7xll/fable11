...
try {
  ...
  if (max - min > 20) ok(`hero field has tonal range (Δlum ${Math.round(max - min)})`);
  else fail("hero field is flat / uniform");
  if (orange > 0) ok(`hero shows orange foreground over navy (${orange} samples)`);
  else fail("no orange foreground detected — colours may not be reaching the shader");
  ...
  if (/sphere/i.test(after)) ok(`shape control updates wordmark (${before} -> ${after})`);
  ...
  const typePressed = await page
    ...
    .getAttribute("aria-pressed");
  if (typePressed === "true") ok("selected dither kernel reports aria-pressed=true");
