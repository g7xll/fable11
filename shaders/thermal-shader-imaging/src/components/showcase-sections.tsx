	...
	[
		{
			...
			type: "string",
			def: "apple.png",
			note: "Image whose ALPHA channel masks the heat. PNG/SVG with transparency.",
		},
		{
			prop: "width",
			type: "number",
			def: "400",
			note: "Canvas width in px (the bay sizes the renderer from its container).",
		},
		{
			prop: "height",
			type: "number",
			def: "400",
			note: "Canvas height in px.",
		},
		{
			prop: "className",
			type: "string",
			def: '""',
			note: "Extra classes on the flex wrapper.",
		},
	];
...
function Shell({
	...
	return (
		<section
			id={id}
			className={cn("mx-auto w-full max-w-5xl px-6 py-16 sm:py-20", className)}
		>
			{children}
		</section>
	);
...
export function IntegrationSection() {
	return (
		<Shell id="integrate">
			<SectionHeading
				kicker="Drop-in"
				title="Wire it into a shadcn project"
			...
			<p className="mb-8 max-w-2xl text-mist">
				...
				<span className="text-cream">TypeScript</span>. The effect is a single
				self-contained file — no providers, no global state. Three steps:
			</p>
  ...
  from "@/components/ui/thermal-shader"`}</CodeBlock>
			...
			<p className="mt-6 text-sm text-mist/70">
				No shadcn project yet? Scaffold one, then add the primitives this
				expects:
			</p>
		...
		<Shell id="why-ui" className="border-y border-line/60 bg-panel/30">
			...
			<div className="grid gap-8 md:grid-cols-2">
				<div className="space-y-4 text-mist">
					<p>
						shadcn’s default alias resolves{" "}
						<code className="rounded bg-panel2 px-1 text-cyan">
							@/components/ui
						</code>{" "}
						to your source tree, and{" "}
						<code className="rounded bg-panel2 px-1 text-cyan">ui</code> is
						reserved for the unstyled, copy-in primitives you own outright.
						Dropping{" "}
						<code className="rounded bg-panel2 px-1 text-cyan">
							thermal-shader.tsx
						</code>{" "}
						there means its own import — written verbatim in the brief —
					</p>
					<CodeBlock>{`import { ThermalEffect }
  from "@/components/ui/thermal-shader"`}</CodeBlock>
					<p>
						resolves with zero edits, and the CLI can update sibling primitives
						without ever clobbering your custom piece.
					</p>
				</div>
				<ul className="space-y-3 text-sm text-mist">
					{[
						...
						<li
							key={line}
							className="flex gap-3 rounded-lg border border-line bg-panel/50 p-3"
						>
...
export function PaletteSection() {
	return (
		<Shell id="palette">
			<SectionHeading
				kicker="Calibration"
				title="The thermal LUT"
				icon={<Palette className="h-4 w-4" />}
			/>
			<p className="mb-6 max-w-2xl text-mist">
				Seven stops drive the gradient inside the mask — a cold void rising
				through blue and cyan to a cream mid-band, then yellow, gold and a hot
				ember peak. The pointer adds heat; the shader keeps a slow procedural
				...
				<div className="grid grid-cols-2 gap-px bg-line sm:grid-cols-4 lg:grid-cols-7">
					{PALETTE.map((stop) => (
						<div key={stop.hex} className="bg-panel px-3 py-2.5">
							<div
								className="mb-1 h-4 w-full rounded-sm"
	...
	return (
		<Shell id="api" className="border-y border-line/60 bg-panel/30">
			...
			<p className="mt-4 flex items-center gap-2 text-sm text-mist/70">
				...
				Hold and drag across a viewport to pump heat into the mask — release and
				it cools back to the resting wave.
			</p>
		</Shell>
	);
}
...
export function HowToUseSection() {
	return (
		<Shell id="usage">
			...
			/>
			<p className="mb-6 max-w-2xl text-mist">
				Point <code className="rounded bg-panel2 px-1 text-cyan">logoUrl</code>{" "}
				at any image with an alpha channel. This is the brief’s exact two-up
				usage, with a custom mask per panel:
			</p>
