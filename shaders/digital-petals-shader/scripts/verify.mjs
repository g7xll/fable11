...
const readClock = (page) =>
  page.evaluate(() => {
    ...
    const clock = cells.find((c) => /^\d{2}:\d{2}$/.test((c.textContent || "").trim()));
...
const readBloom = (page) =>
  page.evaluate(() => {
    ...
    const label = nodes.find((n) => (n.textContent || "").trim() === "Bloom index");
...
try {
  ...
  shotA.length > 4000
    ...
    : fail("Bloom is drawing", `frame only ${shotA.length} bytes (looks black)`);
  !shotA.equals(shotB)
