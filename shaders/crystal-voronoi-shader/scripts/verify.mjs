...
check(
	"page title",
	(await page.title()).startsWith("Crystal Synthesis"),
	await page.title(),
);
...
check(
	"canvas is fixed (full-viewport background)",
	info.pos === "fixed",
	info.pos,
);
...
const avg = sum / n;
check(
	"shader paints non-trivial pixels (avg luminance > 6)",
	avg > 6,
	`avg=${avg.toFixed(1)}`,
);
check(
	"shader has visible cell contrast (max-min > 30)",
	max - min > 30,
...
check(
	"spectral cast swatch present",
	(await page.getByText("Spectral cast").count()) === 1,
);
...
check(
	"cell-density fader updates lattice read-out",
	cellsBefore !== cellsAfter,
	`${cellsBefore.trim()} -> ${cellsAfter.trim()}`,
);
...
check(
	"reset restores cell density to 8.0",
	densityVal === "8" || densityVal === "8.0",
	densityVal,
);

// ── Signature: goniometer reticle SVG present ──────────────────────────────
check(
	"goniometer reticle rendered",
	(await page.locator("main svg").count()) >= 1,
);
check(
	"caliper corner brackets present",
	(await page.locator(".specimen-grain > div span[aria-hidden]").count()) >=
		4 || (await page.locator("span[aria-hidden]").count()) >= 4,
);
...
check(
	"calibration deck still visible on mobile",
	await page.getByText("Calibration Deck", { exact: true }).isVisible(),
);
