...
else if (canvas.w < 2 || canvas.h < 2) fail(`canvas too small: ${JSON.stringify(canvas)}`);
else console.log("OK canvas:", JSON.stringify(canvas));

// 2) Move the mouse across the viewport (drives the point-light uniform path).
for (const [x, y] of [[200, 200], [640, 300], [1000, 500], [640, 420]]) {
...
if (pixelStats.litPct < 0.3)
  fail(`canvas appears black — only ${pixelStats.litPct.toFixed(3)}% lit pixels (color/light fix not working?)`);
...
if (!copy.title || !copy.subtitle || !copy.description) fail("hero copy missing");
...
if (!fade.found) fail(".animate-fade-in-long element not found");
  ...
  fail(`fadeIn keyframes not wired — animation-name is "${fade.animationName}"`);
...
if (badConsole.length) fail("shader/webgl console errors:\n" + badConsole.join("\n"));
console.log(
  `OK errors: ${pageErrors.length} pageerrors, ${consoleErrors.length} console errors (${badConsole.length} shader-related)`,
);
if (consoleErrors.length) console.log("   (non-shader console errors:", JSON.stringify(consoleErrors), ")");
