...
await must("brand 'FIELD GENERATOR' present", () =>
	page
		.getByText(/FIELD GENERATOR/i)
		.count()
		.then((n) => n >= 1),
);
await must("subtitle mentions sine waves", () =>
	page
		.getByText(/sine waves/i)
		.count()
		.then((n) => n >= 1),
);
...
await must("theme toggle button present", () =>
	page
		.getByRole("button", { name: /toggle colour theme/i })
		.count()
		.then((n) => n >= 1),
);
...
if (webglSupported) {
	...
	let darkStats;
	await must(
		"dark palette: dotted field paints (luminance contrast)",
		async () => {
