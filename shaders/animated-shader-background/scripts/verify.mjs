...
console.log(`(environment: WebGL ${webglSupported ? "available" : "UNAVAILABLE -> CSS fallback path"})`);
...
if (webglSupported) {
  ...
  await must("shader paints visible aurora (screenshot is not flat black)", async () => {
...
await must("headline 'unbounded.' present", () =>
  page.getByRole("heading", { level: 1 }).innerText().then((t) => /unbounded/i.test(t))
);
await must("brand 'AnoAI' present", () => page.getByText(/Ano/).first().count().then((n) => n >= 1));
await must("primary CTA present", () =>
  page.getByText(/Start building free/i).count().then((n) => n >= 1)
);
await must("'Watch the demo' (Play icon CTA)", () =>
  page.getByText(/Watch the demo/i).count().then((n) => n >= 1)
);
...
await must("scroll cue present", () => page.getByText(/^Scroll$/i).count().then((n) => n >= 1));
...
await must("divider element rendered", () =>
  page.locator(".divider").count().then((n) => n >= 1)
);
