...
try {
  ...
  check("shader paints non-black field", paint.lit > 50 && paint.maxL > 30, `lit=${paint.lit} maxL=${paint.maxL}`);
  check("field has ring structure (luminance varies)", paint.std > 20, `std=${paint.std}`);
  ...
  const timeText = () =>
    page.evaluate(() => {
      ...
      return el?.parentElement?.querySelector("span:last-child")?.textContent ?? null;
    });
  const t1 = await timeText();
  await page.waitForTimeout(900);
  const t2 = await timeText();
  check("hero time readout renders", t1 !== null, t1 || "missing");
  check("time advances while running", t1 !== null && t2 !== null && t1 !== t2, `${t1} -> ${t2}`);
  ...
  check("channel scope streams 3 readouts", scopeVals.length >= 3, scopeVals.slice(0, 3).join(" / "));
  check("at least one channel is lit", scopeVals.some((v) => Number(v) > 0), scopeVals.slice(0, 3).join(" / "));
