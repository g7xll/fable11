...
export function ChannelScope() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("shader:frame", handler as EventListener);
      ...
      {CHANNELS.map((c, j) => (
        ...
        >
          <span
            ...
            aria-hidden
          />
          <span className={`w-24 flex-shrink-0 font-mono text-[10px] uppercase tracking-[0.12em] ${c.css} sm:text-[11px]`}>
            {c.label}
          </span>
          <canvas
            ref={sparkRefs[j]}
            ...
            aria-hidden
          />
          <div className="relative hidden h-1.5 w-16 overflow-hidden rounded-full bg-ink-900 md:block" aria-hidden>
            <div
              ref={barRefs[j]}
...
function drawSpark(canvas: HTMLCanvasElement | null, hist: number[], hex: string) {
  if (!canvas) return;
