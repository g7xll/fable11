...
try {
  ...
  const canvas = await page.$("div[aria-label='Celestial Matrix animated background'] canvas");
  ...
  shotA.length > 4000
    ...
    : fail("Matrix is drawing", `frame only ${shotA.length} bytes (looks black)`);
  !shotA.equals(shotB)
  ...
  const readWarp = () =>
    page.evaluate(() => {
      const cells = [...document.querySelectorAll(".tabular-nums")].map((n) => n.textContent);
      return cells.join("|");
    });
