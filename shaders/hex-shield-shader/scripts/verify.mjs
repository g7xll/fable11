...
try {
	...
	!before.equals(after)
		...
		: fail(
				"Arc Gain fader re-drives the shader",
				"frozen frame unchanged after moving gain",
			);
	...
	breachOk && !preBreach.equals(postBreach)
		...
		: fail(
				'"Breach Alert" profile repaints the dome',
				`clicked=${breachOk}, frame ${preBreach.equals(postBreach) ? "unchanged" : "changed"}`,
			);
