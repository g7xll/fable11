...
import CelestialMatrixShader, { type MatrixSample } from "@/components/ui/martrix-shader";
...
const BOOT_LINES: BootLine[] = [
  { text: "meridian://relay-9 $ link --feed celestial-matrix", tone: "ion", pause: 280 },
  { text: "carrier lock ......... acquired @ 12.97 GHz", tone: "dim", pause: 140 },
  { text: "decoding digital rain ... 30x30 lattice stable", tone: "dim", pause: 140 },
  { text: "gravitational warp ... bound to operator cursor", tone: "dim", pause: 200 },
  { text: "auth handshake [████████████] verified", tone: "aurora", pause: 220 },
...
function useBootSequence() {
  const [progress, setProgress] = useState({ lineIndex: 0, charIndex: 0, done: false });
    ...
    const tick = () => {
      ...
      for (let i = 0; i < BOOT_LINES.length; i++) {
        ...
        if (budget < typeMs) {
          setProgress({
            lineIndex: i,
            charIndex: Math.min(line.text.length, Math.floor(budget / TYPE_SPEED)),
            done: false,
          });
        ...
        if (budget < line.pause) {
          ...
          setProgress({ lineIndex: i, charIndex: line.text.length, done: false });
          raf = requestAnimationFrame(tick);
          return;
        }
        budget -= line.pause;
      ...
      setProgress({ lineIndex: last, charIndex: BOOT_LINES[last].text.length, done: true });
    };
...
function Readout({
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className="font-mono text-[10px] uppercase tracking-[0.28em] text-mist/60">{label}</span>
      <span className={cn("font-mono text-sm tabular-nums leading-none", accent)}>{value}</span>
    </div>
  );
}
    ...
    <main className="relative h-screen w-screen overflow-hidden bg-void text-white">
        ...
        >
          ...
          <span
            className={cn(
              ...
              reticle.x > window.innerWidth - 160 ? "right-7 text-right" : "left-7",
            )}
          >
            warp · lock
          </span>
          ...
          <div className="relative overflow-hidden rounded-sm border border-ion/25 bg-void/55 shadow-[0_0_60px_-12px_rgba(26,77,222,0.55)] backdrop-blur-md">
            ...
            <div className="flex items-center justify-between border-b border-ion/15 bg-white/[0.02] px-4 py-2.5">
              <div className="flex items-center gap-2">
                <RadioTower className="h-3.5 w-3.5 text-ion" strokeWidth={1.5} />
            ...
            <div className="px-5 pb-5 pt-4 sm:px-7">
              <div className="min-h-[8.5rem] space-y-1.5 font-mono text-[12.5px] leading-relaxed sm:text-[13px]">
                {BOOT_LINES.map((line, i) => {
                  ...
                  return (
                    <p key={i} className={cn("whitespace-pre-wrap break-words", TONE_CLASS[line.tone])}>
                      {shown}
                      {isCurrent && (
              ...
              >
                ...
                <p className="mt-3 max-w-md font-body text-sm leading-relaxed text-mist">
                  ...
                  gravitational lock the lattice falls toward. Every readout below
                  is sampled straight off the render loop.
                </p>
      ...
      <footer className="absolute inset-x-0 bottom-0 z-30 border-t border-ion/12 bg-void/55 px-6 py-3.5 backdrop-blur-sm sm:px-10">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-x-8 gap-y-3">
          <Readout label="Feed clock" value={feedClock} accent="font-mono text-ion" />
