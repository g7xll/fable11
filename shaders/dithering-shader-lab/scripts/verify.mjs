...
try {
	...
	check(
		...
		paint.lit > 80 && paint.maxL > 40,
		`lit=${paint.lit} maxL=${paint.maxL}`,
	);
	check(
		"front ink (magenta) is rendered",
		paint.magenta > 30,
		`magentaPixels=${paint.magenta}`,
	);
	...
	const clock1 = await page
		.locator("text=/\\d+\\.\\d+s/")
		.first()
		.textContent()
		.catch(() => null);
	await page.waitForTimeout(900);
	const clock2 = await page
		.locator("text=/\\d+\\.\\d+s/")
		.first()
		.textContent()
		.catch(() => null);
	check(
		"shader clock advances",
	...
	const frozen1 = await page
		.locator("text=/\\d+\\.\\d+s/")
		.first()
		.textContent()
		.catch(() => null);
	await page.waitForTimeout(800);
	const frozen2 = await page
		.locator("text=/\\d+\\.\\d+s/")
		.first()
		.textContent()
		.catch(() => null);
	check(
		"freeze halts the shader clock",
		!!frozen1 && frozen1 === frozen2,
		`${frozen1} == ${frozen2}`,
	);
