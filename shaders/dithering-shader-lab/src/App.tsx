...
const SHAPES: ShapeDef[] = [
  { key: "wave", label: "Wave", caption: "Advancing sine wavefront", Icon: Waves },
  { key: "simplex", label: "Simplex", caption: "Fractal value-noise field", Icon: Cloud },
  { key: "warp", label: "Warp", caption: "Domain-warped interference", Icon: Spline },
  { key: "dots", label: "Dots", caption: "Randomised stripe lattice", Icon: Grip },
  { key: "ripple", label: "Ripple", caption: "Radial standing waves", Icon: Radio },
  { key: "swirl", label: "Swirl", caption: "Logarithmic spiral vortex", Icon: Tornado },
  { key: "sphere", label: "Sphere", caption: "Lit orbiting hemisphere", Icon: Globe },
];
...
const PRESETS: Preset[] = [
  { name: "Wave", shape: "wave", type: "8x8", colorBack: "#001122", colorFront: "#ff0088", pxSize: 3, speed: 0.6 },
  { name: "Aurora", shape: "simplex", type: "8x8", colorBack: "#00121f", colorFront: "#22ffa8", pxSize: 4, speed: 0.5 },
  { name: "Orbit", shape: "sphere", type: "4x4", colorBack: "#0a0012", colorFront: "#ffd166", pxSize: 3, speed: 0.7 },
  { name: "Vortex", shape: "swirl", type: "random", colorBack: "#070010", colorFront: "#ff5d8f", pxSize: 2, speed: 0.9 },
  { name: "Sonar", shape: "ripple", type: "8x8", colorBack: "#00131c", colorFront: "#4fc3ff", pxSize: 3, speed: 0.8 },
];
...
function App() {
  ...
  const teleRef = useRef<DitherTelemetry>({ fps: 0, time: 0, width: 0, height: 0, cols: 0, rows: 0 });
  ...
  const idx = Math.max(0, SHAPES.findIndex((s) => s.key === shape));
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start justify-between gap-4 bg-gradient-to-b from-ink/85 via-ink/40 to-transparent p-4 pb-10 sm:p-6 sm:pb-14">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <h1 className="font-display text-lg font-bold tracking-[0.18em] text-paper">DITHER LAB</h1>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-[88px] z-20 px-4 sm:px-6 md:inset-x-auto md:bottom-12 md:left-0 md:top-auto md:max-w-[calc(100%-392px)] md:px-10">
        <div key={shape} className="animate-rise">
          <div className="mb-2 flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.26em] text-ash">
            <active.Icon className="h-3.5 w-3.5 text-amber" strokeWidth={1.75} />
          ...
          <p className="mt-3 max-w-sm font-body text-sm leading-relaxed text-ash">
            {active.caption} — pixelised to a {pxSize}px grid, then resolved to two inks through the {type} matrix.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6 p-5 sm:p-6">
              ...
              {SHAPES.map((s, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono text-[11px] uppercase tracking-[0.12em]">{s.label}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {MATRICES.map((m) => {
                ...
                return (
                  ...
                  >
                    <div className="font-mono text-xs font-medium tabular-nums">{m.label}</div>
                    <div className="font-mono text-[8px] uppercase tracking-wide text-ash-dim">{m.sub}</div>
                  </button>
                );
              })}
            </div>
          ...
          <Section title="Inks">
            <div className="grid grid-cols-2 gap-3">
              <Swatch label="front" value={colorFront} onChange={setColorFront} />
          ...
          <p className="border-t border-line pt-4 font-mono text-[10px] leading-relaxed text-ash-dim">
            Drop-in lives at{" "}
            <span className="text-ash">@/components/ui/dithering-shader</span>. This bench drives the same GLSL ES 3.00
            through live uniforms — no recompiles per change.
          </p>
        </div>
      </aside>
    </main>
  ...
  return (
    <section>
      <div className="mb-2.5 flex items-baseline justify-between">
        <h3 className="font-mono text-[10px] uppercase tracking-[0.28em] text-ash">{title}</h3>
        {hint && <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-ash-dim">{hint}</span>}
      </div>
      {children}
    </section>
  );
...
function Swatch({
  ...
  return (
    <div>
      <div className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.22em] text-ash">{label}</div>
      <div className="flex items-center gap-2 rounded-md border border-line bg-ink/30 p-1.5">
        ...
        <span className="font-mono text-xs uppercase tabular-nums text-paper">{value}</span>
      </div>
    </div>
  );
}

function TransportButton({ onClick, children }: { onClick: () => void; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
