...
const errors = [];
const IGNORABLE = /WebGL|swiftshader|GroupMarkerNotSet|Automatic fallback to software WebGL|GL_/i;
...
console.log(`(environment: WebGL2 ${webglSupported ? "available" : "UNAVAILABLE -> CSS fallback"})`);

if (webglSupported) {
  await must("WebGL2 <canvas> mounted", () => page.locator("canvas").count().then((n) => n >= 1));
  await must("canvas fills the viewport", async () => {
    const box = await page.locator("canvas").first().boundingBox();
    return box && box.width >= 1200 && box.height >= 700;
  });
  await must("shader paints visible light (centre is not flat black)", async () => {
...
await must("brand 'GLASSWORKS' present", () => page.getByText("GLASSWORKS").first().count().then((n) => n >= 1));
await must("headline mentions 'glassy'", () =>
  page.getByRole("heading", { level: 1 }).innerText().then((t) => /glassy/i.test(t)),
);
await must("components/ui import line shown", () =>
  page.getByText(/@\/components\/ui\/abstract-glassy-shader/).first().count().then((n) => n >= 1),
);
await must("'Copy component' CTA present", () => page.getByText(/Copy component/i).count().then((n) => n >= 1));
await must("control deck present", () => page.getByText(/Control Deck/i).count().then((n) => n >= 1));
await must("≥ 7 faders (range inputs) wired", () =>
  page.locator('input[type="range"]').count().then((n) => n >= 7),
);
