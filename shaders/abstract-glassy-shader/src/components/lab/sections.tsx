...
function Integration() {
  return (
    <section id="integration" className="mx-auto max-w-[1100px] px-4 py-20 sm:px-6 sm:py-28">
      <SectionHeader index="01" eyebrow="Drop-in integration" title="Straight into components/ui">
        The shader ships as one self-contained file. If your app already runs the shadcn stack —
        Tailwind&nbsp;CSS, TypeScript and a <code className="font-mono text-foreground/90">components.json</code>{" "}
        — integration is a copy-paste plus an import.
      </SectionHeader>

      <div className="mb-12 grid gap-4 md:grid-cols-3">
        <StepCard icon={<FolderTree className="h-[18px] w-[18px]" />} title="Why /components/ui">
          shadcn maps the <code className="font-mono text-foreground/90">ui</code> alias to{" "}
          <code className="font-mono text-foreground/90">@/components/ui</code> in{" "}
          <code className="font-mono text-foreground/90">components.json</code>. Keeping primitives
          there is what makes the demo's <code className="font-mono text-foreground/90">@/components/ui/…</code>{" "}
          import resolve, lets the CLI manage the file, and puts it where every consumer expects.
        </StepCard>
        <StepCard icon={<Boxes className="h-[18px] w-[18px]" />} title="Paste the file">
          Save the component as{" "}
          <code className="font-mono text-foreground/90">src/components/ui/abstract-glassy-shader.tsx</code>.
          It needs nothing but React — no Three.js, no context providers, no images or fonts.
        </StepCard>
        <StepCard icon={<Sparkles className="h-[18px] w-[18px]" />} title="Import & render">
          Pull in the named <code className="font-mono text-foreground/90">ShaderComponent</code> and
          give it a sized parent. Optional <code className="font-mono text-foreground/90">settings</code>{" "}
          and <code className="font-mono text-foreground/90">onFrame</code> props are there when you
          want them.
        </StepCard>
      </div>
      ...
      <CodeTabs
        tabs={[
          ...
          { id: "component", label: "abstract-glassy-shader.tsx", code: componentSource },
  ...
  return (
    <section id="anatomy" className="border-t border-border bg-white/[0.012]">
      <div className="mx-auto max-w-[1100px] px-4 py-20 sm:px-6 sm:py-28">
        <SectionHeader index="02" eyebrow="How it works" title="Anatomy of the field">
          The whole image is six lines of signed-distance math evaluated on a single full-screen
          quad. Every fader on the control deck maps to one term below.
        </SectionHeader>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ANATOMY.map((a) => (
            <div key={a.tag} className="glass-soft rounded-xl p-5">
              ...
              <h3 className="mt-3.5 text-[15px] font-semibold text-foreground">{a.title}</h3>
...
const PROPS: [string, string, string, string][] = [
  ["className", "string", "—", "Applied to the <canvas> (or fallback). Size it through its parent."],
  ["settings", "Partial<ShaderSettings>", "{}", "Live overrides — omit any field to keep the original constant."],
  ["onFrame", "(s: ShaderStats) => void", "—", "Per-frame readouts: time, fps, buffer size, blob centres."],
...
const SETTINGS: [string, string, string, string][] = [
  ...
  ["palette", '"spectrum" | "glass" | "ember"', '"spectrum"', "Colour ramp; spectrum is verbatim."],
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r[0]} className="border-b border-border/60 last:border-0">
              <td className="whitespace-nowrap px-4 py-3 font-mono text-[12.5px] text-accent">{r[0]}</td>
              ...
              <td className="px-4 py-3 text-[13px] text-muted-foreground">{r[3]}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <section id="api" className="border-t border-border">
      <div className="mx-auto max-w-[1100px] px-4 py-20 sm:px-6 sm:py-28">
        <SectionHeader index="03" eyebrow="Reference" title="Props & uniforms">
          Default props reproduce the source shader frame-for-frame. Pass a partial{" "}
          <code className="font-mono text-foreground/90">settings</code> object to promote any baked-in
          constant to a live uniform.
        </SectionHeader>
    ...
    <footer className="border-t border-border">
      <div className="mx-auto flex max-w-[1100px] flex-col items-start justify-between gap-4 px-4 py-10 sm:flex-row sm:items-center sm:px-6">
        <div className="flex items-center gap-2.5">
          <span className="text-[14px] font-semibold tracking-tight">GLASSWORKS</span>
