import { Droplets, Flame, Pause, Play, RotateCcw, Sliders, Waves } from "lucide-react";
import { Fader, Segmented } from "@/components/lab/controls";
import { cn } from "@/lib/utils";
import type { PaletteId, ShaderSettings } from "@/components/ui/abstract-glassy-shader";
      ...
      <div className="grid grid-cols-2 gap-x-5 gap-y-3.5 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <div className="block">
          ...
          <Segmented<PaletteId>
            ...
            options={[
              { value: "spectrum", label: "Spec", icon: <Waves className="h-3 w-3" /> },
              { value: "glass", label: "Glass", icon: <Droplets className="h-3 w-3" /> },
              { value: "ember", label: "Ember", icon: <Flame className="h-3 w-3" /> },
            ]}
          />
        </div>
      </div>
