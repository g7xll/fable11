...
const PALETTE_INDEX: Record<PaletteId, number> = { spectrum: 0, glass: 1, ember: 2 };
    ...
    const animate = (now: number) => {
      ...
      if (onFrame) {
        ...
        onFrame({
          ...
          width: gl.width,
          height: gl.height,
          blobA: [Math.cos(clock) * s.spread, Math.sin(clock * 2) * s.spread * 0.5],
          blobB: [
...
function GlassFallback({ className, style }: { className?: string; style?: React.CSSProperties }) {
  ...
  return (
    <div className={className} style={style} data-shader-fallback="true" aria-hidden="true">
...
/* ---------- Component ---------- */
export function ShaderComponent({ className, style, settings, onFrame }: ShaderComponentProps) {
