...
import AuraReactor, { type ReactorTelemetry } from "@/components/ui/aura-reactor";
...
const PROFILES: Profile[] = [
  ...
  },
  {
    ...
    name: "Ion Bloom",
    band: "DIFFUSE",
    blurb: "Magenta haze. Soft focus and heavy distortion bloom the particle field.",
...
function Readout({
  ...
  return (
    <div className="border-t border-[var(--hair)] py-2.5 first:border-t-0">
      <div className="font-mono text-[9px] uppercase tracking-[0.24em] text-mist/70">{label}</div>
      <div
      ...
      {sub ? <div className="mt-0.5 font-mono text-[9px] tracking-wide text-mist/55">{sub}</div> : null}
    </div>
  );
}
...
export default function App() {
  ...
  const [activeProfile, setActiveProfile] = useState<string | null>(PROFILES[0].id);
  ...
  const integrityColor =
    integrity > 80 ? "var(--plasma)" : integrity > 62 ? "var(--amber)" : "var(--warn)";
    ...
    <div className="plate-grain relative flex h-full w-full flex-col overflow-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between gap-4 border-b border-[var(--line-soft)] px-5 py-3 md:px-8">
        ...
        <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-mist">
          ...
          <span className="tabular-nums text-ink/90">{formatClock(tele.time)}</span>
          <span
      ...
      <main className="relative z-10 grid flex-1 min-h-0 grid-cols-1 gap-4 p-4 md:grid-cols-[1fr_320px] md:gap-5 md:p-6 xl:grid-cols-[260px_1fr_320px]">
        ...
        <aside className="hidden min-h-0 flex-col gap-3 xl:flex">
          <SectionLabel icon={<Hexagon className="h-3 w-3" />} text="Field Profiles" />
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "ring-1 ring-amber/60" : "hover:border-[var(--line)]",
                  )}
                >
                  ...
                  <p className="mt-1.5 text-[11px] leading-snug text-mist">{p.blurb}</p>
                </button>
              );
            })}
          </div>
        ...
        <section className="relative min-h-0">
          <SectionLabel
            ...
            right={
              <span className="font-mono text-[9px] uppercase tracking-[0.24em]" style={{ color: coreCss }}>
                {band} · λ-BAND
              </span>
            }
          />
          <div className="panel bracket scanlines viewport-vignette relative mt-3 h-[calc(100%-1.75rem)] min-h-[280px] overflow-hidden rounded-lg">
            ...
            <div className="pointer-events-none absolute bottom-3 left-3 font-mono text-[9px] uppercase tracking-[0.22em] text-ink/55">
              ...
              <br />
              <span className="text-mist/45">FBM CORE / 10 RAYS / NOISE FIELD</span>
            </div>
        ...
        <aside className="flex min-h-0 flex-col gap-4 overflow-auto thin-scroll md:gap-5">
          ...
          <div className="panel bracket rounded-lg p-4">
            <div className="mb-3 flex items-center justify-between">
              <SectionLabel icon={<Waves className="h-3 w-3" />} text="Field Controls" inline />
              <button
          ...
          <div className="panel bracket rounded-lg p-4 xl:hidden">
            <SectionLabel icon={<Hexagon className="h-3 w-3" />} text="Field Profiles" inline />
          ...
          {/* Telemetry */}
          <div className="panel bracket rounded-lg p-4">
            <SectionLabel icon={<Activity className="h-3 w-3" />} text="Core Telemetry" inline />
            <div className="mt-2">
              <Readout
                label="Containment Integrity"
                value={`${integrity.toFixed(1)} %`}
                sub={integrity > 80 ? "NOMINAL" : integrity > 62 ? "ELEVATED LOAD" : "DESTABILIZING"}
      ...
      <footer className="relative z-10 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--line-soft)] px-5 py-2.5 font-mono text-[9px] uppercase tracking-[0.22em] text-mist/60 md:px-8">
        <span>
          drop-in&nbsp;·&nbsp;<span className="text-mist">@/components/ui/aura-core</span>
        </span>
        <span className="hidden sm:inline">
          shadcn structure&nbsp;·&nbsp;tailwind&nbsp;·&nbsp;typescript&nbsp;·&nbsp;webgl2
        </span>
        <span>
          uMouse · uTime · uColor · uPower · uFocus · uDistortion
        </span>
      </footer>
    </div>
  );
...
function ReticleFrame({
  ...
  return (
    ...
    >
      ...
      <circle cx="100" cy="100" r="92" stroke="rgba(138,151,173,0.16)" strokeWidth="0.4" />
...
function StatusChip({
  ...
  const color = tone === "plasma" ? "var(--plasma)" : tone === "warn" ? "var(--warn)" : "var(--amber)";
  return (
    <div className="flex items-center gap-2">
      <span style={{ color }}>{icon}</span>
      <span className="font-mono text-[9px] uppercase tracking-[0.24em] text-mist/70">{label}</span>
...
function SectionLabel({
  ...
  return (
    <div className={cn("flex items-center justify-between", !inline && "px-0.5")}>
      <div className="flex items-center gap-2">
