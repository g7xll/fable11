...
try {
  ...
  gl.ctx ? ok("WebGL2 context is live") : fail("WebGL2 context is live", "getContext returned null");
  gl.w > 0 && gl.h > 0
  ...
  const before = await page.screenshot({ clip: { x: 0, y: 0, width: 900, height: 800 } });
  const hue = page.getByLabel("Hue", { exact: true });
  await hue.focus();
  for (let i = 0; i < 30; i++) await page.keyboard.press("ArrowRight");
  await sleep(500);
  const after = await page.screenshot({ clip: { x: 0, y: 0, width: 900, height: 800 } });
  !before.equals(after)
    ? ok("Hue fader re-drives the shader")
    : fail("Hue fader re-drives the shader", "frame unchanged after moving Hue");
