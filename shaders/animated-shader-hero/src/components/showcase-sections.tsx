      ...
      <div className="mx-auto max-w-6xl">
        <Reveal>
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-ash md:text-lg">
            The hero is a single self-contained file. It lives where every shadcn
            primitive lives — <code className="text-gold">@/components/ui</code> —
            so the import path is the convention, not an accident. WebGL2 is the
            only runtime dependency, and it ships inside the component.
          </p>
        </Reveal>
          ...
          <Reveal delay={120}>
            <CodeShell
              ...
              lines={[
                ...
                <>
                  {"      "}headline={c.punc("{{")} line1: {c.str('"Launch Your"')},
                </>,
                <>
                  {"               "}line2: {c.str('"Workflow Into Orbit"')} {c.punc("}}")}
                </>,
                <>
        ...
        <Reveal delay={80}>
          <div className="mt-8 flex items-start gap-4 rounded-2xl border border-gold/10 bg-void-900/60 p-5 md:p-6">
            ...
            <p className="text-sm leading-relaxed text-ash md:text-[15px]">
              ...
              shadcn’s <code className="text-gold">components.json</code> aliases{" "}
              <code className="text-gold">ui</code> to this folder, so the CLI, the
              registry, and every generated import resolve here. Keeping the hero
              alongside your <code className="text-gold">button</code> and{" "}
              <code className="text-gold">card</code> means{" "}
              <code className="text-gold">@/components/ui/animated-shader-hero</code>{" "}
          ...
          {items.map((it, i) => (
            <Reveal key={it.uniform} delay={i * 80} className="bg-void">
              <div className="h-full p-7">
                ...
                <p className="mt-3 text-sm leading-relaxed text-ash">{it.copy}</p>
              </div>
            </Reveal>
          ))}
        </div>
