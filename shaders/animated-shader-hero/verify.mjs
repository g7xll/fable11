...
try {
  ...
  check("canvas + WebGL2 context present", gl.ok, gl.ok ? gl.vendor : gl.reason);
  ...
  check("field reads warm (ember palette)", paint.warm, `rAvg=${paint.rAvg} bAvg=${paint.bAvg}`);

  // 3. Live HUD advances (time uniform shown on screen).
  const hud1 = await page.locator("text=/\\d+\\.\\d+s/").first().textContent().catch(() => null);
  await page.waitForTimeout(900);
  const hud2 = await page.locator("text=/\\d+\\.\\d+s/").first().textContent().catch(() => null);
  check("uniforms HUD renders time", !!hud1, hud1 || "no time readout");
  check("time uniform advances", !!hud1 && !!hud2 && hud1 !== hud2, `${hud1} -> ${hud2}`);
  ...
  for (const [name, sel] of [
    ...
    ["integration section present", "text=/Tailwind, TypeScript, zero config/i"],
