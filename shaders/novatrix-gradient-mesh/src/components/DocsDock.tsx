...
const PROP_ROWS: PropRow[] = [
	{
		name: "colors",
		type: "string[]",
		def: '["#3b2a8d","#aaa7d7","#f75092"]',
		note: "First 3 hex stops → uColorA/B/C",
	},
	{
		name: "distortion",
		type: "number",
		def: "5",
		note: "Fractal loop iterations (baked into shader)",
	},
	...
	{ name: "offsetY", type: "number", def: "0", note: "Vertical UV pan" },
	{
		...
		type: "number",
		def: "90",
		note: "UV rotation (radians)",
	},
	{
		name: "waveAmp",
		type: "number",
		def: "0.1",
		note: "Water displacement amplitude",
	},
	{
		name: "waveFreq",
		type: "number",
		def: "10.0",
		note: "Water wave frequency",
	},
			...
			<div className="scroll-thin max-h-[300px] overflow-y-auto p-4">
				{tab === "install" && (
					<div className="space-y-3">
						...
						<CodeBlock
							label="fresh project (TS + Tailwind + shadcn)"
							code={INSTALL_SHADCN}
						/>
					</div>
				)}
							...
							<tbody>
								{PROP_ROWS.map((r) => (
									<tr key={r.name} className="border-t border-line/70">
										...
										<td className="px-3 py-1.5 text-[11px] text-haze">
											{r.note}
										</td>
									</tr>
								))}
				...
				{tab === "usage" && (
					<div className="space-y-3">
						<p className="text-[12.5px] leading-relaxed text-haze">
							...
							<span className="font-mono text-rose">relative</span> box and
							layer content above with{" "}
							<span className="font-mono text-rose">z-10</span>.
						</p>
						<CodeBlock
							label="drop-in hero (verbatim demo)"
							code={USAGE_BASIC}
						/>
				...
				{tab === "why" && (
					<div className="space-y-3">
						<p className="text-[12.5px] leading-relaxed text-haze">
							...
							<span className="font-mono text-rose">components/ui</span>, the
							path its{" "}
							<span className="font-mono text-paper">components.json</span>{" "}
							alias points at. Keeping the file there means the{" "}
							<span className="font-mono text-rose">
								@/components/ui/gradient-mesh
							</span>{" "}
							import in the prompt's{" "}
							<span className="font-mono text-paper">demo.tsx</span> resolves
							with zero edits, the CLI can update it in place, and the shader
							sits beside the rest of your owned primitives.
						</p>
