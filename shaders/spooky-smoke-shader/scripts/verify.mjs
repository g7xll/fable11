...
try {
  ...
  if (canvasReport.error) {
    ...
    if (canvasReport.max - canvasReport.min > 2) ok("smoke field has tonal variation");
  ...
  const reagentButtons = page.locator('section[aria-label="Vapour console"] button[aria-pressed]');
  ...
  if (before !== after) ok(`reagent click changes the readout (${before.trim()} -> ${after.trim()})`);
