...
for (const s of SPECIMENS) {
  const page = await browser.newPage({ viewport: { width: SIZE + 20, height: SIZE + 20 } });
  const url = `file://${htmlPath}?blades=${s.blades}&gain=${s.gain}&hue=${s.hue}&t=${s.t}&size=${SIZE}`;
  await page.goto(url, { waitUntil: "load", timeout: 30000 });
  try {
    await page.waitForFunction(() => window.__ready === true, { timeout: 15000 });
  } catch {
    const err = await page.evaluate(() => window.__error || "timeout (no __ready)");
  ...
  console.log(`ok   ${s.slug}.jpg  (blades ${s.blades}, gain ${s.gain}, hue ${s.hue})`);
  ok++;
  await page.close();
}
