...
const errors = [];
const IGNORABLE = /WebGLRenderer|WebGL context|Error creating WebGL|swiftshader|GroupMarkerNotSet|Automatic fallback to software WebGL|Failed to load resource/i;
...
await must("three live <canvas> layers mounted (hero, deck, drop-in)", () =>
  page.locator("canvas").count().then((n) => n >= 3)
);
...
if (webgl) {
  await must("hero paints a non-black, colored bloom (decoded screenshot)", async () => {
    ...
    console.log(`   hero region: maxLum=${s.maxLum.toFixed(1)} lit=${(s.litFraction * 100).toFixed(1)}% avg=rgb(${s.avg.r.toFixed(0)},${s.avg.g.toFixed(0)},${s.avg.b.toFixed(0)})`);
    // The field is full-spectrum, so just require real, colored light (not flat black).
    return s.maxLum > 18 && s.litFraction > 0.02 && s.avg.r + s.avg.g + s.avg.b > 5;
  });
}

await must("brand 'APER·TURE' present", () => page.getByText(/APER/).first().count().then((n) => n >= 1));
await must("hero headline 'APERTURE'", () => page.getByRole("heading", { level: 1 }).innerText().then((t) => /APERTURE/i.test(t)));
await must("section: control deck", () => page.getByText(/control deck/i).count().then((n) => n >= 1));
await must("section: anatomy", () => page.getByText(/anatomy/i).count().then((n) => n >= 1));
await must("section: field captures", () => page.getByText(/field captures/i).count().then((n) => n >= 1));
await must("section: integration", () => page.getByText(/integration/i).count().then((n) => n >= 1));
await must("section: api", () => page.getByText(/^api$/i).count().then((n) => n >= 1));

await must("4 control sliders present", () => page.locator('input[type="range"]').count().then((n) => n === 4));
await must("5 presets present", () => page.getByText(/^(ORIGIN|CORONA|IRIS|EMBER|HALO)$/).count().then((n) => n >= 5));
await must("'why components/ui' callout present", () => page.getByText(/why components\/ui/i).count().then((n) => n >= 1));
await must("copyable import line present", () => page.getByText(/@\/components\/ui\/raidal-2/).first().count().then((n) => n >= 1));

await must("Space Grotesk applied to body", () =>
  page.evaluate(() => getComputedStyle(document.body).fontFamily.toLowerCase().includes("space grotesk"))
);
...
await must("6 specimen images decode (naturalWidth > 0)", () =>
  page.evaluate(() => {
    const imgs = Array.from(document.querySelectorAll('img[src*="/assets/specimens/"]'));
...
await must("mobile: headline visible", () => page.getByRole("heading", { level: 1 }).isVisible());
