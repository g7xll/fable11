/* ===========================================================================
   raidal-2.tsx — the component from the integration brief, dropped into
   `@/components/ui` and made TypeScript-correct for a Vite + shadcn project.

   It renders a self-contained WebGL2 *radial bloom* on a single full-screen
   triangle: nine accumulating "blades" gated by an angular smoothstep and
   tone-mapped with tanh. No three.js — just a raw WebGL2 context.

   Deliberate, behaviour-preserving changes from the pasted snippet (only):
     1. Dropped the leading "use client" directive — this is a Vite SPA, not a
        React Server Components tree, so the directive is meaningless here and
        only makes the bundler warn.
     2. `import React` -> named `{ useEffect, useRef }`. The default `React`
        import was unused and trips `noUnusedLocals` under strict TypeScript.
     3. The GLSL loop counter is initialised (`float i = 0.0, a = 0.0`) instead
        of being left undefined, so every driver starts the accumulation at the
        same place (the original relied on zero-initialised locals).
     4. `ShaderCanvas` and `SHADER_SRC` are also exported (named) so the
        showcase can reuse the *exact* same shader. The default export and its
...
export function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    // `!` keeps the rest of the effect (incl. nested rAF/resize closures) typed
    // as a live context; the runtime guard below still bails on null.
