...
export interface ShaderCanvasProps {
	/** Hex tint applied to the shader's RGB pattern, e.g. "#ffffff". */
	baseColor: string;
	/** Multiplier on the shader clock — higher animates faster. */
	speed: number;
	/** Horizontal tear amount on glitch rows (0 → 1). */
	glitchIntensity: number;
	/** Chromatic-aberration offset between R and B channels (0 → 0.05). */
	rgbShift: number;
	/** Number of scanlines packed across the vertical axis. */
	scanlineDensity: number;
	/** How strongly scanlines darken the image (0 → 1). */
...
interface ThreeRefs {
	...
	uniforms?: {
		...
		u_base_color: { value: THREE.Color };
		u_glitch_intensity: { value: number };
		u_rgb_shift: { value: number };
		u_scanline_density: { value: number };
		u_scanline_opacity: { value: number };
	};
	clock?: THREE.Clock;
	speed?: number;
}
...
const ShaderCanvas = memo((props: ShaderCanvasProps) => {
	...
	useEffect(() => {
		...
		const animate = () => {
			...
			uniforms.u_time.value =
				(refs.clock?.getElapsedTime() ?? 0) * (refs.speed ?? 1);
