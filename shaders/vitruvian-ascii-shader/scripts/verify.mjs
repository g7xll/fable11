...
const BLOCKED =
	/jsdelivr\.net|unicorn\.studio|fonts\.(googleapis|gstatic)\.com/;
const offenders = [];
...
check(
	"Canvas paints bright glyphs (not a black void)",
	first.maxChan > 120 && first.ink > 200,
...
check(
	...
	Math.abs(first.lum - second.lum) > 1000 ||
		Math.abs(first.ink - second.ink) > 40,
...
check(
	"No CDN / hosted-scene / remote-font requests",
	offenders.length === 0,
	offenders.join("; "),
);
...
check(
	"Mobile: ASCII canvas is hidden",
	!(await mobile.locator("canvas").first().isVisible()),
);
check(
	"Mobile: star backdrop present",
	(await mobile.locator(".stars-bg").count()) >= 1,
);
check(
	"Mobile: headline present",
	(await mobile.getByText("PROPORTIONS").count()) >= 1,
);
check(
