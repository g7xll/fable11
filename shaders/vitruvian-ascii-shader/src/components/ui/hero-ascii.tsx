...
/**
 * hero-ascii — the prompt's "PERFECT PROPORTIONS" hero, integrated verbatim.
 *
 * The only change from the original drop-in is its backdrop: the prompt injects
 * a UnicornStudio embed (runtime from jsdelivr + hosted scene `whwOGlfJ5Rz2rHaEUgHl`),
 * both of which are unreachable in this offline/locked-down environment. They are
 * replaced by <AsciiVitruvian /> — a self-contained canvas ASCII shader that draws
 * the same Vitruvian Man locally, so the hero is fully functional and runs offline.
 * All chrome (header, corner frame, headline, buttons, footer) is unchanged.
 */
	...
	return (
		<main className="relative min-h-screen overflow-hidden bg-black">
			...
			<div className="absolute inset-0 w-full h-full hidden lg:block">
				<AsciiVitruvian className="w-full h-full" />
			</div>
			...
			{/* The prompt's <style jsx> block, kept inline as a plain <style> so the
          component stays a self-contained drop-in under a Vite build. */}
			<style
