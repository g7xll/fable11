...
try {
  ...
  if (/components\/ui|pressed/i.test(h1)) ok(`hero headline renders ("${h1.slice(0, 42)}…")`);
  ...
  else if (canvasCount === 1) ok(`one dither canvas mounted (lazy hero may still resolve)`);
  ...
  const cardBox = await page.locator("section#top canvas").first().boundingBox();
  let live = false;
  let info = "";
  if (cardBox && cardBox.width > 120 && cardBox.height > 120) {
    const clip = { x: cardBox.x + 24, y: cardBox.y + 24, width: 80, height: 80 };
  ...
  if (isDark && backToLight) ok("theme toggle flips paper ↔ lights-out (.dark)");
  ...
  if (missing.length === 0) ok("integration / source / api / console sections render");
