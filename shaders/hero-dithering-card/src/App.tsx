...
const SHAPES: DitherShape[] = ["warp", "simplex", "dots", "wave", "ripple", "swirl", "sphere"];
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 sm:px-8">
        ...
        <nav className="hidden items-center gap-7 font-mono text-[11px] uppercase tracking-[0.14em] text-ink-dim md:flex">
          ...
          <a href="#source" className="transition-colors hover:text-foreground">Source</a>
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={onToggle}
            aria-label={dark ? "Switch to paper (light)" : "Switch to lights-out (dark)"}
    ...
    <section id="top" className="relative overflow-hidden border-b border-border">
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-10 pt-12 sm:px-8 sm:pt-16">
        ...
        <h1 className="max-w-4xl font-serif text-[clamp(2.6rem,7vw,5.5rem)] font-medium leading-[1.02] tracking-tight">
          A dithered hero card,{" "}
          <span className="text-press">pressed</span> into{" "}
        ...
        <p className="mt-6 max-w-2xl text-[15px] leading-relaxed text-ink-dim sm:text-base">
          The brief's <code className="font-mono text-press">CTASection</code> drops in verbatim —
          a transparent-backed <code className="font-mono text-press">Dithering</code> shader laying
          Pantone-orange ink under the headline. Below, the framed proof runs live on the press bed,
          with crop marks, a registration loupe, and ink read straight off the plate.
        </p>
...
function TelemetryStrip({ tel }: { tel: ReturnType<typeof useTelemetry> }) {
  return (
    <div className="mt-4 flex flex-wrap items-stretch gap-px overflow-hidden rounded-lg border border-border bg-border font-mono">
      ...
      <Gauge_ icon={<Droplets className="h-3.5 w-3.5" />} label="ink coverage" suffix="%">
  ...
  return (
    <div className="flex min-w-[7.5rem] flex-1 items-center gap-3 bg-card px-4 py-3">
      ...
      <span className="flex flex-col leading-tight">
        <span className="text-[9px] uppercase tracking-[0.16em] text-ink-dim">{label}</span>
    ...
    <section id="console" className="border-b border-border bg-paper-2/40">
      <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24">
        ...
        <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-ink-dim">
          Every control writes straight to a live <code className="font-mono text-press">Dithering</code>{" "}
          uniform — the exact props the card sets, here promoted to knobs so you can feel the plate
          before you ship it.
        </p>
                ...
                {INKS.map((i) => (
                  <button
                    ...
                    className={`h-8 w-8 rounded-full border transition-transform hover:scale-110 ${
                      ink === i.hex ? "border-foreground ring-2 ring-press/40" : "border-border"
                    }`}
  ...
  const steps = [
    {
      ...
      body: (
        <>
          The card renders through <code className="font-mono text-press">@paper-design/shaders-react</code>{" "}
          (the <code className="font-mono text-press">Dithering</code> shader) and uses{" "}
          <code className="font-mono text-press">lucide-react</code> for its arrow icon. Add both
          before copying the file in.
        </>
      ),
      code: INSTALL_SOURCE,
      filename: "bash",
    },
    {
      ...
      body: (
        <>
          shadcn maps the <code className="font-mono text-press">ui</code> alias to{" "}
          <code className="font-mono text-press">@/components/ui</code> in{" "}
          <code className="font-mono text-press">components.json</code>. The demo imports{" "}
          <code className="font-mono text-press">@/components/ui/hero-dithering-card</code>, so the
          file has to sit there for the alias — and any future{" "}
          <code className="font-mono text-press">shadcn add</code> — to resolve. If your project
          aliases <code className="font-mono text-press">@</code> elsewhere, create the folder so
          the import path stays portable.
        </>
      ),
      code: TREE_SOURCE,
      filename: "tree",
    },
    {
      ...
      body: (
        <>
          The shader sits behind the content at <code className="font-mono text-press">z-0</code> with{" "}
          <code className="font-mono text-press">colorBack="#00000000"</code>, so it blends onto
          whatever surface you place the card on. No props required.
        </>
      ),
      code: USAGE_SOURCE,
      filename: "page.tsx",
        ...
        <ol className="mt-12 grid gap-10 md:grid-cols-3 md:gap-8">
          {steps.map((s, i) => (
            <li key={s.title} className="flex flex-col">
              ...
              <p className="mt-2 flex-1 text-sm leading-relaxed text-ink-dim">{s.body}</p>
...
function Source() {
  ...
  const tabs: { id: Tab; label: string; file: string; code: string }[] = [
    { id: "component", label: "Component", file: "components/ui/hero-dithering-card.tsx", code: COMPONENT_SOURCE },
    { id: "demo", label: "Demo", file: "components/ui/demo.tsx", code: DEMO_SOURCE },
...
function Api() {
  const props: { name: string; type: string; def: string; desc: string }[] = [
    { name: "colorFront", type: "string", def: '"#EC4E02"', desc: "The ink the dither lays down. The card uses Pantone-orange." },
    { name: "colorBack", type: "string", def: '"#00000000"', desc: "Background fill — transparent here so the card surface shows through." },
    { name: "shape", type: "DitheringShape", def: '"warp"', desc: "Underlying field: simplex · warp · dots · wave · ripple · swirl · sphere." },
    { name: "type", type: "DitheringType", def: '"4x4"', desc: "Bayer matrix: random · 2x2 · 4x4 · 8x8 — the dot grid resolution." },
    { name: "speed", type: "number", def: "0.2 / 0.6", desc: "Animation rate. The card eases it up on hover." },
    { name: "minPixelRatio", type: "number", def: "1", desc: "Floors the render DPR so the pattern stays crisp on low-DPI screens." },
    { name: "className", type: "string", def: '"size-full"', desc: "Sizing classes for the canvas wrapper." },
  ];
        ...
        <div className="mt-10 overflow-hidden rounded-lg border border-border">
          {props.map((p, i) => (
            ...
            >
              ...
              <span className="font-mono text-[11px] text-ink-dim">{p.type}</span>
    ...
    <section className="border-b border-border bg-paper-2/40">
      <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24">
        ...
        <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-ink-dim">
          ...
          <code className="font-mono text-press">&lt;CTASection /&gt;</code> exactly as exported —
          no wrapper, no overrides.
        </p>
        <div className="mt-8">
          <DemoOne />
        </div>
