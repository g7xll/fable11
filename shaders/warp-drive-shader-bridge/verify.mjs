...
try {
	...
	check(
		"shader canvas + WebGL context present",
		gl.ok,
	...
	check(
		"warp throttle changes warpSpeed",
		!!s1 && !!s2 && s1 !== s2,
		`${s1} -> ${s2}`,
	);
	...
	for (const [name, sel] of [
		...
		[
			"shadcn/Tailwind/TS setup present",
			"text=/shadcn .* Tailwind .* TypeScript hull/i",
		],
