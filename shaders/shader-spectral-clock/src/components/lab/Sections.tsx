					...
					>
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<code className="font-mono text-[0.64rem] text-muted">
									{a.glsl}
								</code>
							</div>
						</div>
...
const API_ROWS: { name: string; type: string; def: string; note: string }[] = [
	{
		...
		type: "number",
		def: "8",
		note: "warp folds — the brief bakes 8 (capped at 16)",
	},
	{
		name: "warpScale",
		type: "number",
		def: "2.0",
		note: "domain zoom — the brief bakes `p *= 2.0`",
	},
	{
		name: "spectralSpread",
		type: "number",
		def: "50",
		note: "wavelength gain — the brief bakes `p.y * 50.0`",
	},
	{
		name: "speed",
		type: "number",
		def: "1.0",
		note: "multiplier on the `iTime` clock",
	},
	{
		name: "paused",
		type: "boolean",
		def: "false",
		note: "freeze the clock without unmounting",
	},
	{
		name: "onTelemetry",
		type: "(t) => void",
		def: "—",
		note: "per-frame { time, fps, rgb } off the GPU",
	},
	{
		name: "onReady",
		type: "(ok) => void",
		def: "—",
		note: "false when WebGL is unavailable",
	},
];
						...
						<table className="w-full border-collapse text-left text-sm">
							...
							<tbody className="font-mono text-[0.74rem]">
								{API_ROWS.map((r) => (
									<tr
										key={r.name}
										className="border-b border-edge/50 align-top"
									>
		...
		<section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
				...
				{[
					{
						n: "01",
						t: "shadcn structure",
						d: "components.json maps @/components/ui → src/components/ui",
					},
					{
						n: "02",
						t: "Tailwind CSS",
						d: "@tailwindcss/vite + a single @import in index.css",
					},
					{
						n: "03",
						t: "TypeScript",
						d: "strict tsconfig with the @/* path alias to ./src",
					},
				].map((s) => (
					<div
						key={s.n}
					...
					>
						...
						<div className="mt-1 text-[0.78rem] leading-relaxed text-muted">
							{s.d}
						</div>
					</div>
				))}
			...
			<CodeTabs
				tabs={[
					...
					{
						id: "component",
						label: "components/ui/shader-clock.tsx",
						lang: "tsx",
						code: SHADER_CLOCK_TSX,
					},
			...
			<div className="relative overflow-hidden rounded-3xl border border-edge bg-ink shadow-2xl">
				<div className="flex items-center gap-1.5 border-b border-edge bg-ink-2/80 px-4 py-2.5">
					...
					<span className="ml-3 flex items-center gap-1.5 font-mono text-[0.66rem] text-muted">
						<Code2 className="size-3" /> App.tsx — &lt;ShaderBackground/&gt; +
						&lt;TimeDisplay/&gt;
					</span>
				</div>
					...
					<div className="absolute inset-0 flex flex-col items-center justify-center">
						...
						{showInfo && (
							<div className="animate-fadeIn absolute bottom-8 left-1/2 -translate-x-1/2 rounded-lg bg-black/40 px-6 py-4 text-sm text-white/80 shadow-lg backdrop-blur-md">
								...
								<p className="mt-1">
									Click on the temperature to toggle between °C and °F.
								</p>
							</div>
						)}
					</div>
			...
			<div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-4 px-5 py-10 sm:flex-row sm:items-center sm:px-8">
				<div>
					<div className="text-lg font-bold">
						...
						<span className="text-paper/90">CLOCK</span>
					</div>
					<p className="mt-1 text-sm text-muted">
						A WebGL spectral-field timepiece for shadcn · React · TypeScript ·
						Tailwind.
					</p>
				</div>
