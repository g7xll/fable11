...
function category(metres: number) {
	if (metres < 50)
		return { label: "DENSE FOG", code: "CAT IIIb", hazard: true };
	if (metres < 200)
		return { label: "THICK FOG", code: "CAT IIIa", hazard: true };
		...
		<div className="glass relative overflow-hidden rounded-2xl p-5 sm:p-6">
			...
			<div className="relative mx-auto mt-1 w-full max-w-[260px]">
				...
				>
					...
					{/* end labels */}
					<text x="26" y="146" className="data" fontSize="8" fill="var(--ash)">
						1.5k
					</text>
					<text x="210" y="146" className="data" fontSize="8" fill="var(--ash)">
						0
					</text>
					{/* needle */}
					<line
						...
						strokeWidth="2.4"
						strokeLinecap="round"
						style={{ transition: "all 0.4s cubic-bezier(.4,1.2,.4,1)" }}
					/>
					<circle
						cx={cx}
						cy={cy}
				...
				<div className="-mt-6 text-center">
					<div
						className="data text-4xl font-bold tracking-tight"
						style={{ color: accent }}
					>
					...
					<div className="mt-1 flex items-center justify-center gap-2">
						<span
							className="data text-xs font-bold tracking-[0.2em]"
							style={{ color: accent }}
						>
							{cat.label}
						</span>
