...
function resolveExecutable() {
  ...
  const pick = (prefix, leaf) =>
    dirs
      ...
      .find((p) => existsSync(p));
  return pick("chromium-", "chrome") ?? pick("chromium_headless_shell-", "headless_shell");
}
...
try {
  ...
  if (/dithered|résumé|resume/i.test(h1)) ok(`hero headline renders ("${h1.slice(0, 40)}…")`);
  ...
  if (/Jessi\.cv/.test(proof) && /DESIGNER/.test(proof)) ok("verbatim <ResumePage> proof renders (Jessi.cv / DESIGNER)");
  ...
  const heroShot = await heroCanvas.screenshot({ timeout: 8000 }).catch(() => Buffer.alloc(0));
  if (heroShot.length > 1500) ok(`hero proof shader paints ink (${heroShot.length}B PNG)`);
  ...
  if (/0\.00×/.test(minTxt) && /1\.00×/.test(maxTxt)) ok("speed fader sweeps 0.00× → 1.00×");
  ...
  else bad(`console telemetry not reflecting state (snippet: "${tele.replace(/\s+/g, " ").slice(0, 80)}")`);
  ...
  if (isLight && backToDark) ok("studio theme toggle flips darkroom ↔ studio (.light)");
  else bad(`theme toggle failed (light=${isLight} backToDark=${backToDark})`);

  // 10. Story sections present
  const body = await page.locator("body").innerText();
  const sections = ["ink console", "integration", "source", "api", "applied in context"];
  const missing = sections.filter((s) => !new RegExp(s, "i").test(body));
  if (missing.length === 0) ok("console / context / integration / source / api sections render");
