...
const SPECIMENS: Specimen[] = [
  {
    ...
    params: { complexity: 4.0, colorShift: 0.3, lightIntensity: 1.5, mouseInfluence: 0.5 },
    swatch: "#74efd6",
  },
  {
    id: "VG-02",
    ...
    params: { complexity: 2.4, colorShift: 0.62, lightIntensity: 2.1, mouseInfluence: 0.85 },
    swatch: "#ff8a5c",
  },
  {
    id: "FZ-03",
    ...
    params: { complexity: 8.6, colorShift: 0.14, lightIntensity: 1.1, mouseInfluence: 0.32 },
    swatch: "#c4a2ff",
  },
  {
    id: "GL-04",
    ...
    params: { complexity: 5.7, colorShift: 0.08, lightIntensity: 0.8, mouseInfluence: 0.62 },
    swatch: "#8fd0ff",
  },
];
...
export default function App() {
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-void text-frost">
          ...
          <div className="grid h-full w-full place-items-center bg-void px-6 text-center">
            <div className="max-w-sm">
              ...
              <p className="mt-3 text-sm text-steel">
                This browser has no WebGL context, so the specimen can't be lit. The bench
                instrumentation around it is still live.
              </p>
            </div>
          </div>
        )}
      ...
      >
        <div>
          ...
          <p className="mt-2 max-w-xs text-[13px] leading-snug text-steel">
            A ray-marched specimen under live lighting. Move the cursor to walk the key light
            across its facets.
          </p>
        </div>

        <div className="hidden text-right sm:block">
          <p className="font-mono text-[10px] uppercase tracking-widest2 text-dim">Mount</p>
          <p className="mt-1 font-mono text-xs text-steel">@/components/ui/crystalline-cube</p>
          ...
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-dim">
            {current ? `${current.id} · ${current.classification}` : "manual calibration"}
          </p>
        </div>
      </header>
      ...
      >
        <p className="mb-1 font-mono text-[10px] uppercase tracking-widest2 text-dim">Catalog</p>
        {SPECIMENS.map((s) => {
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block truncate font-body text-sm text-frost">{s.name}</span>
      ...
      >
        <div className="flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-seam/70 pt-4">
          ...
          <Readout icon={Hexagon} label="Facets" value={`${facetCount}`} unit="ct" />
          <Readout
            icon={Sparkles}
            label="Refr. index"
    ...
    <div className={cn("flex items-center gap-2.5", wide && "min-w-[7.5rem]")}>
      ...
      <div className="leading-none">
        ...
        <div className="mt-1 font-mono text-sm tabular-nums text-frost">
          {value}
          {unit ? <span className="ml-1 text-[10px] text-dim">{unit}</span> : null}
        </div>
      </div>
    </div>
  );
...
function Reticle() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10">
      {corners.map((c) => (
        <span key={c} className={cn("absolute h-6 w-6 border-refraction/45", c)} />
      ))}
    </div>
  );
}
