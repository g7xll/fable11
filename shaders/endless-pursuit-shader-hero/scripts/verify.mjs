...
// ---- STATIC PHASE -----------------------------------------------------------
ok("index.html ships hashed module + stylesheet", /<script[^>]+type="module"/.test(html) && /<link[^>]+stylesheet/.test(html));
ok("index.html preloads vendored mono font", /rel="preload"[^>]+jetbrains-mono-latin-400-normal\.woff2/.test(html));

for (const w of [400, 500, 700]) {
  const f = join(DIST, "fonts", `jetbrains-mono-latin-${w}-normal.woff2`);
  ok(`vendored font weight ${w} emitted (>5kb)`, existsSync(f) && statSync(f).size > 5000);
}
...
ok("UnicornStudio embed preserved", js.includes("data-us-project") && js.includes("OMzqyUv6M3kSnv0JeAtC"));
ok("UnicornStudio loader script preserved", js.includes("unicornstudio.js") || js.includes("UnicornStudio"));
ok("offline WebGL fallback shader bundled (GLSL)", js.includes("gl_FragColor") && js.includes("fbm"));
ok("WebGL context guarded (no-crash fallback)", js.includes("experimental-webgl") || js.includes('getContext("webgl"') || js.includes("getContext('webgl'"));
...
ok("@font-face → JetBrains Mono in CSS", /@font-face/.test(css) && /JetBrains Mono/.test(css));
...
try {
  ...
  if (chrome && playwright) {
    ...
    ok("live: headline 'ENDLESS PURSUIT' rendered", (await page.getByRole("heading", { level: 1 }).innerText()).includes("ENDLESS PURSUIT"));
    ok("live: backdrop <canvas> mounted full-viewport", await page.evaluate(() => {
      const c = document.querySelector("canvas");
      return !!c && c.getBoundingClientRect().width > 1000;
    }));
    const png = await page.screenshot({ clip: { x: 0, y: 120, width: 1280, height: 400 } });
