      ...
      <aside className="animate-rise fixed bottom-7 left-7 z-10 w-[270px] [animation-delay:380ms] sm:left-auto sm:right-7">
        <div className="rounded-2xl border border-line bg-glass p-5 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                aria-label={
                  interactive ? "Disable cursor interaction" : "Enable cursor interaction"
                }
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 border-t border-line pt-4">
            <Stat icon={<Layers3 className="h-3 w-3" />} k="layers" v={String(layers)} />
            <Stat icon={<Sparkles className="h-3 w-3" />} k="glow" v={`${intensity.toFixed(1)}`} />
            <Stat icon={<Gauge className="h-3 w-3" />} k="speed" v={`${speed.toFixed(1)}`} />
          </div>
        </div>
      </aside>
