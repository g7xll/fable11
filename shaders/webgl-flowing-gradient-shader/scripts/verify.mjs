...
const fpsLine = page.locator("dt", { hasText: "fps" }).locator("xpath=following-sibling::dd[1]");
const timeText = () =>
  page.evaluate(() => {
...
const interactiveBtn = page.getByRole("button", { name: /cursor interaction/i });
...
check("Interactive toggle flips state", before !== after, `${before} -> ${after}`);
...
check("Mobile: canvas present", (await mobile.locator("canvas.shader-canvas").count()) === 1);
check(
