...
try {
	...
	!beforeShot.equals(afterShot) && /crimson/i.test(heading || "")
		...
		: fail(
				"Preset re-renders",
				`pixelsChanged=${!beforeShot.equals(afterShot)} heading=${heading}`,
			);
	...
	!dialBefore.equals(dialAfter)
		...
		: fail(
				"Cloud-density dial drives the shader",
				"scope unchanged after slider move",
			);
	...
	fontReqs.size >= 3
		...
		: fail(
				...
				`only saw ${[...fontReqs].join(", ") || "none"}`,
			);
	starReq
		? ok("Vendored starfield texture loaded")
		: fail("Vendored starfield texture loaded", "stardust.png missing");
