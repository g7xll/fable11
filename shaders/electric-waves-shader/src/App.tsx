  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (now - last >= 500) {
        ...
        frames = 0;
        last = now;
        if (fpsRef.current) fpsRef.current.textContent = String(fps).padStart(2, "0");
        if (barRef.current)
          barRef.current.style.width = `${Math.min(100, (fps / 60) * 100)}%`;
      }
      if (timeRef.current)
        timeRef.current.textContent = ((now - start) / 1000).toFixed(1).padStart(5, "0");
    ...
    <main className="relative h-screen w-screen overflow-hidden bg-ink text-paper">
      ...
      <section className="pointer-events-none fixed left-7 top-24 z-10 max-w-xl sm:left-10 sm:top-1/2 sm:-translate-y-1/2">
        ...
        <p className="animate-rise mt-5 hidden max-w-md text-sm leading-relaxed text-haze [animation-delay:300ms] sm:block sm:text-base">
          Stacked sine fields fold across one full-screen quad and pile into the red,
          green and blue channels — additive interference, no textures, no video. Tune
          the field from the deck below.
        </p>
      </section>
