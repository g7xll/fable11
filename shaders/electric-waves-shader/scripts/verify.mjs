...
check(
	"Electric field animates (frames differ)",
	f1.lum !== f2.lum,
	`${f1.lum} -> ${f2.lum}`,
);
...
check(
	"Five shader controls present",
	(await sliders.count()) === 5,
	`${await sliders.count()}`,
);
...
const beforeVal = (await waveValue.count())
	? (await waveValue.first().textContent())?.trim()
	: "";
await sliders.first().fill("12");
await page.waitForTimeout(150);
const afterVal = (await waveValue.count())
	? (await waveValue.first().textContent())?.trim()
	: "";
check(
