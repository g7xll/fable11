...
/* ---------------- Desktop (1440x900) ---------------- */
{
  ...
  check("--background is 260 87% 3%", vars.background === "260 87% 3%", vars.background)
  check("--foreground is 40 6% 95%", vars.foreground === "40 6% 95%", vars.foreground)
  check("--hero-sub is 40 6% 82%", vars.heroSub === "40 6% 82%", vars.heroSub)
  check("body font is Geist Sans", vars.bodyFont.includes("Geist Sans"), vars.bodyFont)
  ...
  check("video autoplay + muted + playsInline", v.autoplay && v.muted && v.playsInline)
  check("video has NO native loop (custom JS loop)", v.loopAttr === false)
  check("video absolute + object-cover", v.position === "absolute" && v.objectFit === "cover")
  ...
  check("wrapper div has overflow-hidden", wrapperOverflow === "hidden", wrapperOverflow)
  ...
  const blur = await page.evaluate(() => {
    ...
    const el = els.find((e) => getComputedStyle(e).filter.includes("blur(82px)"))
    if (!el) return null
    const cs = getComputedStyle(el)
    const r = el.getBoundingClientRect()
    return {
  ...
  if (blur) {
    check("blur shape is 984x527", approx(blur.w, 984) && approx(blur.h, 527), `${blur.w}x${blur.h}`)
    check("blur shape opacity 0.9 + pointer-events none", blur.opacity === "0.9" && blur.pointerEvents === "none")
    check("blur shape centered", approx(blur.cx, 720, 2) && approx(blur.cy, 450, 2), `${blur.cx},${blur.cy}`)
  }
  const sectionOverflow = await page.evaluate(
    () => getComputedStyle(document.querySelector("section")).overflow,
  )
  check("hero section overflow-visible", sectionOverflow === "visible", sectionOverflow)

  /* Navbar */
  const logoImg = page.locator("nav img")
  check("navbar logo image present", (await logoImg.count()) === 1)
  const logoH = await logoImg.evaluate((el) => el.getBoundingClientRect().height)
  check("logo height is 32px", approx(logoH, 32), `${logoH}px`)
  for (const item of ["Features", "Solutions", "Plans", "Learning"]) {
    check(`nav item "${item}"`, (await page.locator("nav button", { hasText: item }).count()) === 1)
  }
  ...
  check("Sign Up is liquid-glass heroSecondary pill", su.liquid && su.radius === "9999px" && su.backdrop === "blur(4px)", JSON.stringify(su))
  const divider = await page.evaluate(() => {
    const el = document.querySelector("header > div:last-child")
    const cs = getComputedStyle(el)
    return { h: el.getBoundingClientRect().height, bg: cs.backgroundImage, mt: cs.marginTop }
  })
  check("navbar divider 1px gradient, mt 3px", approx(divider.h, 1, 0.1) && divider.bg.includes("linear-gradient") && divider.mt === "3px", `${divider.h}px / ${divider.mt}`)

  /* Headline */
  const h1 = page.locator("h1")
  await h1.waitFor()
  check("headline reads Power AI", ((await h1.textContent()) ?? "").replace(/\s+/g, " ").trim() === "Power AI")
  ...
  check("headline leading-[1.02]", approx(hs.lineHeight, 224.4, 0.5), `${hs.lineHeight}px`)
  check("headline tracking -0.024em", approx(hs.letterSpacing, -5.28, 0.05), `${hs.letterSpacing}px`)
  check("headline font is General Sans", hs.fontFamily.includes("General Sans"), hs.fontFamily)
  const ai = await page.evaluate(() => {
    const span = [...document.querySelectorAll("h1 span")].find((s) => s.textContent === "AI")
  ...
  check("AI span uses bg-clip-text + transparent", ai.clip === "text" && ai.color === "rgba(0, 0, 0, 0)", `${ai.clip} / ${ai.color}`)
  check(
    "AI gradient is to-left indigo→purple→amber",
    ai.bg.includes("to left") && ai.bg.includes("99, 102, 241") && ai.bg.includes("168, 85, 247") && ai.bg.includes("252, 211, 77"),
    ai.bg,
  )

  /* Subtitle */
  const sub = page.locator("p", { hasText: "The most powerful AI ever deployed" })
  check("subtitle present", (await sub.count()) === 1)
  const ss = await sub.evaluate((el) => {
    const cs = getComputedStyle(el)
    return { fontSize: cs.fontSize, lineHeight: cs.lineHeight, opacity: cs.opacity, color: cs.color, maxWidth: cs.maxWidth }
  })
  check("subtitle text-lg leading-8 opacity-80 max-w-md", ss.fontSize === "18px" && ss.lineHeight === "32px" && ss.opacity === "0.8" && ss.maxWidth === "448px", JSON.stringify(ss))
  check("subtitle uses hero-sub color", ss.color === "rgb(212, 210, 206)", ss.color)
  ...
  const cs2 = await cta.evaluate((el) => {
    const cs = getComputedStyle(el)
    return { pl: cs.paddingLeft, pt: cs.paddingTop, liquid: el.classList.contains("liquid-glass"), radius: cs.borderRadius }
  })
  check("CTA px-[29px] py-[24px] heroSecondary pill", cs2.pl === "29px" && cs2.pt === "24px" && cs2.liquid && cs2.radius === "9999px", JSON.stringify(cs2))
  ...
  for (const b of BRANDS) {
    ...
    check(`brand "${b}" duplicated for seamless loop`, n === 2, `${n} instances`)
  }
  ...
  check("marquee 20s linear infinite", mq.name === "marquee" && mq.duration === "20s" && mq.timing === "linear" && mq.iteration === "infinite", JSON.stringify(mq))
  check("marquee icons are 24x24 liquid-glass rounded-lg", approx(mq.iconW, 24) && approx(mq.iconH, 24) && mq.iconRadius === "8px" && mq.iconBackdrop === "blur(4px)", `${mq.iconW}x${mq.iconH} r=${mq.iconRadius}`)
