...
try {
  ...
  check("page title", (await page.title()).includes("Mindloop"), await page.title());
  ...
  check(
    ...
    ["Home", "How It Works", "Philosophy", "Use Cases"].every((l) => navLinks.includes(l)),
    navLinks.join(", "),
  );
  const socialCount = await page.locator("header a.liquid-glass").count();
  check("navbar has 3 liquid-glass social buttons", socialCount === 3, `count=${socialCount}`);
  const headerPos = await page.evaluate(() => getComputedStyle(document.querySelector("header")).position);
  ...
  const heroVideo = await page.evaluate(() => {
    ...
    return v
      ? { src: v.src, muted: v.muted, loop: v.loop, autoplay: v.autoplay, readyState: v.readyState, paused: v.paused }
      : null;
  });
  check(
    "hero MP4 background video configured (muted/loop/autoplay)",
    !!heroVideo && heroVideo.src.includes("hf_20260325_120549") && heroVideo.muted && heroVideo.loop && heroVideo.autoplay,
  ...
  check(
    "social proof line",
    (await page.locator("#home").textContent()).includes("7,000+ people already subscribed"),
  );
  ...
  const btnText = await page.locator("#home form button[type=submit]").textContent();
  check("subscribe interaction works", btnText.includes("SUBSCRIBED"), btnText.trim());

  await page.screenshot({ path: "screenshots/01-hero.png" });

  // ——— search has changed ———
  const searchHeading = (await page.locator("#how-it-works h2").textContent()).replace(/\s+/g, " ").trim();
  check("search section heading", searchHeading.includes("Search has changed.") && searchHeading.includes("Have you?"), searchHeading);

  const platformNames = await page.$$eval("#how-it-works h3", (hs) => hs.map((h) => h.textContent.trim()));
  check(
    "3 platform cards (ChatGPT / Perplexity / Google AI)",
    ["ChatGPT", "Perplexity", "Google AI"].every((n) => platformNames.includes(n)),
    platformNames.join(", "),
  );
  ...
  check(
    ...
    platformIcons.length === 3 && platformIcons.every((i) => i.w > 0 && i.rendered === 200),
    JSON.stringify(platformIcons),
  );
  check(
    "bottom tagline",
    (await page.locator("#how-it-works").textContent()).includes("If you don't answer the questions, someone else will."),
  );
  ...
  const features = await page.$$eval("#use-cases h3", (hs) => hs.map((h) => h.textContent.trim()));
  check(
    "4 feature columns",
    ["Curated Feed", "Writer Tools", "Community", "Distribution"].every((f) => features.includes(f)),
    features.join(", "),
  );
  ...
  const ctaVideo = await page.evaluate(() => {
    ...
    return v ? { src: v.src, readyState: v.readyState, paused: v.paused } : null;
  });
  check(
  ...
  check(
    ...
    ctaText.includes("Start Your Journey") && ctaText.includes("Subscribe Now") && ctaText.includes("Start Writing"),
  );
  ...
  check("liquid-glass elements present (nav x3, hero form, CTA button)", glassCount >= 5, `count=${glassCount}`);
  ...
  check("monochrome audit (no saturated colors in computed styles)", offenders.length === 0, offenders.join(" | "));
  ...
  check("no console errors (network noise excluded)", realConsoleErrors.length === 0, realConsoleErrors.join(" | "));
  if (consoleErrors.length) console.log("ℹ️ network console noise:", consoleErrors.slice(0, 3));
} finally {
  await browser.close();
  server.kill("SIGTERM");
}
