...
// 1B. Sub-nav
check("sub-nav Natura/History/Museum", await page.getByText("Natura", { exact: true }).isVisible());
...
// 1E. Hero copy + CTA
check("TIMELESS WONDERS headline", await page.getByText("TIMELESS").isVisible());
check("Explore Now CTA", await page.getByRole("button", { name: /Explore Now/ }).isVisible());

// 1F. Right sidebar
check("Tyrannosaurus Rex specimen", await page.getByText("Tyrannosaurus Rex").isVisible());
...
check("Scroll to explore", await page.getByText("Scroll to explore").isVisible());
...
check("video absent before 2.8s, present after", videoBefore === 0 && videoAfter === 1,
  `before=${videoBefore} after=${videoAfter}`);
const videoSrc = await page.locator("video").getAttribute("src");
check("video src is cloudinary mp4", /res\.cloudinary\.com.+\.mp4$/.test(videoSrc || ""));
const videoPlaying = await page
  .locator("video")
  .evaluate((v) => !v.paused && v.readyState >= 2)
  .catch(() => false);
check("video is playing (autoplay muted)", videoPlaying, `playing=${videoPlaying}`);

// Section 2
check("[ 02 ] section label", await page.getByText("Explore Our World").first().isVisible());
await page.getByText("Unearth the stories").scrollIntoViewIfNeeded();
await page.waitForTimeout(1200);
check("section 2 heading revealed", await page.getByText("Unearth the stories").isVisible());
for (const pill of ["Dinosaurs", "Ancient Life", "Minerals", "Fossils", "Learn More"]) {
  check(`pill "${pill}"`, (await page.locator(`button:has-text("${pill}")`).count()) >= 1);
}
check("WE DON'T JUST TELL STORIES.", (await page.getByText("WE DON'T JUST TELL STORIES.").count()) === 1);
check("PALEONTOLOGY (C) 2026", (await page.getByText("PALEONTOLOGY (C) 2026").count()) === 1);

// Section 3
await page.getByText("Curated from millions").scrollIntoViewIfNeeded();
await page.waitForTimeout(1800);
check("section 3 heading", await page.getByText("Curated from millions").isVisible());
...
check("pterodactyl revealed (opacity → 1)", Number(pteroOpacity) > 0.9, `opacity=${pteroOpacity}`);
for (const pill of ["Educational", "Authentic", "Inspiring"]) {
  check(`dark pill "${pill}"`, (await page.locator(`button:has-text("${pill}")`).count()) === 1);
}

// 3C. chapters
const chapterNames = [
  "Age of Dinosaurs", "Fossils of Ancient Life", "Reptiles of the Mesozoic",
  "Marine Fossil Gallery", "Prehistoric Giants",
];
for (const name of chapterNames) {
  check(`chapter "${name}"`, (await page.locator(`button:has-text("${name}")`).count()) === 1);
}
...
check("sand image uses svg filter", /url\(["']?#sand-filter/.test(sandFilter), sandFilter);
check("feTurbulence present", (await page.locator("feTurbulence").count()) >= 1);
check("feDisplacementMap present", (await page.locator("feDisplacementMap").count()) >= 1);
...
const activeClass = await page
  ...
  .getAttribute("class");
check("clicking chapter activates it", /text-white/.test(activeClass || ""), "has text-white");
...
const activeAfterCycle = await page
  .locator('button:has-text("Age of Dinosaurs")')
  .getAttribute("class");
check("auto-cycle advances chapter", !/text-white/.test(activeAfterCycle || ""));

// Footer
check("DIGGING INTO OUR PLANET'S PAST", await page.getByText("DIGGING INTO OUR PLANET'S PAST").isVisible());

await page.screenshot({ path: path.join(here, "verify-desktop.png"), fullPage: true });
await page.close();
...
check("mobile: hamburger visible", await mob.getByRole("button", { name: "Open menu" }).isVisible());
check("mobile: desktop nav hidden", !(await mob.locator("header nav").first().isVisible()));
await mob.getByRole("button", { name: "Open menu" }).click();
await mob.waitForTimeout(600);
const visibleVisitOpen = await mob.locator('header a:has-text("Visit"):visible').count();
check("mobile: menu opens with links", visibleVisitOpen === 1, `visible=${visibleVisitOpen}`);
await mob.getByRole("button", { name: "Close menu" }).click();
await mob.waitForTimeout(600);
const visibleVisitClosed = await mob.locator('header a:has-text("Visit"):visible').count();
check("mobile: menu closes", visibleVisitClosed === 0, `visible=${visibleVisitClosed}`);
check("mobile: right sidebar hidden", !(await mob.getByText("Tyrannosaurus Rex").isVisible()));
await mob.screenshot({ path: path.join(here, "verify-mobile.png"), fullPage: false });
await mob.close();
...
const realConsoleErrors = consoleErrors.filter((e) => !/net::|Failed to load resource/i.test(e));
console.log(`page JS errors: ${pageErrors.length}`);
console.log(`console errors (non-network): ${realConsoleErrors.length}`);
if (consoleErrors.length) console.log("console (all):", consoleErrors.slice(0, 5));
