...
try {
  ...
  ({ chromium } = require(
    "../../scripts/record-demos/node_modules/playwright/index.js",
  ));
}
...
async function main() {
  ...
  assert(resp && resp.status() === 200, "page returns HTTP 200", resp && resp.status());
  ...
  const counts = await page.evaluate(() => ({
    ...
    source: document.querySelector("video.hero__video source")?.getAttribute("src") || "",
    ...
    headingLines: document.querySelectorAll(".hero__heading .hero__line").length,
    ...
    cardImg: document.querySelector(".about-card__image img")?.getAttribute("src") || "",
  ...
  assert(/body\.mp4/.test(counts.source), "video sourced from vendored body.mp4", counts.source);
  assert(counts.blurLayers === 8, "8 progressive-blur layers", counts.blurLayers);
  ...
  assert(counts.activeSide === 1, "one active side-nav link", counts.activeSide);
  assert(counts.btnHeader === 1, '"Book a call" header button', counts.btnHeader);
  assert(counts.btnFooter === 1, '"Discuss the project" footer button', counts.btnFooter);
  ...
  assert(/card-image\.png/.test(counts.cardImg), "about-card uses vendored card-image.png", counts.cardImg);
  assert(counts.label.includes("01 — Our goal"), 'label is "01 — Our goal"', counts.label);
  ...
  assert(reveal.charCount > 30, "headline split into many .hero__char spans", reveal.charCount);
  assert(reveal.labelChars > 0, "label split into .hero__char spans", reveal.labelChars);
  assert(reveal.descChars > 0, "description split into .hero__char spans", reveal.descChars);
  assert(reveal.firstDelay !== "", "chars carry staggered animationDelay", reveal.firstDelay);
  ...
  assert(charOpacity > 0.9, "headline chars are revealed (opacity ~1)", charOpacity);
  ...
  const sideState = await page.evaluate(() => {
    ...
    return { activeIdx, lineIdx, lineCount: document.querySelectorAll(".side-nav__line").length };
  });
  assert(sideState.activeIdx === 2, "clicked side-nav link becomes active", sideState.activeIdx);
  assert(sideState.lineIdx === 2, "active underline marker follows the click", sideState.lineIdx);
  assert(sideState.lineCount === 1, "only one underline marker exists", sideState.lineCount);
  ...
  assert(consoleErrors.length === 0, "no console errors", consoleErrors.slice(0, 3));
