  ...
  return (
    <header className="relative z-20 w-full bg-transparent">
      <div className="animate-fade flex w-full items-center px-6 py-[16px] lg:px-[120px]">
        ...
        <nav className="ml-12 hidden items-center gap-8 lg:flex" aria-label="Primary">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.label}
              href="#"
            ...
            >
              {item.label}
              {item.hasChevron && <ChevronDown size={16} strokeWidth={2} aria-hidden="true" />}
            </a>
          ))}
        </nav>
        ...
        <div className="animate-fade fixed inset-0 z-50 flex flex-col bg-black lg:hidden">
          ...
          <nav className="mt-10 flex flex-col gap-2 px-6" aria-label="Mobile">
            {NAV_ITEMS.map((item, i) => (
              ...
              >
                {item.label}
                {item.hasChevron && <ChevronDown size={22} aria-hidden="true" />}
              </a>
            ))}
          </nav>

          <div className="animate-rise mt-auto flex flex-col gap-3 px-6 pb-10" style={{ animationDelay: '360ms' }}>
            <button
              type="button"
