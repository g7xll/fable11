...
check(
  ...
  await desktop.locator('main').evaluate((m) => m.getBoundingClientRect().height >= window.innerHeight),
)

// Fonts loaded
await desktop.evaluate(() => document.fonts.ready)
for (const font of ['Manrope', 'Cabin', 'Instrument Serif', 'Inter']) {
  check(`Font loaded: ${font}`, await desktop.evaluate((f) => document.fonts.check(`16px "${f}"`), font))
}

// Navbar
const header = desktop.locator('header')
check('Navbar is z-20 + transparent', await header.evaluate((h) => {
...
check(
  ...
  await desktop.evaluate(() => {
    ...
    return p?.getAttribute('d')?.startsWith('M1.04356 6.35771L13.6437 0.666504') && p?.getAttribute('fill') === 'white'
  }),
)
for (const link of ['Home', 'Services', 'Reviews', 'Contact us']) {
  const el = desktop.locator(`header nav[aria-label="Primary"] a:has-text("${link}")`)
  check(`Nav link "${link}" visible`, await el.isVisible())
}
check(
  'Nav links: Manrope 14px medium white',
  await desktop.evaluate(() => {
    const a = [...document.querySelectorAll('header nav[aria-label="Primary"] a')][0]
...
check(
  ...
  (await desktop.locator('header nav[aria-label="Primary"] a:has-text("Services") svg').count()) === 1,
)

const signIn = desktop.locator('header > div button', { hasText: 'Sign In' })
const getStarted = desktop.locator('header > div button', { hasText: 'Get Started' })
check('"Sign In" visible, white bg, #d4d4d4 border, 8px radius, #171717 text', await signIn.evaluate((b) => {
...
check('"Get Started" visible, #7b39fc bg, 8px radius, #fafafa text, shadow', await getStarted.evaluate((b) => {
  const s = getComputedStyle(b)
  return (
...
check('Hamburger hidden on desktop', !(await desktop.locator('button[aria-label="Open menu"]').isVisible()))

// Tagline pill
const pill = desktop.locator('section > div').first()
check('Pill: 38px tall, 10px radius, glass bg + blur + border', await pill.evaluate((el) => {
...
check('Pill badge: "New", #7b39fc bg, 6px radius', await desktop.evaluate(() => {
  const badge = [...document.querySelectorAll('section span')].find((el) => el.textContent === 'New')
  if (!badge) return false
  const s = getComputedStyle(badge)
  return s.backgroundColor === 'rgb(123, 57, 252)' && s.borderRadius === '6px'
}))
check(
  'Pill text "Say Hello to Datacore v3.2" in Cabin 14px',
  await desktop.evaluate(() => {
    const t = [...document.querySelectorAll('section span')].find((el) =>
      el.textContent === 'Say Hello to Datacore v3.2',
    )
    if (!t) return false
    const s = getComputedStyle(t)
    return s.fontFamily.includes('Cabin') && s.fontSize === '14px' && s.fontWeight === '500'
  }),
)
...
check(
  'H1 text matches',
  (await h1.innerText()).replace(/\s+/g, ' ').trim() === 'Book your perfect stay instantly and hassle-free',
)
check('H1: Instrument Serif, 96px on desktop, line-height 1.1, white', await h1.evaluate((el) => {
  const s = getComputedStyle(el)
  return (
...
check('"and" is italicized with spacing', await desktop.evaluate(() => {
  const em = document.querySelector('h1 em')
  if (!em || em.textContent !== 'and') return false
  const s = getComputedStyle(em)
  return s.fontStyle === 'italic' && parseFloat(s.marginLeft) > 0 && parseFloat(s.marginRight) > 0
}))

// Subtext
const sub = desktop.locator('section p')
check('Subtext: Inter 18px normal, white/70, max-w 662px', await sub.evaluate((el) => {
  const s = getComputedStyle(el)
  return (
...
const startNow = desktop.locator('section button', { hasText: 'Get Started Now' })
check('CTA 1: #7b39fc, 10px radius, Cabin 16px medium, white', await demo.evaluate((b) => {
  const s = getComputedStyle(b)
  return (
    s.backgroundColor === 'rgb(123, 57, 252)' &&
    s.borderRadius === '10px' &&
...
check('CTA 2: #2b2344, 10px radius, Cabin 16px medium, #f6f7f9', await startNow.evaluate((b) => {
  const s = getComputedStyle(b)
  return (
...
check('CTA 1 lightens on hover', await demo.evaluate(async (b) => {
...
check(
  ...
  (await demo.evaluate((b) => getComputedStyle(b).backgroundColor)) === 'rgb(141, 83, 255)',
)

// Hero container alignment
check('Hero content: centered column, text-center, z-10, mt-32', await desktop.locator('section').evaluate((el) => {
  const s = getComputedStyle(el)
  return (
    s.display === 'flex' &&
...
check('Mobile: desktop nav hidden', !(await mobile.locator('header nav[aria-label="Primary"]').isVisible()))
check('Mobile: desktop buttons hidden', !(await mobile.locator('header > div button', { hasText: 'Sign In' }).isVisible()))
const burger = mobile.locator('button[aria-label="Open menu"]')
check('Mobile: hamburger visible + white', await burger.isVisible()
  && (await burger.evaluate((b) => getComputedStyle(b).color === 'rgb(255, 255, 255)')))
check(
  'Mobile: H1 is text-5xl (48px)',
  (await mobile.locator('h1').evaluate((el) => getComputedStyle(el).fontSize)) === '48px',
)
...
check('Mobile: overlay opens, full-screen black', await overlay.evaluate((el) => {
  const s = getComputedStyle(el)
...
for (const link of ['Home', 'Services', 'Reviews', 'Contact us']) {
  check(`Mobile overlay link "${link}"`, await mobile.locator(`nav[aria-label="Mobile"] a:has-text("${link}")`).isVisible())
}
check('Mobile overlay: body scroll locked', await mobile.evaluate(() => document.body.style.overflow === 'hidden'))
await mobile.locator('button[aria-label="Close menu"]').click()
await mobile.waitForTimeout(300)
check('Mobile: overlay closes', (await overlay.count()) === 0)
check('Mobile: body scroll restored', await mobile.evaluate(() => document.body.style.overflow === ''))
