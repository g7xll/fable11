    ...
    <header className="nav reveal" style={{ '--reveal-order': 0 }}>
      <div className="nav__glass">
        ...
        <nav className="nav__links" aria-label="Primary">
          {LINKS.map((label) => (
            <a key={label} className="nav__link" href={`#${label.toLowerCase()}`}>
              {label}
            </a>
          ))}
        </nav>
      ...
      <div id="mobile-menu" className={`nav__sheet${open ? ' is-open' : ''}`}>
        ...
        <a className="nav__sheet-link nav__sheet-link--cta" style={{ '--sheet-order': LINKS.length }} href="#start" onClick={() => setOpen(false)}>
