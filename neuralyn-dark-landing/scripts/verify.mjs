...
try {
  ...
  check("logo image rendered", await page.locator("header img[alt='Neuralyn logo']").isVisible());
  for (const link of ["Home", "Services", "Reviews", "Contact us"]) {
    check(`nav link "${link}"`, await page.locator(`nav >> text=${link}`).isVisible());
  }
  check(
    "Services has ChevronDown icon",
    (await page.locator("nav a", { hasText: "Services" }).locator("svg").count()) === 1,
  );
  ...
  check("Sign In solid white bg", signInStyle.bg === "rgb(255, 255, 255)", signInStyle.bg);
  check("Sign In black text", signInStyle.color === "rgb(0, 0, 0)", signInStyle.color);
  check("Sign In rounded-lg (8px)", signInStyle.radius === "8px", signInStyle.radius);

  console.log("\n— Hero content —");
  const pill = page.locator(".liquid-glass");
  check("liquid-glass pill exists", (await pill.count()) === 1);
  check("pill 'New' badge", await pill.locator("span", { hasText: "New" }).first().isVisible());
  check(
    "pill Corewave text",
  ...
  const pillStyle = await pill.evaluate((el) => {
    const s = getComputedStyle(el);
    return { blur: s.backdropFilter || s.webkitBackdropFilter, radius: s.borderRadius };
  });
  check("pill backdrop blur(4px)", pillStyle.blur.includes("blur(4px)"), pillStyle.blur);
  ...
  const serifSpan = await h1.locator("span").first().evaluate((el) => {
    const s = getComputedStyle(el);
    return { family: s.fontFamily, style: s.fontStyle, weight: s.fontWeight };
  });
  check(
    "'Overview' in Instrument Serif italic",
    serifSpan.family.includes("Instrument Serif") && serifSpan.style === "italic",
    JSON.stringify(serifSpan),
  );
  const h1Style = await h1.evaluate((el) => {
    const s = getComputedStyle(el);
    return { tracking: s.letterSpacing, size: s.fontSize, weight: s.fontWeight, opacity: s.opacity };
  });
  check("title tracking -2px", h1Style.tracking === "-2px", h1Style.tracking);
  check("title text-7xl at desktop (72px)", h1Style.size === "72px", h1Style.size);
  check("title font-medium", h1Style.weight === "500", h1Style.weight);
  check("title entrance animation completed", h1Style.opacity === "1", h1Style.opacity);
  ...
  check(
    ...
    JSON.stringify({ class: subStyle.hasOpacityClass, opacity: subStyle.opacity }),
  );
  ...
  check("CTA solid white, black text", ctaStyle.bg === "rgb(255, 255, 255)" && ctaStyle.color === "rgb(0, 0, 0)");
  ...
  check(
    ...
    videoStyle.fit === "cover" && videoStyle.muted && videoStyle.autoplay && videoStyle.loop && videoStyle.playsInline,
  ...
  check("dashboard area 16/9", areaStyle.aspect.replace(/\s/g, "") === "16/9", areaStyle.aspect);
  check(
  ...
  const dashStyle = await dashImg.evaluate((el) => {
    const s = getComputedStyle(el);
    return { blend: s.mixBlendMode, radius: s.borderRadius, naturalWidth: el.naturalWidth };
  });
  check("dashboard image luminosity blend", dashStyle.blend === "luminosity", dashStyle.blend);
  check("dashboard image rounded-2xl (16px)", dashStyle.radius === "16px", dashStyle.radius);
  check("dashboard image loaded", dashStyle.naturalWidth > 0, String(dashStyle.naturalWidth));
  const fade = page.locator("[data-testid='hero-fade']");
  const fadeStyle = await fade.evaluate((el) => {
    const s = getComputedStyle(el);
    return { bg: s.backgroundImage, z: s.zIndex, pe: s.pointerEvents, h: s.height };
  });
  check(
    "bottom gradient fade (linear-gradient, z-30, pointer-events-none, h-40)",
    fadeStyle.bg.includes("linear-gradient") && fadeStyle.z === "30" && fadeStyle.pe === "none" && fadeStyle.h === "160px",
  ...
  const tyBefore = matrixTY(await heroContent.evaluate((el) => getComputedStyle(el).transform));
  const dashTyBefore = matrixTY(await dashImg.evaluate((el) => getComputedStyle(el).transform));
  await page.evaluate(() => window.scrollTo({ top: 450, behavior: "instant" }));
  await page.waitForTimeout(450);
  const tyAfter = matrixTY(await heroContent.evaluate((el) => getComputedStyle(el).transform));
  const opacityAfter = parseFloat(await heroContent.evaluate((el) => getComputedStyle(el).opacity));
  const dashTyAfter = matrixTY(await dashImg.evaluate((el) => getComputedStyle(el).transform));
  check("hero content moves up on scroll", tyAfter < tyBefore, `${tyBefore} -> ${tyAfter}`);
  check("hero content fades on scroll", opacityAfter < 1, String(opacityAfter));
  check("dashboard parallax (y up to -250)", dashTyAfter < dashTyBefore, `${dashTyBefore} -> ${dashTyAfter}`);
  await page.screenshot({ path: path.join(outDir, "hero-scrolled.png") });

  console.log("\n— Testimonial —");
  const quoteWords = page.locator("[data-testid='testimonial-quote'] > span:not(.ml-2)");
  check("31 word spans rendered", (await quoteWords.count()) === 31, String(await quoteWords.count()));
  check(
    "closing quote mark span",
    await page.locator("[data-testid='testimonial-quote'] span.ml-2.text-muted-foreground").isVisible(),
  );
  ...
  check("words dim before scroll (0.2)", Math.abs(firstWordTop.opacity - 0.2) < 0.05, String(firstWordTop.opacity));
  ...
  const sampled = await page.evaluate(() => {
    const spans = [
      ...document.querySelectorAll("[data-testid='testimonial-quote'] > span:not(.ml-2)"),
    ];
    const at = (i) => {
  ...
  const quoteImg = page.locator("[data-testid='testimonial'] img[aria-hidden='true']");
  check("quote symbol image (object-contain)", (await quoteImg.evaluate((el) => getComputedStyle(el).objectFit)) === "contain");
  const avatar = page.locator("img[alt='Brooklyn Simmons']");
  const avatarStyle = await avatar.evaluate((el) => {
    const s = getComputedStyle(el);
    return { radius: s.borderRadius, border: s.borderTopWidth, fit: s.objectFit, loaded: el.naturalWidth > 0 };
  });
  check(
    "avatar round, 3px border, object-cover, loaded",
    avatarStyle.radius === "9999px" && avatarStyle.border === "3px" && avatarStyle.fit === "cover" && avatarStyle.loaded,
  ...
  check("Sign In still visible on mobile", await mobile.locator("header button", { hasText: "Sign In" }).isVisible());
