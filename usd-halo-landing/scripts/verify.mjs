...
try {
  await new Promise((resolve, reject) => {
    ...
    preview.on('exit', (code) => reject(new Error(`vite preview exited early (code ${code})`)))
  })
  ...
  await page.route(/cloudfront\.net|images\.higgs\.ai|fonts\.googleapis/, (route) => route.abort())
  ...
  check('page title', (await page.title()).includes('Halo'), await page.title())

  // Hero
  const h1 = await page.locator('h1').innerText()
  check('h1 = "Your Wealth / Works"', /Your Wealth\s*\n?\s*Works/.test(h1), JSON.stringify(h1))
  check(
    'h1 letter-spacing -0.04em',
    (await page.locator('h1').getAttribute('style'))?.includes('-0.04em') ?? false,
  )
  ...
  // Navbar
  check('5 nav links, hidden-below-md container', await page.locator('nav .hidden.md\\:flex a').count() === 5)
  for (const label of ['Open Wallet', 'Join us', 'Discover it']) {
    check(`button "${label}"`, await page.locator(`button:has-text("${label}")`).count() === 1)
  }
  ...
  check('2 background videos', videoStats.length === 2, JSON.stringify(videoStats))
  check(
  ...
  // Marquees
  check('hero marquee renders 7 brands twice', await page.locator('.marquee-track > span').count() === 14)
  check('backers marquee renders 8 brands twice', await page.locator('.backers-track > span').count() === 16)
  const heroAnim = await page
    .locator('.marquee-track')
    .evaluate((el) => getComputedStyle(el).animation)
  check('hero marquee animation 22s linear infinite', /22s linear .*infinite/.test(heroAnim), heroAnim)
  const backersAnim = await page
    .locator('.backers-track')
    .evaluate((el) => getComputedStyle(el).animation)
  check('backers marquee animation 30s linear infinite', /30s linear .*infinite/.test(backersAnim), backersAnim)
  ...
  // Section copy
  for (const text of ['Meet USD Halo.', 'Use modes', 'Commerce', 'Savings that bloom', 'Know more']) {
    check(`copy present: "${text}"`, (await page.getByText(text, { exact: false }).count()) >= 1)
  }

  // Page + card backgrounds
  const bodyBg = await page.evaluate(() => getComputedStyle(document.body).backgroundColor)
  check('page background #F5F5F5', bodyBg === 'rgb(245, 245, 245)', bodyBg)
  const darkCards = await page.$$eval('div', (els) =>
    els.filter((el) => getComputedStyle(el).backgroundColor === 'rgb(43, 38, 68)').length,
  )
