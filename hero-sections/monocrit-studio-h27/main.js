(function () {
  ...
  var CAPTIONS = ["FIELD STUDY", "TYPE SPECIMEN", "MOTION FRAME", "BRAND MARK", "EDITORIAL", "STILL LIFE"];
  ...
  /* ---- entrance ---- */
  chars.forEach(function (c, i) { c.style.transitionDelay = 0.18 + i * 0.035 + "s"; });
  requestAnimationFrame(function () {
    requestAnimationFrame(function () { app.classList.add("is-in"); });
  });
  ...
  if (coarse || reduce) {
    var auto = setInterval(function () { setImage(current + 1); }, 1800);
    if (reduce && !coarse) {
    ...
    for (var i = 0; i < chars.length; i++) {
      ...
      if (dist < 150) {
        ...
        chars[i].style.transform =
          "translate(" + (-Math.cos(ang) * push) + "px," + (-Math.sin(ang) * push) + "px)";
      } else {
  ...
  hero.addEventListener("mouseleave", function () {
    chars.forEach(function (c) { c.style.transform = "translate(0,0)"; });
  });
  ...
  function loop() {
    ...
    card.style.transform =
      "translate(" + pos.x + "px," + pos.y + "px) translate(-50%,-50%) rotate(" + rot + "deg) skewY(" + skew + "deg)";
