...
(function () {
  ...
  var io = new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      ...
      if (net && !net.dataset.done) { net.dataset.done = "1"; countUp(net); }
      ...
      if (chart && !chart.classList.contains("grow")) {
        chart.querySelectorAll(".bar").forEach(function (bar, i) {
          setTimeout(function () { bar.style.height = getComputedStyle(bar).getPropertyValue("--h"); }, reduced ? 0 : i * 70);
        });
  ...
  setTimeout(function () {
    document.querySelectorAll("[data-reveal]:not(.in)").forEach(function (el) {
      ...
      if (r.top < window.innerHeight) {
        ...
        if (chart && !chart.classList.contains("grow")) chart.classList.add("grow");
      }
    });
  }, 1200);
  ...
  if (solEl && !reduced) {
    ...
    setInterval(function () {
      ...
      setTimeout(function () { solEl.style.opacity = "1"; }, 120);
    }, 2600);
